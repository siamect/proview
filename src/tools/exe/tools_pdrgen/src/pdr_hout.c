...
static void pdefine(char *name,
                    char *num);

static void puldefine(char *name,
                      char *num);

static int define_printed(proc_list *stop,
                           version_list *start);

static void pprogramdef(definition *def);

static void pprocdef(proc_list *proc,
                     version_list *vp);
...
static  void pdeclaration(char *name,
                          declaration *dec,
                          int tab);

static int undefined2(char *type,
                      char *stop);
...
print_datadef(definition *def)
{
	...
	if (def->def_kind != DEF_PROGRAM && def->def_kind != DEF_CONST) {
		f_print(fout, "pwr_tBoolean pdr_%s(PDR *pdrs, %s *objp);\n", def->def_name, def->def_name);
	}
	if (def->def_kind != DEF_CONST) {
		f_print(fout, "\n");
	}
