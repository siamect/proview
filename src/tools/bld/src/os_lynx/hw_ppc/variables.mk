...
log_done	=
csetos		:= -DOS_LYNX=1 -DOS=lynx -DHW_PPC=1 -DHW=ppc
...
ifeq ($(pwre_btype),rls)
  cflags	:= -O3 -DPOSIX_SOURCE -DPWR_NDEBUG
  cxxflags	:= $(cflags) 
  linkflags	:= -L/lib/thread -L$(lib_dir)
  clis		= /lis=$(list)
  dolist	= /lis=$(list)
  domap		= -Xlinker -Map -Xlinker $(map)
else
  cflags	:= -g -Wall -DPOSIX_SOURCE -D_POSIX_THREADS_CALLS
  cxxflags	:= $(cflags) 
