...
int re_compile_pattern (pattern, size, bufp)
     char *pattern;
     int size;
...
# endif
 ...
 end_of_pattern:
  return REG_BADPAT; /* "Premature end of regular expression";*/

 nesting_too_deep:
  return REG_BADPAT; /*	"Nesting too deep";*/

 too_big:
  return REG_ESPACE; /* "Regular expression too big";*/
...
)
{
  ...
  static char upcase[0400] = {
    ...
    0300, 0301, 0302, 0303, 0304, 0305, 0306, 0307,
    0310, 0311, 0312, 0313, 0314, 0315, 0316, 0317,
    0360, 0321, 0322, 0323, 0324, 0325, 0326, 0327,
    0330, 0331, 0332, 0333, 0334, 0335, 0376, 0377
  };

  preg->translate = NULL;
  obscure_syntax = RE_SYNTAX_POSIX_BASIC;
  if (cflags != 0) {
    if (cflags & REG_EXTENDED) obscure_syntax = RE_SYNTAX_POSIX_EXTENDED;
    if (cflags & REG_NEWLINE) return REG_EFATAL; 
    if (cflags & REG_ICASE) preg->translate = upcase;
  }
  preg->allocated = 40;
  preg->buffer = (char *) malloc(preg->allocated);
  preg->fastmap = (char *) malloc(1 << BYTEWIDTH);
  sts = re_compile_pattern(pattern, strlen(pattern), (struct re_pattern_buffer *) preg);
  if (sts != REG_OK) {
...
)
{
  ...
  if (nmatch > RE_NREGS) nmatch = RE_NREGS;
  if (pmatch != NULL) {
    for (i = 0; i < nmatch; i ++) {
