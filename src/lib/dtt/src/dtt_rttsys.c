...
#define SHOW_SUBCLI_PAGESIZE 16
  ...
  ***********************************************************/
  ...
  case RTT_APPL_UPDATE:
    ...
    SHOW_SUBCLI_LOCAL = gdbroot->my_node->subc_lc;
    SHOW_SUBCLI_UNKNOWN = gdbroot->no_node->subc_lc;
      ...
      for (nl = pool_Qsucc(&sts, gdbroot->pool, &gdbroot->db->nod_lh);
        ...
        if ((k >= page * SHOW_NODES_PAGESIZE)
          ...
          case co_eOS_Lynx:
            ...
            break;
          case co_eOS_Linux:
            strcpy(menu_ptr->value_ptr, "Linux");
            break;
          case co_eOS_MacOS:
            strcpy(menu_ptr->value_ptr, "MacOS");
            break;
          case co_eOS_FreeBSD:
            strcpy(menu_ptr->value_ptr, "FreeBSD");
            break;
          case co_eOS_OpenBSD:
            strcpy(menu_ptr->value_ptr, "OpenBSD");
            break;
          case co_eOS_Cygwin:
            strcpy(menu_ptr->value_ptr, "Cygwin");
            break;
          default:
            strcpy(menu_ptr->value_ptr, "Unknwn");
          }
          ...
          case co_eHW_x86:
            ...
            break;
          case co_eHW_x86_64:
            strcpy(menu_ptr->value_ptr, "x86_64");
            break;
          case co_eHW_68k:
            strcpy(menu_ptr->value_ptr, "68k");
            break;
          case co_eHW_PPC:
            strcpy(menu_ptr->value_ptr, "PPC");
            break;
          case co_eHW_ARM:
  ...
  ***********************************************************/
  if (event == RTT_APPL_INIT) {
    ...
    case 1:
      ...
      break;
    case 3:
      strcpy(menu_ptr->value_ptr, "I+P");
      break;
    case 6:
      strcpy(menu_ptr->value_ptr, "P");
      break;
    case 7:
      strcpy(menu_ptr->value_ptr, "PI");
      break;
    case 11:
      strcpy(menu_ptr->value_ptr, "I+PD");
      break;
    case 14:
      strcpy(menu_ptr->value_ptr, "P+D");
      break;
    case 15:
      strcpy(menu_ptr->value_ptr, "PI+D");
      break;
    case 30:
      strcpy(menu_ptr->value_ptr, "PD");
      break;
    case 31:
      strcpy(menu_ptr->value_ptr, "PID");
      break;
    default:
...
int rttsys_start_system_picture(menu_ctx ctx, char* picture_name)
{
  ...
  if (cdh_NoCaseStrncmp(picture_name, "SYSTEM", strlen(picture_name)) == 0) {
    ...
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "PLCPGM", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "PLCPGM", "PLCPGM", 0, &RTTSYS_PLCPGM);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "GRAFCET", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "GRAFCET", "GRAFCET", 0, &RTTSYS_GRAFCET);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "DEVICE", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "DEVICE", "DEVICE", 0, &RTTSYS_DEVICE);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "PLCTHREAD", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "PLCTHREAD", "PLCTHREAD", 0, &RTTSYS_PLCTHREAD);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "PID", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(ctx, pwr_cNObjid, "PID", "PID", 0, &RTTSYS_PID);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "LOGGING", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "LOGGING", "LOGGING", 0, &RTTSYS_LOGGING);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "NODES", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "NODES", "NODES", 0, &RTTSYS_SHOW_NODES);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "SUBCLI", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "SUBCLI", "SUBCLI", 0, &RTTSYS_SHOW_SUBCLI);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "SUBSRV", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "SUBSRV", "SUBSRV", 0, &RTTSYS_SHOW_SUBSRV);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "NMPSCELL", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "NMPSCELL", "NMPSCELL", 0, &RTTSYS_NMPSCELL);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "REMNODE", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "REMNODE", "REMNODE", 0, &RTTSYS_REMNODE);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "REMTRANS", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
    ...
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "QQUEUES", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(ctx, pwr_cNObjid, "QCOM QUEUES", "QCOM QUEUES",
        0, &RTTSYS_QCOM_ALLQUEUES);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "QNODES", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "QCOM NODES", "QCOM NODES", 0, &RTTSYS_QCOM_NODES);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "QLINKS", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "QCOM LINKS", "QCOM LINKS", 0, &RTTSYS_QCOM_LINKS);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "POOLS", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "POOLS", "POOLS", 0, &RTTSYS_POOLS);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "CACHE", strlen(picture_name))
      == 0) {
    sts = rtt_menu_new_sysedit(
        ctx, pwr_cNObjid, "CACHE", "CACHE", 0, &RTTSYS_CACHE);
    return sts;
  } else if (cdh_NoCaseStrncmp(picture_name, "ERROR", strlen(picture_name))
      == 0) {
...
#define CHANDO_PAGESIZE 20
  ...
  ***********************************************************/
  if (event == RTT_APPL_INIT) {
    ...
    if (signal_test_mode)
      strcpy(CHANDO_INFO, " RETURN show signal | c/A testvalue 1/0 | c/T test "
                          "on/off | c/R back   ");
    else
      strcpy(CHANDO_INFO, " RETURN show signal | c/A show channel | c/T show "
                          "cross | c/R back     ");
    ...
    void* arg1, void* arg2, void* arg3, void* arg4)
{
  ...
  strcat(name, ".TestOn");
  sts = gdh_GetObjectInfo(name, &test_on, sizeof(test_on));
  if (EVEN(sts))
    return sts;
  ...
  if (event == RTT_APPL_PREVPAGE || event == RTT_APPL_NEXTPAGE) {
    ...
    for (i = 0; i < devicelist_count; i++) {
      if ((i >= page * DEVICE_PAGESIZE) && (i < (page + 1) * DEVICE_PAGESIZE)) {
        ...
        menu_ptr++;
        *(pwr_tObjid*)menu_ptr->value_ptr = devicelist_ptr->device_class_objid;
        menu_ptr++;
  ...
  if (event == RTT_APPL_PREVPAGE || event == RTT_APPL_NEXTPAGE) {
    ...
    for (i = 0; i < remtranslist_count; i++) {
      if ((i >= page * REMTRANS_PAGESIZE)
        ...
        menu_ptr++;
        menu_ptr->value_ptr = (char*)&remtranslist_ptr->remtrans_ptr->DataValid;
        menu_ptr++;
        ...
        menu_ptr++;
        menu_ptr->value_ptr = (char*)&remtranslist_ptr->remtrans_ptr->ErrCount;
        menu_ptr++;
        menu_ptr->value_ptr = (char*)&remtranslist_ptr->remtrans_ptr->LastSts;
        menu_ptr++;
        j++;
      }
      remtranslist_ptr++;
