...
static void init()
{
  ...
  if (cnf_get_value("statusMonitorTimeout", timeoutstr, sizeof(timeoutstr)) != 0
    ...
    soap.recv_timeout
        = -1000000 * timeout; // Negative value is us, positive is s
  else
    ...
    const char* nodename, statussrv_sGetStatus* result)
{
  ...
  if (soap_call___s0__GetStatus(
          &soap, endpoint, NULL, &get_status, &get_status_response)
      == SOAP_OK) {
    strncpy(result->Version, get_status_response.Version.c_str(),
        sizeof(result->Version));
    result->SystemStatus = get_status_response.SystemStatus;
    strncpy(result->SystemStatusStr,
        get_status_response.SystemStatusStr.c_str(),
        sizeof(result->SystemStatusStr));
    if (get_status_response.Description)
      strncpy(result->Description, get_status_response.Description->c_str(),
    ...
    if (get_status_response.SystemTime)
      time_AsciiToA(
          (char*)get_status_response.SystemTime->c_str(), &result->SystemTime);
    else
      memset(&result->SystemTime, 0, sizeof(result->SystemTime));

    if (get_status_response.BootTime)
      time_AsciiToA(
          (char*)get_status_response.BootTime->c_str(), &result->BootTime);
    else
      memset(&result->BootTime, 0, sizeof(result->BootTime));

    if (get_status_response.RestartTime)
      time_AsciiToA((char*)get_status_response.RestartTime->c_str(),
          &result->RestartTime);
    else
    ...
    if (get_status_response.UserStatusStr1)
      strncpy(result->UserStatusStr[0],
          get_status_response.UserStatusStr1->c_str(),
          sizeof(result->UserStatusStr[0]));
    else
      strcpy(result->UserStatusStr[0], "");

    result->UserStatus[1] = get_status_response.UserStatus2;
    if (get_status_response.UserStatusStr2)
      strncpy(result->UserStatusStr[1],
          get_status_response.UserStatusStr2->c_str(),
          sizeof(result->UserStatusStr[0]));
    else
    ...
    if (get_status_response.UserStatusStr3)
      strncpy(result->UserStatusStr[2],
          get_status_response.UserStatusStr3->c_str(),
          sizeof(result->UserStatusStr[0]));
    else
    ...
    if (get_status_response.UserStatusStr4)
      strncpy(result->UserStatusStr[3],
          get_status_response.UserStatusStr4->c_str(),
          sizeof(result->UserStatusStr[0]));
    else
    ...
    if (get_status_response.UserStatusStr5)
      strncpy(result->UserStatusStr[4],
          get_status_response.UserStatusStr5->c_str(),
          sizeof(result->UserStatusStr[0]));
    else
    ...
    const char* nodename, statussrv_sGetExtStatus* result)
{
  ...
  if (soap_call___s0__GetExtStatus(
          &soap, endpoint, NULL, &get_status, &get_status_response)
      == SOAP_OK) {
    ...
    strncpy(result->ServerStsName[0],
        get_status_response.ServerSts1Name.c_str(),
    ...
    strncpy(result->ServerStsStr[1], get_status_response.ServerSts2Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[1],
        get_status_response.ServerSts2Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[2] = get_status_response.ServerSts3;
    strncpy(result->ServerStsStr[2], get_status_response.ServerSts3Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[2],
        get_status_response.ServerSts3Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[3] = get_status_response.ServerSts4;
    strncpy(result->ServerStsStr[3], get_status_response.ServerSts4Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[3],
        get_status_response.ServerSts4Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[4] = get_status_response.ServerSts5;
    strncpy(result->ServerStsStr[4], get_status_response.ServerSts5Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[4],
        get_status_response.ServerSts5Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[5] = get_status_response.ServerSts6;
    strncpy(result->ServerStsStr[5], get_status_response.ServerSts6Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[5],
        get_status_response.ServerSts6Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[6] = get_status_response.ServerSts7;
    strncpy(result->ServerStsStr[6], get_status_response.ServerSts7Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[6],
        get_status_response.ServerSts7Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[7] = get_status_response.ServerSts8;
    strncpy(result->ServerStsStr[7], get_status_response.ServerSts8Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[7],
        get_status_response.ServerSts8Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[8] = get_status_response.ServerSts9;
    strncpy(result->ServerStsStr[8], get_status_response.ServerSts9Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[8],
        get_status_response.ServerSts9Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[9] = get_status_response.ServerSts10;
    strncpy(result->ServerStsStr[9], get_status_response.ServerSts10Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[9],
        get_status_response.ServerSts10Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[10] = get_status_response.ServerSts11;
    strncpy(result->ServerStsStr[10],
        get_status_response.ServerSts11Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[10],
        get_status_response.ServerSts11Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[11] = get_status_response.ServerSts12;
    strncpy(result->ServerStsStr[11],
        get_status_response.ServerSts12Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[11],
        get_status_response.ServerSts12Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[12] = get_status_response.ServerSts13;
    strncpy(result->ServerStsStr[12],
        get_status_response.ServerSts13Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[12],
        get_status_response.ServerSts13Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[13] = get_status_response.ServerSts14;
    strncpy(result->ServerStsStr[13],
        get_status_response.ServerSts14Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[13],
        get_status_response.ServerSts14Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[14] = get_status_response.ServerSts15;
    strncpy(result->ServerStsStr[14],
        get_status_response.ServerSts15Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[14],
        get_status_response.ServerSts15Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[15] = get_status_response.ServerSts16;
    strncpy(result->ServerStsStr[15],
        get_status_response.ServerSts16Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[15],
        get_status_response.ServerSts16Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[16] = get_status_response.ServerSts17;
    strncpy(result->ServerStsStr[16],
        get_status_response.ServerSts17Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[16],
        get_status_response.ServerSts17Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[17] = get_status_response.ServerSts18;
    strncpy(result->ServerStsStr[17],
        get_status_response.ServerSts18Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[17],
        get_status_response.ServerSts18Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[18] = get_status_response.ServerSts19;
    strncpy(result->ServerStsStr[18],
        get_status_response.ServerSts19Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[18],
        get_status_response.ServerSts19Name.c_str(),
        sizeof(result->ServerStsName[0]));

    result->ServerSts[19] = get_status_response.ServerSts20;
    strncpy(result->ServerStsStr[19],
        get_status_response.ServerSts20Str.c_str(),
        sizeof(result->ServerStsStr[0]));
    strncpy(result->ServerStsName[19],
        get_status_response.ServerSts20Name.c_str(),
        sizeof(result->ServerStsName[0]));
...
pwr_tStatus statussrv_Restart(const char* nodename)
{
  ...
  if (soap_call___s0__Restart(
          &soap, endpoint, NULL, &restart, &restart_response)
      == SOAP_OK) {
  } else {
    sts = PWR__SRVCONNECTION;
  }
    ...
    const char* lang, const char* display, const char* gui)
{
  ...
  if (soap_call___s0__XttStart(
          &soap, endpoint, NULL, &xtt_start, &xtt_start_response)
      == SOAP_OK) {
  } else {
    sts = PWR__SRVCONNECTION;
  }
    ...
    const char* display, const char* gui)
{
  ...
  if (soap_call___s0__RtMonStart(
          &soap, endpoint, NULL, &rtmon_start, &rtmon_start_response)
      == SOAP_OK) {
  } else {
    sts = PWR__SRVCONNECTION;
  }
