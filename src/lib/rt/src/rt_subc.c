...
static void deleteClient(sub_sClient* cp)
{
  ...
  if (EVEN(sts))
    errh_Bugcheck(sts, "remove client from hash table");

  np = hash_Search(&sts, gdbroot->nid_ht, &cp->nid);
  if (np == NULL)
    errh_Bugcheck(sts, "node not found");
  ...
  if (cp->cclass != pool_cNRef) {
    ...
    if (ccp == NULL)
      errh_Bugcheck(GDH__WEIRD, "cached class address");
...
static gdb_sNode* testClient(pwr_tStatus* sts, sub_sClient* cp)
{
      ...
      do {
        ...
        if (!op->u.c.flags.b.classChecked || !op->u.c.flags.b.classEqual) {
          ...
          if (equal) {
            ...
            if (ccr == pool_cNRef)
              errh_Bugcheck(GDH__WEIRD, "cache class address to reference");
          ...
          /** @todo Check if we can do it more efficient, eg.
           * vol_ArefToAttribute */
          if (fetched) {
            np = NULL;
            op = NULL;
  ...
  if (rcp == cp) { /* OK, it exists! */
    cp->sts = lsts;
    if (op != NULL) {
      if (gdbroot->db->log.b.sub)
        errh_Info("Test client %s", op->g.f.name.orig);
      vp = pool_Address(NULL, gdbroot->pool, op->l.vr);
...
void subc_ActivateList(pool_sQlink* lh, pwr_tObjid oid)
{
  ...
  for (aep = tree_Minimum(&sts, add); aep != NULL;
       aep = tree_Successor(&sts, add, aep)) {
    if (aep->msg != NULL) {
      if (aep->msg->count > 0) {
...
void subc_CancelUser(pid_t subscriber)
{
  ...
  for (nl = pool_Qsucc(NULL, gdbroot->pool, &gdbroot->db->nod_lh);
    ...
    for (cl = pool_Qsucc(NULL, gdbroot->pool, &np->subc_lh);
         cl != &np->subc_lh;) {
      cp = pool_Qitem(cl, sub_sClient, subc_ll);
      cl = pool_Qsucc(NULL, gdbroot->pool, cl);
