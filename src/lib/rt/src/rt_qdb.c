...
static int copyBufferData(void* itp, qdb_sBuffer* bp, int offs, int size);
...
static qdb_sLocal* mapLocalDb(pwr_tStatus* sts)
{
  hash_sTable* ht;
  ...
  if (ht == NULL)
    errh_Bugcheck(*sts, "initiating qix hash table");

  ht = hash_Create(sts, &qdb->pool, &qdb->aix_ht, &qdb->g->aix_ht, NULL, NULL);
  if (ht == NULL)
    errh_Bugcheck(*sts, "initiating aix hash table");

  ht = hash_Create(sts, &qdb->pool, &qdb->pid_ht, &qdb->g->pid_ht, NULL, NULL);
  if (ht == NULL)
    errh_Bugcheck(*sts, "initiating pid hash table");

  return qdb;
}

qdb_sAppl* qdb_AddAppl(pwr_tStatus* status, pwr_tBoolean system)
{
  qdb_sAppl* ap;
...
qdb_sNode* qdb_AddNode(pwr_tStatus* status, pwr_tUInt32 nid, pwr_tBitMask flags)
{
  qdb_sNode* np;
  pwr_dStatus(sts, status, QCOM__SUCCESS);
  ...
  if (np == NULL)
    errh_Bugcheck(QCOM__WEIRD, "adding new node identifier");
...
qdb_sQbond* qdb_AddBond(pwr_tStatus* status, qdb_sQue* sq, qdb_sQue* tq)
{
  qdb_sQbond* bp;
  pwr_dStatus(sts, status, QCOM__SUCCESS);
...
void qdb_ApplEvent(pwr_tStatus* status, qdb_sAppl* ap, qcom_eStype event)
{
  qcom_sAppl* appl;
  qdb_sQue* qp;
  qdb_sBuffer* bp;
...
qdb_sQbond* qdb_GetBond(pwr_tStatus* sts, qdb_sQue* sq, qdb_sQue* tq)
{
  pool_sQlink* bol;
  qdb_sQbond* bop;
    ...
    pwr_tStatus* status, qdb_eBuffer btype, unsigned int size)
{
  ...
  // pool_Reference(sts,&qdb->pool,bp),size);
...
void* qdb_CopyBufferData(pwr_tStatus* status, qdb_sBuffer* bp, qcom_sGet* gp)
{
  pwr_dStatus(sts, status, QCOM__SUCCESS);
...
qdb_sBuffer* qdb_Deque(pwr_tStatus* status, qdb_sQue* qp, int tmo)
{
  pool_sQlink* bl;
  ...
  if (bl == NULL)
    pwr_Return(NULL, sts, QCOM__QEMPTY);
  qp->in_lc--;
...
pwr_tBoolean qdb_Enque(pwr_tStatus* status, qdb_sBuffer* bp, qdb_sQue* qp)
{
  pwr_dStatus(sts, status, QCOM__SUCCESS);
...
void qdb_Free(pwr_tStatus* status, qdb_sBuffer* bp)
{
  qdb_sBuffer* bbp;
  qdb_sBuffer* sbp;
  pool_sQlink* sl;
...
void qdb_GetInfo(qcom_sGet* gp, qdb_sBuffer* bp)
{
...
qdb_sLocal* qdb_CreateDb(pwr_tStatus* status, qdb_sInit* ip)
{
  ...
  if (!created)
    errh_Bugcheck(QCOM__WEIRD, "db lock was allready created");
...
qdb_sLocal* qdb_MapDb(pwr_tStatus* status)
{
  ...
  if (created)
    errh_Bugcheck(QCOM__WEIRD, "db lock was not created");
  *sts = sync_MutexInit(&qdb->thread_lock.mutex);
  if (EVEN(*sts))
    return NULL;
  *sts = sync_CondInit(&qdb->thread_lock.cond);
...
qdb_sBuffer* qdb_CopyBuffer(pwr_tStatus* status, qdb_sBuffer* bp)
{
  qdb_sBuffer* nbp;
  pwr_dStatus(sts, status, QCOM__SUCCESS);
...
qdb_sBuffer* qdb_DetachBuffer(pwr_tStatus* status, qdb_sBuffer* bp)
{
  pwr_dStatus(sts, status, QCOM__SUCCESS);
...
void qdb_Put(pwr_tStatus* status, qdb_sBuffer* bp, qdb_sQue* qp)
{
  qdb_sQue* fqp = NULL;
  ...
  case qdb_eQue_forward:
    ...
    for (/* all bound queues */
      ...
      if (fqp->qix == qdb_cIexport && bp->c.flags.b.imported)
        continue;
...
void qdb_RemoveAppl(pwr_tStatus* status, qdb_sAppl* ap)
{
  pool_sQlink* ql;
  pool_sQlink* bl;
  qdb_sBuffer* bp;
...
qdb_sQue* qdb_AddQue(pwr_tStatus* status, qcom_tQix qix)
{
  qdb_sQue* qp;
  pwr_dStatus(sts, status, QCOM__SUCCESS);
...
qdb_sQue* qdb_AttachQue(pwr_tStatus* status, qdb_sQue* qp, qdb_sAppl* ap)
{
  pwr_dStatus(sts, status, QCOM__SUCCESS);
...
void qdb_Eput(pwr_tStatus* status, qdb_sQue* ep)
{
  qdb_sBuffer* bp;
  qcom_sEvent* event;
...
pwr_tBoolean qdb_RemoveQue(pwr_tStatus* status, qdb_sQue* qp)
{
  pool_sQlink* bl;
  pool_sQlink* qbl;
  qdb_sBuffer* bp;
...
void qdb_NetEvent(pwr_tStatus* status, qdb_sNode* np, qcom_eStype event)
{
  qcom_sNode* node;
  qdb_sQue* qp;
  qdb_sBuffer* bp;
...
void qdb_NodeInfo(pwr_tStatus* status, qcom_sNode* node, qdb_sNode* np)
{
...
void qdb_Platform(qdb_sNode* np)
{
...
qdb_sQue* qdb_Que(pwr_tStatus* status, const qcom_sQid* qid, qdb_sNode** onp)
{
  qdb_uQid q;
  qdb_sNode* np = NULL;
  qdb_sQue* qp = NULL;
  ...
  if (q.pwr.nid != pwr_cNNodeId && q.pwr.nid != qdb->g->nid) {
    ...
    if (np == NULL)
      pwr_Return(NULL, sts, QCOM__NOLINK);
    if (!np->link[np->clx].qflags.b.connected)
      pwr_Return(NULL, sts, QCOM__NOLINK);
    if (!np->link[np->clx].qflags.b.active && !qdb->flags.b.ignoreStall)
      pwr_Return(NULL, sts, QCOM__LINKDOWN);
...
pwr_tBoolean qdb_Signal(pwr_tStatus* status, qdb_sQue* ep)
{
  pool_sQlink* ql;
  qdb_sQue* qp;
...
int qdb_Wait(pwr_tStatus* status, qdb_sQue* qp, qdb_sQue* ep, int tmo)
{
  pwr_dStatus(sts, status, QCOM__SUCCESS);
