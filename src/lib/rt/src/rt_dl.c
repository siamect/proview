...
void dl_Cancel(pwr_tStatus* sts, pwr_tDlid dlid)
{
  ...
  if (dp == NULL)
    errh_Bugcheck(GDH__WEIRD, "hash_Remove");
  ...
  if (op == NULL)
    errh_Bugcheck(GDH__WEIRD, "direct link inconsitency");
...
dl_sLink* dl_Create(pwr_tStatus* sts, mvol_sAttribute* ap, pwr_sAttrRef* arp)
{
  pwr_tStatus lsts;
  dl_sLink* dp;
  dl_sLink* rdp;
