/* 
 * Proview   $Id: rt_io_m_pb_euro2500.c,v 1.2 2005-09-01 14:57:57 claes Exp $
...
/* rt_io_m_pb_euro2500.c -- io methods for the profibus slave */
...

/*----------------------------------------------------------------------------*\
   Init method for the Pb slave EURO2500
\*----------------------------------------------------------------------------*/
...
) 
{
  ...
  op = (pwr_sClass_Pb_Euro2500 *) rp->op;
  ...
  if (op->Status < 1) {
    ...
    if (sts != PB_OK) {
      errh_Info( "ERROR Init Pb slave Euro2500 %s", rp->Name );
  ...
  if (op->BytesOfInput != input_counter || op->BytesOfOutput != output_counter) {
    errh_Info( "Configuration mismatch in Pb slave Euro2500 %s", rp->Name);
//    op->Status = 0;
//    return IO__SUCCESS;
  }

  op->Status = 1;
  errh_Info( "Init Pb slave Euro2500 %s", rp->Name );
  return IO__SUCCESS;
}


/*----------------------------------------------------------------------------*\
   Read method for the Pb slave Euro2500
\*----------------------------------------------------------------------------*/
...

/*----------------------------------------------------------------------------*\
   Write method for the Pb slave Euro2500
\*----------------------------------------------------------------------------*/
...
pwr_dExport pwr_BindIoMethods(Pb_Euro2500) = {
