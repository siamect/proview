/* 
 * Proview   $Id: rt_io_m_pb_et200m.c,v 1.2 2005-09-01 14:57:57 claes Exp $
...
/* rt_io_m_pb_et200m.c -- io methods for the profibus slave
   SIEMENS ET200M
*/
...

/*----------------------------------------------------------------------------*\
   Init method for the Pb slave ET200M
\*----------------------------------------------------------------------------*/
...
) 
{
  ...
  if (op->BytesOfInput != input_counter || op->BytesOfOutput != output_counter) {
    errh_Info( "Configuration mismatch in Pb slave ET200M %s", rp->Name);
    op->Status = 0;
    return IO__SUCCESS;
  }

  op->Status = 1;
  errh_Info( "Init Pb slave ET200M %s", rp->Name );
  return IO__SUCCESS;
}


/*----------------------------------------------------------------------------*\
   Read method for the Pb slave ET200M
\*----------------------------------------------------------------------------*/
...

/*----------------------------------------------------------------------------*\
   Write method for the Pb slave ET200M
\*----------------------------------------------------------------------------*/
...
pwr_dExport pwr_BindIoMethods(Pb_ET200M) = {
