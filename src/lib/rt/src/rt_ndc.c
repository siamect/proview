  ...
  for (; i<cp->acount&& * size> 0; i++, ap++) {
    ...
    if (raidx == UINT_MAX || cidx == conv_eIdx_invalid) {
      ...
      *size -= zsize;
       ...
       * add a flag to the attribute reference that indicates single array
       * element.
       */
      adef = cap->flags;
        ...
        if (ap->flags.b.isclass) {
          /*          const gdb_sClass  *lcp = hash_Search(sts, gdbroot->cid_ht,
             &ap->tid);
                    ...
                    if ((lcp == NULL) || (lccp == NULL))
             errh_Bugcheck(GDH__WEIRD, "can't get class");

                    if (!lccp->flags.b.rnConv) {
                      ndc_sRemoteToNative   *ltbl;
                      ltbl = pool_Alloc(sts, gdbroot->pool, sizeof(*tbl) *
             lcp->acount);
                      ndc_UpdateRemoteToNativeTable(sts, ltbl, lcp->acount, lcp,
             lccp, nid);
                      if (ODD(*sts)) {
                        lccp->rnConv = pool_Reference(NULL, gdbroot->pool,
             ltbl);
                        lccp->flags.b.rnConv = 1;
                      } else {
                        pool_Free(NULL, gdbroot->pool, ltbl);
