...
gdb_sClass* mvol_LinkClass(pwr_tStatus* sts, gdb_sClass* cp, pwr_tBitMask ilink)
{
  cdh_uObjid coid;
  pwr_sObjBodyDef* bp;
  pwr_sParam* abp;
  ...
  if (bp == NULL)
    errh_Bugcheck(MVOL__NOBODYDEF, "body missing");
    ...
    pwr_tStatus* sts, gdb_sVolume* vp, gdb_sObject* op, pwr_tBitMask ilink)
{
  ...
  if (cdh_isClassVolumeClass(vp->g.cid)) {
    ...
    case pwr_eClass_ClassDef:
      ...
      if (scp == NULL)
        errh_Bugcheck(*sts, "add classdef object");

      if (scp->cor != pool_cNRef)
        printf("scp->cor != pool_cNRef, %s\n", op->g.f.name.orig);
      scp->cor = pool_ItemReference(sts, gdbroot->pool, op);
      if (scp->cbr != pool_cNRef)
        printf("scp->cbr != pool_cNRef, %s\n", op->g.f.name.orig);
      scp->cbr = op->u.n.body;
      if (!decodeClassDef(sts, op, vp->u.n.format.b.bo))
        return NULL;
      break;
    ...
    case pwr_eClass_ObjBodyDef:
      ...
      scp->bbr = op->u.n.body;
      if (!decodeObjBodyDef(sts, op, vp->u.n.format.b.bo))
        return NULL;
      break;
    ...
    case pwr_eClass_Buffer:
    case pwr_eClass_Param:
      if (!decodeAttribute(sts, op, vp->u.n.format.b.bo))
        return NULL;
      break;
    }
  }
