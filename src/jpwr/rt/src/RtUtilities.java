...
public class RtUtilities {
  ...
  public static int createInstanceFile( String from_name, String to_name,
    ...
    try {
      ...
      // Replace åäö in instance name because mozilla can't pass them as 
      // parameter
      for ( int i = 0; i < instance.length(); i++) {
        if ( instanceBuff[i] == 'å' || instanceBuff[i] == 'å')
          instanceBuff[i] = '\\';
        if ( instanceBuff[i] == 'ä' || instanceBuff[i] == 'Ä')
          instanceBuff[i] = '/';
        if ( instanceBuff[i] == 'ö' || instanceBuff[i] == 'Ö')
          instanceBuff[i] = '@';
      }
