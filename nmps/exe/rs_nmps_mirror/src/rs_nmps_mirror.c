...
#define NMPS_CELLBUFF_SIZE                                                     \
  1000 /* This must be >= the size of                                          \
          the largest cell */
#define NMPS_DATABUFF_SIZE                                                     \
  1000 /* This is the maximum size of                                          \
          data subscriptions */
...
static pwr_tStatus nmpsmir_find_collect_cell(mir_ctx mirctx,
    pwr_tObjid collect_cell, pwr_sClass_NMpsCell** collect_cell_ptr);
pwr_tStatus nmpsmir_data_db_delete(
    nmpsmir_t_data_list** data_list, nmpsmir_t_data_list* data_ptr);
...
static pwr_tStatus nmpsmir_data_db_create(mir_ctx mirctx,
    nmpsmir_t_data_list** data_list, pwr_tObjid objid, pwr_tObjid mircell_objid,
    int convert, pwr_sClass_NMpsConvConfig* convconfig,
    nmpsmir_t_data_list** datalist_ptr);
static pwr_tStatus nmpsmir_data_db_find_orig(nmpsmir_t_data_list* data_list,
    pwr_tObjid objid, pwr_tObjid mircell_objid,
    nmpsmir_t_data_list** datalist_ptr);
static pwr_tStatus nmpsmir_data_db_find_orig_name(
    nmpsmir_t_data_list* data_list, pwr_tObjid objid, pwr_tObjid mircell_objid,
    nmpsmir_t_data_list** datalist_ptr);
static pwr_tStatus nmpsmir_cellmirlist_add(mir_ctx mirctx, pwr_tObjid objid,
    nmpsmir_t_cellmir_list** cellmirlist, int* cellmirlist_count);
static pwr_tStatus nmpsmir_convconfiglist_add(pwr_tObjid objid,
    nmpsmir_t_convconfig_list** convconfiglist, int* convconfiglist_count);
...
*
**************************************************************************/
static pwr_tStatus nmpsmir_bckremoved_store(nmpsmir_t_bckremoved** bckremlist,
    pwr_tObjid objid, nmpsmir_t_bckremoved** bckremlist_ptr)
{
...
*
**************************************************************************/
pwr_tStatus nmpsmir_bckremoved_delete(nmpsmir_t_bckremoved** bckremoved_list,
...
*
**************************************************************************/
static int nmnps_time_to_release(
    pwr_tTime* remove_time, pwr_tTime* current_time, float release_time)
{
  /***********
          pwr_tTime	diff_time;
          float		time;
...
static pwr_tStatus nmpsmir_data_copy(nmpsmir_t_data_list* datalist_ptr)
{
  ...
  if (!datalist_ptr->local) {
    sts = gdh_SubData(datalist_ptr->orig_data_subid, databuf, sizeof(databuf));
    if (sts == GDH__SUBOLD)
      return sts;
    else if (EVEN(sts)) {
    ...
    from_ptr = databuf + datalist_ptr->convconfig->CopyOffset;
  } else
    from_ptr
        = datalist_ptr->orig_data_ptr + datalist_ptr->convconfig->CopyOffset;
  to_ptr = datalist_ptr->data_ptr + datalist_ptr->convconfig->PasteOffset;
...
static pwr_tStatus nmpsmir_data_handler(mir_ctx mirctx)
{
    ...
    if (data_ptr->removed) {
      ...
      if (nmnps_time_to_release(
        ...
        if (data_ptr->collect) {
          ...
          sts = nmpsmir_find_collect_cell(
              mirctx, data_ptr->collect_cell, &collect_cell_ptr);
          if (EVEN(sts))
  ...
  bckremoved_ptr = (struct nmpsmir_bckremoved_tag*)mirctx->bckremoved_list;
  while (bckremoved_ptr != NULL) {
    /* Check time since remove */
    if (nmnps_time_to_release(&bckremoved_ptr->remove_time, &current_time,
            bckremoved_ptr->release_time)) {
      mirctx->RemoveDataCount++;

      /* Put into collect cell */
      sts = nmpsmir_find_collect_cell(
          mirctx, bckremoved_ptr->collect_cell, &collect_cell_ptr);
      if (EVEN(sts))
        errh_CErrLog(sts, NULL);
      ...
      sts = nmpsmir_bckremoved_delete(&mirctx->bckremoved_list, bckremoved_ptr);
    ...
    nmpsmir_t_data_list** datalist_ptr)
{
  ...
  *data_list = *datalist_ptr;
  ...
  if (convert) {
    ...
    sts = gdh_ObjidToName(
        convconfig->OutDataParent, new_name, sizeof(new_name), cdh_mNName);
    if (EVEN(sts))
      LogAndReturn(NMPS__MIRPARENT, sts);
    strcat(new_name, "-");
    strcat(new_name, name);

    sts = gdh_CreateObject(new_name,
        cdh_ClassObjidToId(convconfig->OutDataClass), 0,
        &(*datalist_ptr)->data_objid, pwr_cNObjid, 0, pwr_cNObjid);
    if (sts == GDH__DUPLNAME) {
      ...
      if (EVEN(sts))
        LogAndReturn(NMPS__DATARECONERR, sts);
    ...
    if (local) {
      ...
      if (EVEN(sts))
        LogAndReturn(NMPS__MIRORIGDATA, sts);
    } else {
      /* Get a subscription to the original object */
      memset(&attrref, 0, sizeof(attrref));
      sts = gdh_ClassAttrToAttrref(
          cdh_ClassObjidToId(convconfig->OutDataClass), NULL, &attrref);
      if (EVEN(sts))
        LogAndReturn(NMPS__MIRORIGDATA, sts);

      attrref.Objid = objid;
      /*	    attrref.Size = convconfig->CopyOffset +
       * convconfig->CopySize;
      */
      /** Test
                  {
      ...
      **/
      sts = gdh_SubRefObjectInfoAttrref(
          &attrref, &(*datalist_ptr)->orig_data_subid);
      if (EVEN(sts))
        LogAndReturn(NMPS__MIRORIGDATA, sts);
    ...
    if (EVEN(sts))
      LogAndReturn(NMPS__MIRDATA, sts);
...
*
**************************************************************************/
static pwr_tStatus nmpsmir_data_db_find(nmpsmir_t_data_list* data_list,
    pwr_tObjid objid, pwr_tObjid mircell_objid,
    nmpsmir_t_data_list** datalist_ptr)
{
  nmpsmir_t_data_list* data_ptr;
...
*
**************************************************************************/
static pwr_tStatus nmpsmir_origcell_init(pwr_tObjid objid, char** orig_cell,
    gdh_tSubid* orig_subid, pwr_tBoolean* orig_local, pwr_tClassId* orig_class)
{
  pwr_sAttrRef attrref;
  pwr_tStatus sts;
  pwr_tAName name;
  ...
  if (EVEN(sts))
    LogAndReturn(NMPS__MIRORIGCELL, sts);
    ...
    nmpsmir_t_cellmir_list** cellmirlist, int* cellmirlist_count)
{
  ...
  if (*cellmirlist_count == 0) {
    *cellmirlist = calloc(1, sizeof(nmpsmir_t_cellmir_list));
    ...
    new_cellmirlist
        = calloc(*cellmirlist_count + 1, sizeof(nmpsmir_t_cellmir_list));
  ...
  if (EVEN(sts))
    LogAndReturn(NMPS__MIRCELL, sts);
  ...
  if (cellmirlist_ptr->data_collect) {
    ...
    sts = gdh_GetObjectClass(cellmirlist_ptr->collect_cell, &class);
    if (EVEN(sts)) {
      errh_CErrLog(NMPS__COLLECTCELL, errh_ErrArgL(objid.oix), NULL);
      return NMPS__COLLECTCELL;
    }

    if (!(class == pwr_cClass_NMpsCell || class == pwr_cClass_NMpsStoreCell
  ...
  for (i = 0; i < (int)cellmirlist_ptr->cell->NumberOfCellObj; i++) {
    ...
    sts = nmpsmir_origcell_init(cellmirlist_ptr->orig_objid[i],
        (char**)&cellmirlist_ptr->orig_cell[i], &cellmirlist_ptr->orig_subid[i],
        &cellmirlist_ptr->orig_local[i], &cellmirlist_ptr->orig_class[i]);
  ...
  if (cellmirlist_ptr->conversion) {
    ...
    for (i = 0; i < mirctx->convconfig_count; i++) {
      if (cdh_ObjidIsEqual(
              convconf_ptr->objid, cellmirlist_ptr->cell->ConvConfig)) {
    ...
    nmpsmir_t_convconfig_list** convconfiglist, int* convconfiglist_count)
{
  ...
  if (*convconfiglist_count == 0) {
    *convconfiglist = calloc(1, sizeof(nmpsmir_t_convconfig_list));
    ...
    new_convconfiglist
        = calloc(*convconfiglist_count + 1, sizeof(nmpsmir_t_convconfig_list));
  ...
  sts = gdh_GetObjectClass(
      convconfiglist_ptr->convconfig->OutDataClass, &class);
  if (EVEN(sts)) {
    errh_CErrLog(NMPS__CONVOUTDATACLASS, errh_ErrArgL(objid.oix), NULL);
    return NMPS__CONVOUTDATACLASS;
  }
  sts = gdh_GetObjectClass(
      convconfiglist_ptr->convconfig->OutDataParent, &class);
  if (EVEN(sts)) {
    errh_CErrLog(NMPS__CONVPARENT, errh_ErrArgL(objid.oix), NULL);
    return NMPS__CONVPARENT;
  }

  sts = gdh_GetObjectClass(convconfiglist_ptr->convconfig->InDataClass, &class);
  if (EVEN(sts)) {
...
static pwr_tStatus nmps_cellmir_init(mir_ctx mirctx)
{
  ...
  while (ODD(sts)) {
    ...
    sts = nmpsmir_convconfiglist_add(
        objid, &mirctx->convconfiglist, &mirctx->convconfig_count);
    if (EVEN(sts))
      return sts;
  ...
  while (ODD(sts)) {
    ...
    sts = nmpsmir_cellmirlist_add(
        mirctx, objid, &mirctx->cellmirlist, &mirctx->cellmir_count);
    if (EVEN(sts))
      return sts;
  ...
  mirctx->OrigCellDownCount = mirctx->OrigCellCount - mirctx->OrigCellInitCount;
      ...
      for (j = 0; j < cellmir_ptr->orig_cell_count; j++) {
        if (cellmir_ptr->orig_notinit[j]) {
          sts = nmpsmir_origcell_init(cellmir_ptr->orig_objid[j],
              (char**)&cellmir_ptr->orig_cell[j], &cellmir_ptr->orig_subid[j],
              &cellmir_ptr->orig_local[j], &cellmir_ptr->orig_class[j]);
          if (sts == NMPS__NODEDOWN) {
            /* Try again later */
  ...
  for (i = 0; i < mirctx->cellmir_count; i++) {
    ...
    for (j = 0; j < cellmir_ptr->orig_cell_count; j++) {
      ...
      if (!cellmir_ptr->orig_local[j]) {
        old = 0;
        sts = gdh_SubData(
            cellmir_ptr->orig_subid[j], orig_cellbuf, sizeof(orig_cellbuf));
        if (EVEN(sts))
          old = 1;
        /*	      sts = gdh_GetSubscriptionOldness(
      ...
      case pwr_cClass_NMpsStoreCell: {
        ...
        if (orig_cell_ptr->MirrorRestart && !cellmir_ptr->orig_restart_old[j]) {
          cellmir_ptr->restart = 1;
          /* Reset the orig cell */
          sts = gdh_ObjidToName(
              cellmir_ptr->orig_objid[j], name, sizeof(name), cdh_mNName);
          if (ODD(sts)) {
      ...
      case pwr_cClass_NMpsMirrorCell: {
        ...
        /* Put the data objids in the orig_objid array */
        if (cellmir_ptr->orig_local[j])
          orig_cell_ptr = (pwr_sClass_NMpsMirrorCell*)cellmir_ptr->orig_cell[j];
        else
    ...
    if (cellmir_ptr->restart && cellmir_ptr->conversion) {
      ...
      for (j = 0; j < orig_data_count; j++) {
        sts = nmpsmir_data_db_find_orig_name(mirctx->data_list,
            cellmir_ptr->orig_data_objid[j], cellmir_ptr->objid, &datalist_ptr);
        if (sts == NMPS__NODEDOWN) {
    ...
    for (j = 0; j < orig_data_count; j++) {
      ...
      if (sts == NMPS__DATANOTFOUND) {
        /* Insert the object */
        sts = nmpsmir_data_db_create(mirctx, &mirctx->data_list,
            cellmir_ptr->orig_data_objid[j], cellmir_ptr->objid,
            cellmir_ptr->conversion, cellmir_ptr->convconfig, &datalist_ptr);
        if (sts == NMPS__ORIGDATA) {
          ...
          for (k = 0; k < cellmir_ptr->cell->TempLastIndex; k++) {
            if (cdh_ObjidIsEqual(copyarea->datainfo[k].DataP.Aref.Objid,
              ...
              datalist_ptr->plc_dlid = copyarea->datainfo[k].Data_Dlid;
              break;
            }
          }
        } else if (EVEN(sts))
    ...
    for (j = 0; j < cellmir_ptr->cell->TempLastIndex; j++) {
      ...
      if (!found) {
        ...
        sts = nmpsmir_data_db_find(mirctx->data_list,
            copyarea->datainfo[j].DataP.Aref.Objid, cellmir_ptr->objid,
            &datalist_ptr);
        if (EVEN(sts)) {
          if (cellmir_ptr->cell->InitFlag
              && (cellmir_ptr->cell->Function & NMPS_CELLFUNC_BACKUP)
              && cellmir_ptr->cell->ReloadDone && cellmir_ptr->data_collect) {
            /* Object is reloaded by backup function but removed */
            /* Insert the object in the backup removed list */
            sts = nmpsmir_bckremoved_store(&mirctx->bckremoved_list,
                copyarea->datainfo[j].DataP.Aref.Objid, &bckremlist_ptr);
            if (ODD(sts)) {
    ...
    if (cellmir_ptr->cell->InitFlag) {
      ...
      for (j = 0; j < orig_data_count; j++) {
        if (count >= cellmir_ptr->cell->MaxSize)
          break;
        if (!cellmir_ptr->data_noinit[j]) {
          copyarea->datainfo[count].DataP.Aref.Objid
              = cellmir_ptr->data_objid[j];
