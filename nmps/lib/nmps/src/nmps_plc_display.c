...
void CellDisp_init(pwr_sClass_CellDisp* object)
{
  ...
  for (;;) {
    sts = gdh_ObjidToName(cdh_ClassIdToObjid(object->DataClass), classname,
  ...
  for (i = 0; i < 5; i++) {
    if (object->FloatAttr[i][0] != 0) {
      ...
      sts = gdh_ClassAttrToAttrref(object->DataClass, attr_str, &attr_ref);
      if (ODD(sts))
...
void CellDisp_exec(plc_sThread* tp, pwr_sClass_CellDisp* object)
{
      ...
      for (i = num; i < num + cell->LastIndex; i++) {
        ...
        if (datap) {
          ...
          if (object->BooleanAttrOffs[0] >= 0)
            object->B1[i]
                = *(pwr_tBoolean*)(datap + object->BooleanAttrOffs[0]);
          if (object->BooleanAttrOffs[1] >= 0)
            object->B2[i]
                = *(pwr_tBoolean*)(datap + object->BooleanAttrOffs[1]);
          if (object->BooleanAttrOffs[2] >= 0)
            object->B3[i]
                = *(pwr_tBoolean*)(datap + object->BooleanAttrOffs[2]);
          if (object->BooleanAttrOffs[3] >= 0)
            object->B4[i]
                = *(pwr_tBoolean*)(datap + object->BooleanAttrOffs[3]);
          if (object->BooleanAttrOffs[4] >= 0)
            object->B5[i]
                = *(pwr_tBoolean*)(datap + object->BooleanAttrOffs[4]);
          if (object->IntAttrOffs[0] != -1) {
            if (object->IntAttrOffs[0] & (1 << 31))
              object->I1[i] = *(
                  pwr_tInt16*)(datap + (object->IntAttrOffs[0] & 0x3fffffff));
            else
              object->I1[i] = *(
                  pwr_tInt32*)(datap + (object->IntAttrOffs[0] & 0x3fffffff));
          }
          if (object->IntAttrOffs[1] != -1) {
            if (object->IntAttrOffs[1] & (1 << 31))
              object->I2[i] = *(
                  pwr_tInt16*)(datap + (object->IntAttrOffs[1] & 0x3fffffff));
            else
              object->I2[i] = *(
                  pwr_tInt32*)(datap + (object->IntAttrOffs[1] & 0x3fffffff));
          }
          if (object->IntAttrOffs[2] != -1) {
            if (object->IntAttrOffs[2] & (1 << 31))
              object->I3[i] = *(
                  pwr_tInt16*)(datap + (object->IntAttrOffs[2] & 0x3fffffff));
            else
              object->I3[i] = *(
                  pwr_tInt32*)(datap + (object->IntAttrOffs[2] & 0x3fffffff));
          }
          if (object->IntAttrOffs[3] != -1) {
            if (object->IntAttrOffs[3] & (1 << 31))
              object->I4[i] = *(
                  pwr_tInt16*)(datap + (object->IntAttrOffs[3] & 0x3fffffff));
            else
              object->I4[i] = *(
                  pwr_tInt32*)(datap + (object->IntAttrOffs[3] & 0x3fffffff));
          }
          if (object->IntAttrOffs[4] != -1) {
            if (object->IntAttrOffs[4] & (1 << 31))
              object->I5[i] = *(
                  pwr_tInt16*)(datap + (object->IntAttrOffs[4] & 0x3fffffff));
            else
              object->I5[i] = *(
                  pwr_tInt32*)(datap + (object->IntAttrOffs[4] & 0x3fffffff));
          }
        }
        if (i == max_size - 1) {
          full = 1;
  ...
  if (object->OldLastIndex > num) {
    /* Reset values */
    memset(&object->Objid[num], 0,
        (object->OldLastIndex - num) * sizeof(pwr_tObjid));
    memset(&object->Select[num], 0,
        (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    memset(&object->OldSelect[num], 0,
        (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    if (object->FloatAttrOffs[0] >= 0)
      memset(&object->F1[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    if (object->FloatAttrOffs[1] >= 0)
      memset(&object->F2[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    if (object->FloatAttrOffs[2] >= 0)
      memset(&object->F3[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    if (object->FloatAttrOffs[3] >= 0)
      memset(&object->F4[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    if (object->FloatAttrOffs[4] >= 0)
      memset(&object->F5[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    if (object->BooleanAttrOffs[0] >= 0)
      memset(&object->B1[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    if (object->BooleanAttrOffs[1] >= 0)
      memset(&object->B2[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    if (object->BooleanAttrOffs[2] >= 0)
      memset(&object->B3[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    if (object->BooleanAttrOffs[3] >= 0)
      memset(&object->B4[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    if (object->BooleanAttrOffs[4] >= 0)
      memset(&object->B5[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    if (object->IntAttrOffs[0] != -1)
      memset(&object->I1[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    if (object->IntAttrOffs[1] != -1)
      memset(&object->I2[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    if (object->IntAttrOffs[2] != -1)
      memset(&object->I3[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    if (object->IntAttrOffs[3] != -1)
      memset(&object->I4[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    if (object->IntAttrOffs[4] != -1)
      memset(&object->I5[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
  }

  memset(&object->Select[object->OldLastIndex], 0,
      (max_size - object->OldLastIndex) * sizeof(pwr_tBoolean));
  ...
  if (link && link->DoSelectNext) {
    for (i = 0; i < num; i++) {
      if (object->Select[i]) {
        if (((i == num - 1 && object->SelDirection == NMPS_DISP_DIRECT_FORW)
                || (i == 0 && object->SelDirection != NMPS_DISP_DIRECT_FORW))
...
void CellUpdate_exec(plc_sThread* tp, pwr_sClass_CellUpdate* object)
{
  ...
  if (object->OldLastIndex > num) {
    ...
    if (object->FloatAttrOffs[0] >= 0) {
      memset(&object->F1[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
      memset(&object->F1Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    }
    if (object->FloatAttrOffs[1] >= 0) {
      memset(&object->F2[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
      memset(&object->F2Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    }
    if (object->FloatAttrOffs[2] >= 0) {
      memset(&object->F3[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
      memset(&object->F3Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    }
    if (object->FloatAttrOffs[3] >= 0) {
      memset(&object->F4[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
      memset(&object->F4Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    }
    if (object->FloatAttrOffs[4] >= 0) {
      memset(&object->F5[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
      memset(&object->F5Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tFloat32));
    }
    if (object->BooleanAttrOffs[0] >= 0) {
      memset(&object->B1[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
      memset(&object->B1Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    }
    if (object->BooleanAttrOffs[1] >= 0) {
      memset(&object->B2[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
      memset(&object->B2Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    }
    if (object->BooleanAttrOffs[2] >= 0) {
      memset(&object->B3[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
      memset(&object->B3Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    }
    if (object->BooleanAttrOffs[3] >= 0) {
      memset(&object->B4[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
      memset(&object->B4Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    }
    if (object->BooleanAttrOffs[4] >= 0) {
      memset(&object->B5[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
      memset(&object->B5Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tBoolean));
    }
    if (object->IntAttrOffs[0] >= 0) {
      memset(&object->I1[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
      memset(&object->I1Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    }
    if (object->IntAttrOffs[1] >= 0) {
      memset(&object->I2[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
      memset(&object->I2Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    }
    if (object->IntAttrOffs[2] >= 0) {
      memset(&object->I3[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
      memset(&object->I3Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    }
    if (object->IntAttrOffs[3] >= 0) {
      memset(&object->I4[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
      memset(&object->I4Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    }
    if (object->IntAttrOffs[4] >= 0) {
      memset(&object->I5[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
      memset(&object->I5Old[num], 0,
          (object->OldLastIndex - num) * sizeof(pwr_tInt32));
    }
  }
