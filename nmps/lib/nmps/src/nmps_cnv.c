    ...
    int* conv_table_count)
{
  ...
  sts = cnv_convtable_compress(conv_table, conv_table_count);
  return CNV__SUCCESS;
}
    ...
    char* from_data, char* to_data)
{
  ...
  for (i = 0; i < conv_table_count; i++) {
    ...
    case cnv_eConvType_AsciiToInt32:
      ...
      sscanf(tmpstr, "%d", (pwr_tInt32*)(to_data + conv_item->to));
      break;
    case cnv_eConvType_AsciiToFloat32:
      strncpy(tmpstr, from_data + conv_item->from, conv_item->size);
      tmpstr[conv_item->size] = 0;
      sscanf(tmpstr, "%f", (pwr_tFloat32*)(to_data + conv_item->to));
      break;
    }
    conv_item++;
  }
    ...
    pwr_tObjid class, pwr_sClass_ConvDef** convdef)
{
  ...
  *convdef = calloc(1, sizeof(**convdef));
  ...
  sts = gdh_ObjidToName(class, name, sizeof(name), cdh_mName_volumeStrict);
  if (EVEN(sts))
    return sts;
