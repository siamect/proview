/** 
 * Proview   $Id: wb_gcg.c,v 1.22 2005-09-01 14:57:58 claes Exp $
...
)
{
	...
	sts = (gcg_comp_m[0]) (plc, codetype, errorcount, warningcount, spawn); 
	if ( EVEN(sts)) return sts;
/***********
	if ( codetype == 1)
	{
...
)
{
	...
	sts = (gcg_comp_m[1]) (wind, codetype, errorcount, 
		warningcount, spawn); 
	if ( EVEN(sts)) return sts;
/************
	if ( codetype == 1)
...
)
{
	...
	if ( output_count > 0 )
	{
	  ...
	  sts = gcg_pgmname_to_parname( gcgctx.ldhses, class, 
		output_par, conn_par );
	  if( EVEN(sts)) return sts;
...
{
	/* Create the context */
	*gcgctx = (gcg_ctx)XtCalloc( 1, sizeof( **gcgctx ) );

	(*gcgctx)->wind = wind;

#if 0

	/* This should not be needed when you use XtCalloc. ML 950727 */

	/* Create the arrays for io read, io write and referencenodes */
	(*gcgctx)->ioread = (gcg_t_reflist *)XtCalloc( 1, sizeof( gcg_t_reflist));
	(*gcgctx)->ioreadcount = 0;
	(*gcgctx)->iowrite = (gcg_t_reflist *)XtCalloc( 1, sizeof(gcg_t_reflist);
	(*gcgctx)->ioreadcount = 0;
	(*gcgctx)->ref = (gcg_t_reflist *)XtCalloc( 1, sizeof(gcg_t_reflist));
	(*gcgctx)->refcount = 0;

	(*gcgctx)->compobjcount = 0;
	(*gcgctx)->errorcount = 0;
	(*gcgctx)->order_comp = 0;
	(*gcgctx)->step_comp = 0;
	(*gcgctx)->reset_checked = 0;
#endif
...
)
{
	...
	if (gcgctx->ioreadcount > 0)
	  XtFree((char *) gcgctx->ioread);
	if (gcgctx->iowritecount > 0)
	  XtFree((char *) gcgctx->iowrite);
	if (gcgctx->refcount > 0)
	  XtFree((char *) gcgctx->ref);
	XtFree((char *) gcgctx);
}
...
* pwr_tClassId	class		I	object class.
* unsigned long	reftype		I	iocopied object or not.
*
* Description:
...
*	The list should be freed by a Xtfree call after use.
*
**************************************************************************/
...
)
{
	...
	timebase = (gcg_t_timebase *) XtCalloc( thread_count, 
	...
	for ( i = 0; i < thread_count; i++ )
	{
	  ...
	  (timebase+i)->plclist = (pwr_tObjid *)XtCalloc( plc_count, 
		sizeof( pwr_tObjid));
	  for ( k = 0; k < plc_count; k++ )
	  {
...
/*	  timebase->scantime = single_scantime; */
	  ...
	  for ( i = 1; i < thread_count; i++ )
	  {
	    ...
	    XtFree( (char *)(timebase+i)->plclist);
	  }
	...
	if ( codetype)
	{
	  ...
	  {
	    timebase_ms = (timebase+i)->scantime * 1000 + 0.5;
...
int	gcg_comp_m0( plc, codetype, errorcount, warningcount, spawn)
vldh_t_plc	plc;
unsigned long	codetype;
unsigned long	*errorcount;
unsigned long	*warningcount;
...
int	gcg_comp_m1( wind, codetype, errorcount, warningcount, spawn)
vldh_t_wind	wind;
unsigned long	codetype;
unsigned long	*errorcount;
unsigned long	*warningcount;
...
vldh_t_node	node;
{
	...
	if ( point_count > 1 )
	{
	  for ( k = 1; k < point_count; k++)
	  {
	    ...
	    if ( 
		(class == vldh_uclass( ldhses, "MpsIn")) || 
		(class == vldh_uclass( ldhses, "MpsOut")) || 
		(class == vldh_uclass( ldhses, "MpsTube")) || 
		(class == vldh_uclass( ldhses, "MpsStack")) || 
		(class == vldh_uclass( ldhses, "MpsWare")) || 
		(class == vldh_uclass( ldhses, "MpsQueue")) ) 
	    {
	      cellcount++;
	      /* Print in the input list */
	    ...
	    else if (class != vldh_uclass( ldhses, "MpsTrp") )
	    {
	      /* Bad class */
	      gcg_error_msg( gcgctx, GSX__NOSTEP, node);
	    }
...
/*	if ( !(class == vldh_class( ldhses, VLDH_CLASS_TRP)))
	{
	  gcg_error_msg( gcgctx, GSX__BADWIND, node);  
	  return GSX__NEXTNODE;
	}
...
vldh_t_node	node;
{
	...
	if ( !((class == vldh_eclass( ldhses, "NMpsCell")) ||
	       (class == vldh_eclass( ldhses, "CLoop")) ||
	       (class == vldh_eclass( ldhses, "NMpsMirrorCell")) ||
	       (class == vldh_eclass( ldhses, "NMpsStoreCell"))))
	{
	  gcg_error_msg( gcgctx, GSX__BADWIND, node);  
	  return GSX__NEXTNODE;
	}
...
vldh_t_node	node;
{
	...
	sts = ldh_GetObjectClass ( ldhses, *parvalue, &class);
	if ( EVEN(sts))
	{
...
static    pwr_tClassId		plc_pgm = 0;
	...
	while(1)
	{
	  ...
	  sts = ldh_GetObjectClass(ldhses, od, &class);
          if (EVEN (sts)) return sts;
