    ...
    int* outsize)
{
  ...
  *newstr = 0;
  ...
  first_line = 1;
  while (1) {
    end_of_text = dataa_get_next_line(str, &p, line, sizeof(line), first_line);
    if (end_of_text == -1) {
    ...
    if (strstr(line, "classdef")) {
      ...
      if (EVEN(sts)) {
        strncpy(error_line, line, *error_line_size);
        *error_line_num = dataactx->line_count, dataa_delete_ctx(dataactx);
        return sts;
      }
      ...
      while (*t != 0) {
        ...
        if (hit) {
          if (dataactx->comment && !(item_ptr->func == &dataa_comment_end)) {
