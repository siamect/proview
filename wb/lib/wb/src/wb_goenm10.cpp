...
int goen_create_nodetype_m10(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid,
    ...
    flow_tNodeClass* node_class, vldh_t_node node)
{
  ...
  if (annot_count <= 1) {
    ...
    f_width = MAX(
        f_strlength * 4 + MAX(classname_width, annot_width[0]) + annot_width[1],
        f_defwidth + f_strlength * 2);
  ...
  if (subwindowmark != 0) {
    flow_AddLine(nc_pid, 0, f_header - f_yoffs, f_width, f_header - f_yoffs,
        flow_eDrawType_LineGray, 4);
  }

  if (annot_count >= 2) {
    flow_AddLine(nc_pid, f_width - f_width_left, -f_yoffs,
        f_width - f_width_left, f_height - f_yoffs, flow_eDrawType_Line, 1);
  }

  /* Draw the class name */
  flow_AddText(nc_pid, graphbody->graphname, f_namepos,
      f_header * .75 - f_yoffs, flow_eDrawType_TextHelvetica, GOEN_F_TEXTSIZE);
    ...
    if ((*inmask_pointer & pointmask) != 0) {
      if ((*invertmask_pointer & pointmask) == 0) {
        ...
        flow_AddLine(nc_pid, -f_pinlength,
            f_header / 2 + f_repeat * ipoints - f_yoffs, -f_circle,
            f_header / 2 + f_repeat * ipoints - f_yoffs, flow_eDrawType_Line,
            2);
        flow_AddArc(nc_pid, -f_circle,
            f_header / 2 + f_repeat * ipoints - f_circle / 2 - f_yoffs, 0,
            f_header / 2 + f_repeat * ipoints + f_circle / 2 - f_yoffs, 0, 360,
            flow_eDrawType_Line, 2);
      }
      flow_AddConPoint(nc_pid, -f_pinlength,
          f_header / 2 + f_repeat * ipoints - f_yoffs, conpoint_nr++,
  ...
  flow_AddAnnot(nc_pid, (f_width - f_width_left - annot_width[0]) / 2,
      f_height - f_header * 0.25 - f_yoffs, 0, flow_eDrawType_TextHelvetica,
      GOEN_F_TEXTSIZE, flow_eAnnotType_OneLine, flow_mDisplayLevel_1);

  if (annot_count >= 2) {
    flow_AddAnnot(nc_pid, f_width - f_width_left + f_strlength,
        f_height - f_header * 0.75 - f_yoffs, 1, flow_eDrawType_TextHelvetica,
        GOEN_F_TEXTSIZE, flow_eAnnotType_OneLine, flow_mDisplayLevel_1);
  }
    ...
    goen_conpoint_type* info_pointer, vldh_t_node node)
{
  ...
  if ((int)point < inputpoints) {
    ...
    info_pointer->y
        = f_height / 2.0 - f_header / 2 - (point - inputpoints) * f_repeat;
