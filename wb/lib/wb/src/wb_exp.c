/** 
 * Proview   $Id: wb_exp.c,v 1.5 2005-09-01 14:57:58 claes Exp $
...
)
{
	...
	if ( sts == VLDH__OBJNOTFOUND )
	{
	  ...
	  else
	  {
	    ...
	    sts = vldh_wind_load( plc, node, window, 0, 0, &wind, access);
	    if ( EVEN(sts)) return sts;
	  }
	  sts = vldh_wind_load_all( wind);
	  if ( EVEN(sts)) return sts;
...
)
{
	...
	while ( ODD(sts) )
	{
	       ...
	       (class == pwr_cClass_porder) )
	  {
	    ...
	    /* Print class and name */
	    sts = ldh_ObjidToName(
			genctx->ldhses, cdh_ClassIdToObjid( class), 
			ldh_eName_Object, class_name, sizeof( class_name), &size);
	    if( EVEN(sts)) return sts;
