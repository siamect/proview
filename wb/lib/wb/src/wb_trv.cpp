...
)
{
  ...
  while ( ODD(sts) ) {
    ...
    sts = ldh_GetObjectBuffer( ldhses,
			       objdid, "DevBody", "PlcWindow", (pwr_eClass *) &cid,	
			       &windbuffer, &size);
    if ( ODD(sts)) {
      free( windbuffer);
      window_found = 1;	
...
)
{
  ...
  *objectlist = NULL;
  ...
  while ( ODD(sts) ) {
    /* Check if node */
    sts = ldh_GetObjectBuffer( ldhses,
			       objdid, "DevBody", "PlcNode", (pwr_eClass *) &cid,	
			       &nodebuffer, &size);
    if ( ODD(sts))  {
      free( nodebuffer);
  ...
  *object_count = 0;
  ...
  while ( ODD(sts) || plc_found ) {
    ...
    if ( ODD( ldh_GetObjectBuffer( ldhses,
				   objdid, "DevBody", "PlcProgram", (pwr_eClass *) &cid,	
				   &plcbuffer, &size) )) {
      plc_found = 1;
      free( plcbuffer);
    }
...
)
{
  ...
  sts = ldh_GetChild( ldhses, objdid, &objdid);
  while ( ODD(sts) ) {
    sts = trv_get_child_object_class( ldhses, cid, objdid, backcall, 
				      arg1, arg2, arg3, arg4, arg5);
    if ( EVEN(sts)) return sts;
    sts = ldh_GetNextSibling( ldhses, objdid, &objdid);
  }
  ...
  if ( obj_class != pwr_eClass_MountObject) {
    ...
    while ( ODD(sts)) {
      sts = trv_get_child_object_hi_cl_na( ldhses, hierobjdid,
					   cid, name, childobjdid, backcall, arg1, arg2,
					   arg3, arg4, arg5);
      if ( EVEN(sts)) return sts;
...
)
{
  ...
  if ( cdh_ObjidIsNotNull( hierobjdid)) {
    ...
    sts = trv_get_child_object_hi_cl_na( ldhses, hierobjdid,
					 cid, name, hierobjdid, backcall, arg1, arg2, arg3, 
					 arg4, arg5);
    if ( EVEN(sts)) return sts;
  }
  else {
	...
	sts = trv_get_child_object_hi_cl_na( ldhses, hierobjdid,
					     cid, name, objdid, backcall, arg1, arg2, arg3, 
					     arg4, arg5);
	if ( EVEN(sts)) return sts;
      }
      sts = ldh_GetNextSibling( ldhses, objdid, &objdid);
...
)
{
  ...
  while ( ODD(sts) ) {
    sts = trv_get_child_object_hier_class( ldhses, hierobjdid, hierflag,
					   cid, childobjdid, backcall, arg1, arg2, arg3, arg4, arg5);
    if ( EVEN(sts)) return sts;
    sts = ldh_GetNextSibling( ldhses, childobjdid, &childobjdid);
  }
...
)
{
  ...
  if ( cdh_ObjidIsNotNull( hierobjdid)) {
    ...
    sts = trv_get_child_object_hi_cl_na( ldhses, hierobjdid,
					 cid, name, hierobjdid, backcall, arg1, arg2, arg3, arg4, arg5);
    if ( EVEN(sts)) return sts;
  }
  else {
    sts = ldh_GetRootList( ldhses, &objdid);
	...
	/* Check if the children */
	sts = trv_get_child_object_hi_cl_na( ldhses, hierobjdid,
					     cid, name, objdid, backcall, arg1, arg2, arg3, arg4, arg5);
	if ( EVEN(sts)) return sts;
      }
      sts = ldh_GetNextSibling( ldhses, objdid, &objdid);
    }
...
)
{
  ...
  if ( cdh_ObjidIsNull( hierobjdid)) {
    ...
    while ( ODD(sts)) {
      ...
      /* Check that the class of the node object is correct */
      if ( !( cid == pwr_eClass_ClassHier ||
	      cid == pwr_eClass_TypeHier)) {
	/* Check if the children is a plc */
