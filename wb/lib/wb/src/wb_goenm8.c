/* 
 * Proview   $Id: wb_goenm8.c,v 1.3 2005-09-06 10:43:31 claes Exp $
    ...
    vldh_t_node		node)
{
	...
	if ( (*mask == 1 || *mask == 2) && *(mask + 1) <= 1 )
	  sts = goen_create_nodetype_m7( graphbody, class, ldhses, ctx, 
			mask, subwindowmark, node_width, node_class,
			node);
	else if ( *(mask + 1) <= 1)
	  sts = goen_create_nodetype_m15( graphbody, class, ldhses, ctx, 
			mask, subwindowmark, node_width, node_class,
			node);
	else
	{
	  ...
	  if ( annot_count > 1)
	    sts = goen_create_nodetype_m2( graphbody, class, ldhses, ctx, 
			mask, subwindowmark, node_width, node_class,
			node);
	  else
	    sts = goen_create_nodetype_m0( graphbody, class, ldhses, ctx, 
			mask, subwindowmark, node_width, node_class,
			node);
	}
	return sts;
...
*
**************************************************************************/
int goen_get_point_info_m8( grectx, graphbody, point, mask, node_width, 
		info_pointer, node)
    gre_ctx		 grectx;
    pwr_sGraphPlcNode	*graphbody;
    unsigned long	 point;
...
*
**************************************************************************/
int	goen_get_parameter_m8( graphbody, class, ldhses,
...
unsigned long		*par_index;
{
	...
	if ( (*mask == 1 || *mask == 2) && *(mask + 1) <= 1 )
	  sts = goen_get_parameter_m7( graphbody, class, ldhses,
			con_point, mask, par_type, par_inverted, par_index);
	else if ( *(mask + 1) <= 1)
	  sts = goen_get_parameter_m15( graphbody, class, ldhses,
			con_point, mask, par_type, par_inverted, par_index);
	else
	  sts = goen_get_parameter_m0( graphbody, class, ldhses,
			con_point, mask, par_type, par_inverted, par_index);
	return sts;
}
...
/*************************************************************************
*
* Name:		goen_get_location_point_m8()
*
* Type		
*
* Type		Parameter	IOGF	Description
...
*
**************************************************************************/
int goen_get_location_point_m8( grectx, graphbody, mask, node_width, 
		info_pointer, node)
    gre_ctx		 grectx;
    pwr_sGraphPlcNode	*graphbody;
    unsigned long 	 *mask;
