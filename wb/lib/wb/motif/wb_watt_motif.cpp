...
//
void WAttMotif::activate_change_value(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  watt->change_value(1);
}

void WAttMotif::activate_close_changeval(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  watt->change_value_close();
}

void WAttMotif::activate_exit(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  if (watt->close_cb)
    (watt->close_cb)(watt);
...
void WAttMotif::create_msg_label(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  watt->msg_label = w;
}
void WAttMotif::create_cmd_prompt(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  watt->cmd_prompt = w;
}
void WAttMotif::create_cmd_input(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  Arg args[2];
...
void WAttMotif::create_cmd_scrolledinput(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  watt->cmd_scrolledinput = w;
}
void WAttMotif::create_cmd_scrolled_ok(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  watt->cmd_scrolled_ok = w;
}
void WAttMotif::create_cmd_scrolled_ca(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  watt->cmd_scrolled_ca = w;
}
void WAttMotif::create_wattnav_form(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  watt->wattnav_form = w;
}
...
static void watt_disable_set_focus(WAttMotif* watt, int time)
{
  watt->set_focus_disabled++;
  watt->focus_timerid
      = XtAppAddTimeOut(XtWidgetToApplicationContext(watt->toplevel), time,
...
void WAttMotif::change_value_close()
{
  ...
  if (input_open) {
    if (input_multiline) {
      sts = ((WAttNav*)wattnav)->set_attr_value(input_node, input_name, text);
...
void WAttMotif::activate_cmd_input(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  char* text;
  int sts;
...
void WAttMotif::activate_cmd_scrolled_ok(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  char* text;
  unsigned char* s;
...
void WAttMotif::activate_cmd_scrolled_ca(
    Widget w, WAttMotif* watt, XmAnyCallbackStruct* data)
{
  if (watt->input_open) {
    XtUnmanageChild(watt->cmd_scrolledinput);
...
WAttMotif::WAttMotif(Widget wa_parent_wid, void* wa_parent_ctx,
      ...
      parent_wid(wa_parent_wid), set_focus_disabled(0), value_current_recall(0)
{
  ...
  static MrmRegisterArg reglist[] = { { (char*)"watt_ctx", 0 },
    ...
    { (char*)"watt_create_cmd_scrolledinput",
        (caddr_t)create_cmd_scrolledinput },
    ...
    { (char*)"watt_activate_cmd_scrolled_ok",
        (caddr_t)activate_cmd_scrolled_ok },
    { (char*)"watt_activate_cmd_scrolled_ca",
        (caddr_t)activate_cmd_scrolled_ca } };
  ...
  sts = ldh_AttrRefToName(ldhses, &aref,
      cdh_mName_path | cdh_mName_object | cdh_mName_attribute, &title_p, &size);
  ...
  sts = MrmFetchWidgetOverride(s_DRMh, (char*)"watt_window", parent_wid, name,
  ...
  wattnav = new WAttNavMotif((void*)this, wattnav_form, "Plant", ldhses, aref,
