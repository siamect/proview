...
//
WNavMotif::WNavMotif(void* xn_parent_ctx, Widget xn_parent_wid,
    const char* xn_name, const char* xn_layout, Widget* w,
    ...
    : WNav(xn_parent_ctx, xn_name, xn_layout, xn_ldhses, root_menu, xn_type,
          status),
      parent_wid(xn_parent_wid), deadlock_timerid(0), trace_timerid(0),
      avoid_deadlock(0)
{
  Arg args[5];
  int i;
...
void WNavMotif::trace_scan(WNavMotif* wnav)
{
  ...
  if (wnav->trace_started) {
    ...
    wnav->trace_timerid
        = XtAppAddTimeOut(XtWidgetToApplicationContext(wnav->brow_widget), time,
...
WGe* WNavMotif::wge_new(
    char* name, char* filename, char* object_name, int modal)
{
  WGe* wge = new WGeMotif(parent_wid, this, name, filename, 0, 0, 0, 0, 0, 0, 0,
      object_name, modal);
  return wge;
}
...
static void wnav_set_avoid_deadlock(WNavMotif* wnav, int time)
{
  ...
  wnav->deadlock_timerid
      = XtAppAddTimeOut(XtWidgetToApplicationContext(wnav->brow_widget), time,
          (XtTimerCallbackProc)wnav_reset_avoid_deadlock, wnav);
}

Boolean WNavMotif::sel_convert_cb(Widget w, Atom* selection, Atom* target,
    ...
    int* format_return)
{
  ...
  if (*target == XA_STRING
      ...
      || *target == wnav->graph_atom || *target == wnav->objid_atom
...
int WNavMotif::open_foe(const char* name, pwr_tOid plcpgm, void** foectx,
    int map_window, ldh_eAccess access, pwr_tOid oid)
{
  ...
  if (foe)
    ...
    foe = new WFoeMotif((void*)parent_ctx, parent_wid, name, plcpgm, wbctx,
        ldhses, map_window, access, options, &sts);
  if (EVEN(sts))
    return sts;
...
CoLogin* WNavMotif::login_new(const char* name, const char* groupname,
...
static void wnav_message_dialog_read(
    Widget w, XtPointer client_data, XtPointer data)
{
...
void WNavMotif::message_dialog(char* title, char* text)
{
  ...
  flow_AddCloseVMProtocolCb(
      XtParent(dialog), (XtCallbackProc)wnav_message_dialog_ok, this);
    ...
    char* title, char* text, int display_cancel, int* cancel)
{
  ...
  if (!display_cancel)
    flow_AddCloseVMProtocolCb(
        XtParent(dialog), (XtCallbackProc)wnav_message_dialog_cancel, this);
  else
    flow_AddCloseVMProtocolCb(
        XtParent(dialog), (XtCallbackProc)wnav_message_dialog_help, this);
