...
let OpSys_X86_64_MACOS=10
let OpSys_X86_64_FREEBSD=11
let OpSys_X86_64_OPENBSD=12
let OpSys_X86_CYGWIN=13
let OpSys__High=14

vOpSys="custombuild,vax_vms,axp_vms,ppc_lynx,x86_lynx,ppc_linux,x86_linux,x86_64_linux,arm_linux,x86_64_macos,x86_64_freebsd,x86_64_openbsd,x86_cygwin"
...
if [ $machine = "x86_64" ]; then
  CurrentOpSys=$OpSys_X86_64_CYGWIN
else
  CurrentOpSys=$OpSys_X86_CYGWIN
fi
...
elif [ $OpSys -eq $OpSys_X86_64_OPENBSD ]; then
  pwrp_gc="$pwrp_tmp"

# Suppress all warnings, -x
  if [ $CurrentOpSys -eq $OpSys ]; then
      cc_cmd="gcc -c -x c -w $cc_debug -D_REENTRANT -DOS_OPENBSD -DOS_POSIX -I$pwr_inc -I$pwrp_inc -I$pwrp_tmp $PWR_EXT_INC"
...
elif [ $OpSys -eq $OpSys_X86_CYGWIN ]; then
  pwrp_gc="$pwrp_tmp"
