    ...
    io_tCtx ctx, io_sAgent* ap, io_sRack* rp, io_sCard* cp)
{
  ...
  if (paramc && ((diff & setup_mask) == 0)) {
    ...
    /* Update status word */
    if ((diff & setup_req_mask) != 0)
      local->Istat[0] ^= setup_req_mask;
  ...
  if (local->objP->UseInc3p)
    local->Istat[0] |= inc3p_mask;
  if (local->objP->UsePos3p)
    local->Istat[0] |= pos3p_mask;
  if (local->objP->UseDynBias)
    local->Istat[0] |= dyn_bias_mask;
  if (local->objP->UseAo)
    local->Istat[0] |= stall_freeze_mask;
  else
    local->Istat[0] |= stall_cont_mask;
