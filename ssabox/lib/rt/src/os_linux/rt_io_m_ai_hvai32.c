    ...
    if (cop->ConversionOn) {
      if (local->ScanCount[i] <= 1) {
        ...
        case pwr_cClass_ChanAit:
          io_ConvertAit((pwr_sClass_ChanAit*)cop, (pwr_tInt16)data, &actvalue);
          break;
        }

        /* Check max/min
        if (actvalue > cop->ActValRangeHigh && cop->ActValRangeHigh >
        cop->ActValRangeLow)
                actvalue = cop->ActValRangeHigh;
        if (actvalue < cop->ActValRangeLow && cop->ActValRangeHigh >
        cop->ActValRangeLow)
...
pwr_dExport pwr_BindIoMethods(Ai_HVAI32) = { pwr_BindIoMethod(IoCardInit),
