...
class GrowAxis extends GrowRect {
  ...
  tdraw(t, highlight, hot, node, colornode) {
    ...
    if (45 >= rotation || rotation > 315) {
      ...
      if (draw_text) {
        let d2 = this.ctx.gdraw.getTextExtent("0", Math.max(0, text_idx),
            Font.Helvetica, this.text_drawtype);
      ...
      for (i = 0; i < this.lines; i++) {
        ...
        if (draw_text && i % this.valuequotient === 0) {
          text =
              this.format_text(this.format, this.max_value - i * this.increment);
          let d3 = this.ctx.gdraw.getTextExtent(text, Math.max(0, text_idx),
              Font.Helvetica, this.text_drawtype);
          z_width = d3.width;
          z_height = d3.height;
          z_descent = z_height / 4;
      ...
      if (draw_text) {
        for (i = 0; i < this.lines; i++) {
          if (i % this.valuequotient === 0) {
            text = this.format_text(this.format, this.max_value - i *
                this.increment);
            let d4 = this.ctx.gdraw.getTextExtent(text, Math.max(0, text_idx),
                Font.Helvetica, this.text_drawtype);
            z_width = d4.width;
            z_height = d4.height;
            z_descent = z_height / 4;
      ...
      // Calculate max value text height
      if (draw_text) {
        let d5 = this.ctx.gdraw.getTextExtent("0", Math.max(0, text_idx),
            Font.Helvetica, this.text_drawtype);
      ...
      for (i = 0; i < this.lines; i++) {
        ...
        if (draw_text && i % this.valuequotient === 0) {
          text =
              this.format_text(this.format, this.max_value - i * this.increment);
          let d6 = this.ctx.gdraw.getTextExtent(text, Math.max(0, text_idx),
              Font.Helvetica, this.text_drawtype);
          z_width = d6.width;
          z_height = d6.height;
          z_descent = z_height / 4;
