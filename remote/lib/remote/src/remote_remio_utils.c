...
pwr_tFloat32 ConvAItoAct(pwr_tInt16 raw, pwr_sClass_RemChan_Ai* body)
{
  ...
  if (body->SensorPolyType == 1)
    ...
    if (dval >= 0)
      actval = body->SensorPolyCoef2 * sqrt(dval);
    else
      actval = 0;
...
pwr_tInt16 ConvAOtoRaw(pwr_tFloat32 act, pwr_sClass_RemChan_Ao* body)
{
  ...
  if (act > body->OutMaxLimit)
    actval = body->OutMaxLimit;
  else if (act < body->OutMinLimit)
    actval = body->OutMinLimit;
  else
    actval = act;
