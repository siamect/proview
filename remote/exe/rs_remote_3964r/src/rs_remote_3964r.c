...
static unsigned int send_it(char* buf, int buffer_size)
{
  ...
  {
    errh_Error(
        "Remtrans 3964R, sändning, inget svar från mottagaren, försöker igen");
  ...
  if (sts < 1 || buff != DLE) /*if somthing went wrong*/
  {
    write(ser_fd, snak, 1);
    if (sts < 1)
      errh_Error("Remtrans 3964R sändning misslyckades, inget initierande "
                 "DLE-tecken från mottagaren");
    else if (buff != DLE)
      errh_Error("Remtrans 3964R sändning misslyckades, inget avslutande "
                 "DLE-tecken från mottagaren");
    sts = false;
  }
  ...
  if (sts > 0) {
    if (received_char == STX) {
      ...
      if (received_char != NAK)
        write(ser_fd, snak, 1);
      errh_Error("3964R felaktigt meddelande i mottagning, annat starttecken "
                 "än STX (0x%x)",
          received_char);
      sts = 0; // felstatus
    }
...
static unsigned int ReceiveHandler(int fd)
{
  ...
  while (cont) {
    ...
    if (receive_buffer[data_size] == ETX) {
      ...
      data_size++;
      cont = false;
    } else if (receive_buffer[data_size] != DLE)
      data_size++;
  }
  ...
  if (BCC == receive_buffer[data_size - 1]) {
    ...
    errh_Error("3964R mottagning, felaktig checksumma, %d, %02x %02x %02x %02x "
  ...
  /*************************************************************************/
  {
    ...
    if (strstr(name, "PSS")) {
      ...
      sts = RemIO_Receive_3964R(
          &rn, (unsigned char*)&io_buf->data, io_size - NET_HEADER_SIZE_IO);
      if (debug)
        printf("  Receiving I/O area\n");
...
int main(int argc, char* argv[]) /*argv[2]=remnode name*/
{
  ...
  while (remtrans) {
    ...
    if (i >= (int)(sizeof(rn_3964R->RemTransObjects)
                 / sizeof(rn_3964R->RemTransObjects[0])))
      break;
    remtrans = (remtrans_item*)remtrans->next;
  }
  ...
  ser_fd = RemUtils_InitSerialDev(rn_3964R->DevName, rn_3964R->Speed,
      rn_3964R->DataBits, rn_3964R->StopBits, rn_3964R->Parity);
  ...
  while (1) {
    ...
    if (use_remote_io) {
      if ((rn_3964R->LinkUp && time_since_poll >= rn_3964R->ScanTime * 2.0)
          || (!rn_3964R->LinkUp
                 && time_since_poll >= rn_3964R->ScanTime * 10.0)) {
        if (debug)
          printf("RemIO Cyclic\n");
