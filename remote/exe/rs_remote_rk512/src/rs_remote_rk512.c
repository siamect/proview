        ...
        if (sts > 0) {
          ...
          if (sts > 0 && (received_char == DLE)) {
            ...
            if (sts > 0 && (received_char == STX)) {
              ...
              if (sts > 0) {
                BCC = '\0';
                for (i = 0; i < RESP_MESSAGE_SIZE && sts > 0; i++) {
                  load_timeval(&tv, rn_RK512->CharTimeout);
                  FD_ZERO(&read_fd);
                  FD_SET(ser_fd, &read_fd);
                  select(ser_fd + 1, &read_fd, NULL, NULL, &tv);
...
static unsigned int send_it(char* buf, int buffer_size, int double_dle)
{
  ...
  {
    errh_Error(
        "Remtrans RK512, sändning, inget svar från mottagaren, försöker igen");
  ...
  if (sts < 1 || buff != DLE) /*if somthing went wrong*/
  {
    write(ser_fd, snak, 1);
    if (sts < 1)
      errh_Error("Remtrans RK512 sändning misslyckades, inget initierande "
                 "DLE-tecken från mottagaren");
    else if (buff != DLE)
      errh_Error("Remtrans RK512 sändning misslyckades, inget avslutande "
                 "DLE-tecken från mottagaren");
    sts = false;
  }
...
static unsigned int ReceiveHandler(int fd)
{
  ...
  if (!telegram_counter && (receive_buffer[0] == 0X00)) {
    ...
    if (!CODE) {
      ...
      if (datasize <= remtrans->objp->MaxLength) {
        if (datasize > 128)
          telegram_counter = (datasize - 1) / 128;
      } else {
    ...
    if (telegram_counter && !CODE) {
      ...
      for (i = HEADER_SIZE; i < (HEADER_SIZE + MAX_SIZE_DATA_BLOCK); i++) {
  ...
  if (telegram_counter && (receive_buffer[0] == 0XFF) && !CODE) {
    ...
    if (!CODE) {
      ...
      for (i = FOLLOW_ON_HEADER_SIZE; i < (FOLLOW_ON_HEADER_SIZE + delta_pos);
           i++) {
        *TelegramBuffer_ptr++ = receive_buffer[i];
      }
      telegram_counter -= 1;
