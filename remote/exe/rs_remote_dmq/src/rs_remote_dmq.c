...
unsigned int InitCycleTimer()
{
  ...
  dmq_sts
      = pams_set_timer(&timer_id, &format, &tick_count, (int32*)&delta_time);
...
unsigned int WaitEvent(void)
{
  ...
  dmq_sts = pams_get_msgw(receive_msg, &receive_prio, &receive_src,
      ...
      &receive_timeout, 0, &receive_psb, 0, 0, 0, 0, 0);
...

{
  ...
  dmq_sts = pams_put_msg(buf, &put_prio, &dest, &class, &type, &delivery,
      &msg_size, &timeout, &put_psb, &put_uma, 0, 0, 0, 0);
...
main(int argc, char* argv[])
{
  ...
  while (ODD(sts)) {
    ...
    /* Check if transport type is DMQ and if remnode's process number matches
     * ours */
