...
FUNCTION extern INT16 CALL_CONV profi_set_default(IN USIGN8 Board,
    ...
    This function is used to initialize the WIN32 PROFIBUS-API. The function
    function
    ...
    IN:  ReadTimeout    -> receive timeout in ms (WAIT_FOREVER for infinite
    wait)
    ...
    - E_IF_INVALID_CNTRL_TYPE_VERSION  -> invalid controller type or software
    version
...
FUNCTION extern INT16 CALL_CONV init_profibus(IN USIGN32 DprAdress,
    ...
    This function is used initialize the PROFIBUS-API with default values and
    start
    the PROFIBUS controller. The function function has to be called before any
    other
...
FUNCTION extern INT16 CALL_CONV profi_snd_req_res(
    ...
    - E_IF_NO_CNTRL_RES              -> controller does not respond
    (CMI_TIMEOUT)
    - E_IF_INVALID_DATA_SIZE         -> not enough CMI memory available for REQ
    or RES
...
profi_rcv_con_ind(OUT T_PROFI_SERVICE_DESCR* pSdb, OUT VOID* pData,
    ...
    This function is used to receive a Service-Indication or a
    ...
    - CON_IND_RECEIVED          -> a confirmation or indication has been
    received
    - NO_CON_IND_RECEIVED       -> no confirmation or indication has been
    received
...
FUNCTION extern INT16 CALL_CONV profi_set_data(IN USIGN8 DataId,
    ...
    IN:   DataId         -> data image identifier
                             - ID_DP_SLAVE_IO_IMAGE     (DPM: image for slave io
    data)
                             - ID_DP_STATUS_IMAGE       (DPM: image for status
    data)
...
FUNCTION extern INT16 CALL_CONV profi_get_data(IN USIGN8 DataId,
    ...
    IN:    DataId        -> data image identifier
                             - ID_DP_SLAVE_IO_IMAGE        (DPM: image for slave
    io data)
                             - ID_DP_STATUS_IMAGE          (DPM: image for
    status data)
...
FUNCTION extern INT16 CALL_CONV profi_set_dps_input_data(IN USIGN8* pData,
    ...
    OUT:  pState     -> pointer to a status variable for the recent input data
    status

    possible return values:
    - E_OK                          -> OK
    - E_IF_INVALID_DATA_SIZE        -> data_size does not match the expected
    input data size
...
profi_get_versions(OUT CSTRING* pPapiVersion, OUT CSTRING* pFirmwareVersion)
    ...
    NOTE: There must be at least 100 Bytes (VERSION_STRING_LENGTH) free space
    for
          each INOUT buffer.
...
#ifdef WIN32
/**********************************************************************************/
/* extern function declarations using for enhanced WinNT/Win2K PROFIBUS
 * Interface */
/**********************************************************************************/
...
FUNCTION extern HANDLE CALL_CONV profi_open_basic_management(
    ...
    It opens the the BASIC MANAGEMENT DEVICE and updates the PROFIBUS firmware
    onto
    PROFIBUS controller if necessary.

    IN:  Board           -> number of the PROFIBUS board (0..9)
    IN:  Channel         -> channel number
    IN:  DesiredAccess   -> GENERIC_READ  specifies the read access to the
    device
                            GENERIC_WRITE specifies the write access to the
    device
...
FUNCTION extern HANDLE CALL_CONV profi_open(IN HANDLE hBasicMgmtDevice,
    ...
    IN:  DeviceType        -> type of the device to open
                              ...
                              DEVICE_DP_MSAC        -> DP master slave acyclic
    device
    ...
    IN:  Index             -> index of the device to open
                              1..128           DP service-, DP slave data- and
    DP/V1 service devices
...
FUNCTION extern BOOL CALL_CONV profi_close(IN HANDLE hDevice)
    /*----------------------------------------------------------------------------
    FUNCTIONAL_DESCRIPTION

    This function is used to close a device opened with
    'profi_open_basic_management'
...
profi_read_service(IN HANDLE hDevice, OUT T_PROFI_SERVICE_DESCR* pSdb,
    ...
    - E_IF_INVALID_DATA_SIZE           -> size of data block provided not
    sufficient
    - E_IF_OS_ERROR                    -> NT system error
    - E_IF_RESOURCE_UNAVAILABLE        -> no resource available
    -----------------------------------------------------------------------------*/
...
FUNCTION extern INT16 CALL_CONV profi_read_dps_data(IN HANDLE hDevice,
    ...
    OUT:    pState      -> status of input or output data if read or write was
    successfully
...
FUNCTION extern BOOL CALL_CONV profi_set_timeout(IN HANDLE hBasicMgmtDevice,
    ...
    IN:  ReadTimeout       -> receive timeout in ms (WAIT_FOREVER for infinite
    wait)
    IN:  WriteTimeout      -> send timeout in ms (WAIT_FOREVER for infinite
    wait)
...
FUNCTION extern BOOL CALL_CONV profi_get_timeout(IN HANDLE hBasicMgmtDevice,
    ...
    This function is used to get the set send and receive TIMEOUTs for
    SEND/RECEIVE
    interface functions

    IN:   hBasicMgmtDevice  -> basic management device handle
    OUT:  pReadTimeout      -> pointer to receive timeout in ms (WAIT_FOREVER
    for infinite wait)
...
FUNCTION extern BOOL CALL_CONV profi_get_queue_size(IN HANDLE hBasicMgmtDevice,
    ...
    This function is used to get the set max. receive queue size for the
    PROFIBUS driver.
