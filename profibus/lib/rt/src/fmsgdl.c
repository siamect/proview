...
-----------------------------------------------------------------------------*/
{
  ...
  case FMS_INITIATE:
    if (primitive == REQ)
      ...
      return (sizeof(T_CTXT_INIT_CNF));

  case FMS_ABORT:
    if (primitive == REQ)
      return (sizeof(T_CTXT_ABORT_REQ));
    else
      return (0);
...
-----------------------------------------------------------------------------*/
{
  ...
  case FMS_IDENTIFY:
    if (primitive == RES)
      return (sizeof(T_VFD_IDENTIFY_CNF));
    else
      return (0);

  case FMS_UNSOLICITEDSTATUS:
    if (primitive == REQ)
      return (sizeof(T_VFD_UNSOL_STATUS_REQ));
    else
      return (0);

  case FMS_CREATE_VFD_LOC:
    if (primitive == REQ)
      return (sizeof(T_VFD_CREATE_REQ));
    else
      return (0);

  case FMS_VFD_SET_PHYS_STATUS_LOC:
    if (primitive == REQ)
      return (sizeof(T_VFD_SET_PHYS_STATUS_REQ));
    else
      return (0);
...
-----------------------------------------------------------------------------*/
{
  ...
  case FMS_INIT_PUT_OD:
    if (primitive == REQ)
      return (sizeof(T_INIT_PUT_OD_REQ));
    else
      return (0);
  ...
  case FMS_INIT_LOAD_OD_LOC:
    if (primitive == REQ)
      return (sizeof(T_INIT_LOAD_OD_REQ));
    else
      return (0);

  case FMS_TERM_LOAD_OD_LOC:
    if (primitive == REQ)
      return (sizeof(T_TERM_LOAD_OD_REQ));
    else
      return (0);
  ...
  case FMS_OD_READ_LOC:
    if (primitive == REQ)
      return (sizeof(T_OD_READ_LOC_REQ));
    else
      return (0);
...
-----------------------------------------------------------------------------*/
{
  ...
  case FMS_DEL_VAR_LIST:
    if (primitive == REQ)
      return (sizeof(T_VAR_DELETE_VAR_LIST_REQ));
    else
      return (0);
...
-----------------------------------------------------------------------------*/
{
  ...
  case FMS_ALT_EVN_CND_MNT:
    if (primitive == REQ)
      return (sizeof(T_ALT_EVN_CND_MNT_REQ));
    else
      return (0);

  case FMS_ACK_EVN_NOTIFY:
    if (primitive == REQ)
      return (sizeof(T_ACK_EVN_NOTIFY_REQ));
    else
      return (0);
...
-----------------------------------------------------------------------------*/
{
  ...
  case FMS_GEN_TERM_DOWNL_SEQ:
    if (primitive == REQ)
      ...
      return (sizeof(T_GEN_TERM_DNL_CNF));
...
-----------------------------------------------------------------------------*/
{
  ...
  case FMS_PI_DELETE:
    if (primitive == REQ)
      return (sizeof(T_PI_DEL_REQ));
    else
      return (0);

  case FMS_PI_START:
    if (primitive == REQ)
      return (sizeof(T_PI_START_REQ));
    else
      return (0);

  case FMS_PI_STOP:
    if (primitive == REQ)
      return (sizeof(T_PI_STOP_REQ));
    else
      return (0);

  case FMS_PI_RESUME:
    if (primitive == REQ)
      return (sizeof(T_PI_RESUME_REQ));
    else
      return (0);

  case FMS_PI_RESET:
    if (primitive == REQ)
      return (sizeof(T_PI_RESET_REQ));
    else
      return (0);

  case FMS_PI_KILL:
    if (primitive == REQ)
      return (sizeof(T_PI_KILL_REQ));
    else
      return (0);

  case FMS_PI_SET_STATE_LOC:
    if (primitive == REQ)
      return (sizeof(T_PI_SET_STATE_REQ));
    else
      return (0);
...
-----------------------------------------------------------------------------*/
{
  ...
  *data_len_ptr = 0;
  ...
  case FMS_ABORT:
    if (result == POS)
      *data_len_ptr = fmsgdl_get_ctxt_data_len(service, primitive);
    else
      *data_len_ptr = fmsgdl_get_error_data_len(service);
    break;
  ...
  case FMS_VFD_SET_PHYS_STATUS_LOC:
    if (result == POS)
      *data_len_ptr = fmsgdl_get_vfd_data_len(service, primitive);
    else
      *data_len_ptr = fmsgdl_get_error_data_len(service);
    break;
  ...
  case FMS_REQ_DOM_UPL:
    if (result == POS)
      *data_len_ptr = fmsgdl_get_dom_data_len(service, primitive, data_ptr);
    else
      *data_len_ptr = fmsgdl_get_error_data_len(service);
    break;
  ...
  case FMS_ACK_EVN_NOTIFY:
    if (result == POS)
      *data_len_ptr = fmsgdl_get_evn_data_len(service, primitive, data_ptr);
    else
      *data_len_ptr = fmsgdl_get_error_data_len(service);
    break;
  ...
  case FMS_TERM_LOAD_OD_LOC:
    if (result == POS)
      *data_len_ptr = fmsgdl_get_od_data_len(service, primitive, data_ptr);
    else
      *data_len_ptr = fmsgdl_get_error_data_len(service);
    break;
  ...
  case FMS_PI_SET_STATE_LOC:
    if (result == POS)
      *data_len_ptr = fmsgdl_get_pi_data_len(service, primitive, data_ptr);
    else
      *data_len_ptr = fmsgdl_get_error_data_len(service);
    break;
  ...
  case FMS_PHYS_WRITE:
    if (result == POS)
      *data_len_ptr = fmsgdl_get_var_data_len(service, primitive, data_ptr);
    else
      *data_len_ptr = fmsgdl_get_error_data_len(service);
    break;
