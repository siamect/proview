...
FUNCTION extern PB_BOOL CALL_CONV pbt_get_fms_fm7_def_bus_param(
    ...
    IN USIGN16 Mode, OUT T_SET_BUSPARAMETER_REQ FAR* pBusParameters)
...
FUNCTION extern PB_BOOL CALL_CONV pbt_get_fdlif_def_bus_param(
    IN USIGN8 BaudRate, IN USIGN8 StationAddr, IN PB_BOOL InRingDesired,
    IN USIGN16 Mode, OUT T_FDLIF_SET_BUSPARAMETER_REQ FAR* pBusParameters)
...
#ifdef __PB_FM7__
FUNCTION extern VOID CALL_CONV ccrl_resrces_init(IN T_CRL_HDR FAR* pCrlHdr)

    /*----------------------------------------------------------------------------
    FUNCTIONAL_DESCRIPTION

    At first this function has to be called to initialize the internal
    structures
...
ccrl_resrces_add_entry(IN USIGN16 Cr, IN T_CRL_STATIC FAR* pCrlStatic,
    ...
    corrected
    it returns with a negative result. If the entry is OK, this function
    calculates the
    resources. After function return, the output buffer holds the sum of
    resources that
    are needed by all CRL entries that where put into ccrl_resrces_add_entry
    since last
    call of ccrl_resrces_init. The result after the last call of
    ccrl_resrces_add_entry
    this is the number of all needed resources for the whole CRL. This result
    can be used
    ...
    - E_FM7_CRL_INVALID_ENTRY     -> invalid entry found in CRL, resources are
    not
                                     calculated
