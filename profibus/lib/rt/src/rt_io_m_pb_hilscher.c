...
The code refers to some documents provided by Hilscher, the documents are:
        ...
        dpm_pie.pdf: found on the system software disc that comes with the
board.
*/
...
static short dpm_init(io_sAgentLocalHilscher* local, io_sAgent* ap)
{
  short rv;
...
be returned.  */
static short dpm_exit(io_sAgentLocalHilscher* local, io_sAgent* ap)
{
  short rv;
...
static short module_cnt_inputs(io_sCard* cp)
{
...
(Profibus module) cp.  Based on the IoRackInit() method of
rt_io_m_pb_dp_slave.c.  */
static short module_cnt_outputs(io_sCard* cp)
{
...
{
  errh_Error("Profibus DP Master %s - DDLM Download: this board", ap->Name);
...
static void dpm_ddlm_answer_msg_print_error(RCS_MESSAGE* msg, io_sAgent* ap)
{
  char* s;
...
static void dpm_print_diag(io_sAgent* ap, DPM_DIAGNOSTICS* diag)
{
  char* s;
  ...
  else if (diag->tError.bErr_Rem_Adr == 255)
  {
    ...
    case TASK_F_NO_USR_TASK:
      ...
      break;
    case TASK_F_NO_GLOBAL_DATA:
      s = "no global entry in data base";
      break;
    case TASK_F_NO_FDL:
      s = "fdl task not found";
      break;
    case TASK_F_NO_PLC:
      s = "plc task not found";
      break;
    case TASK_F_NO_BUS_DP:
      s = "no bus parameters";
      break;
    case TASK_F_INVALID_BUS_DP:
      s = "bus parameters faulty";
      break;
    case TASK_F_NO_SL_TAB:
      s = "no slave data sets";
      break;
    case TASK_F_INVALID_SL_TAB:
      s = "slave data set faulty";
      break;
    case TASK_F_REM_ADR_DOUBLE:
      s = "double station addr detected";
      break;
    case TASK_F_A_OFFSET_MAX:
      s = "maximum output offset reached";
      break;
    case TASK_F_E_OFFSET_MAX:
      s = "maximum input offset reached";
      break;
    case TASK_F_A_OVERLAP:
      s = "overlap in output area";
      break;
    case TASK_F_E_OVERLAP:
      s = "overlap in input area";
      break;
    case TASK_F_DPM_UNKNOWN_MODE:
      s = "warmstart with unknown mode";
      break;
    case TASK_F_RAM_OVERRUN:
      s = "extended ram exceeded";
      break;
    case TASK_F_SL_PRM_FAULT:
      s = "slave data set faulty";
      break;
    case TASK_F_WATCHDOG:
      s = "user watchdog failure";
      break;
    case TASK_F_NO_DATA_ACK:
      s = "mode0 no acknowledge";
      break;
    case TASK_F_AUTO_CLEAR:
      s = "auto clear activated";
      break;
    case TASK_F_FATAL_ASIC_ERROR:
      s = "fatal error occurred";
      break;
    case TASK_F_UNKNOWN_EVENT:
      s = "unknown event detected";
      break;
    case TASK_F_SEGMENT_CNT:
      s = "no segments to communicate";
      break;
    default:
      return;
    }
  ...
  else
  {
    ...
    case CON_NA:
      ...
      break;
    case CON_DS:
      s = "local FDL not in logical token ring";
      break;
    case CON_RS:
      s = "master request not supported by the slave";
      break;
    case CON_NR:
      s = "no response data";
      break;
                                  ...
                                  unsigned char* db_startsegment)
{
  ...
  errh_Info("Start segment of database: %d", known_boards[i].db_startsegment);
                                    ...
                                    io_sAgent* ap)
{
  ...
  if ((rv = dpm_check_board_type(local, ap, &db_startsegment)) != DRV_NO_ERROR)
    return rv;
