...
----------------------------------------------------------------------------*/
{
  ...
  case FM7_READ_CRL_REM:
    ...
    else
    {
      ...
      if (rsp->desired_cr == 0)
        return (sizeof(T_READ_CRL_CNF));
      else
        return (sizeof(T_READ_CRL_CNF) + rsp->id.crl_entry.crl_status_len);
    }
...
-----------------------------------------------------------------------------*/
{
  ...
  case FM7_SET_BUSPARAMETER:
    if (primitive == REQ)
      return (sizeof(T_SET_BUSPARAMETER_REQ));
    else
      return (0);
...
-----------------------------------------------------------------------------*/
{
  ...
  if (primitive == REQ)
    return (sizeof(T_IDENT_REQ));
  else
    return (sizeof(T_IDENT_CNF));
}

FUNCTION LOCAL INT16 fm7gdl_get_live_list_data_len(IN VOID)
...
FUNCTION LOCAL INT16 fm7gdl_get_lsap_status_data_len(IN USIGN8 primitive)
...
-----------------------------------------------------------------------------*/
{
  ...
  if (primitive == REQ)
    return (sizeof(T_LSAP_STATUS_REQ));
  else
    return (sizeof(T_LSAP_STATUS_CNF));
}

FUNCTION LOCAL INT16 fm7gdl_get_fault_mngt_data_len(IN VOID)
...
-----------------------------------------------------------------------------*/
{
  ...
  *data_len_ptr = 0;
  ...
  case FM7_TERM_LOAD_CRL_REM:
    if (result == POS)
      *data_len_ptr = fm7gdl_get_crl_data_len(service, primitive, data_ptr);
    else
  ...
  case FM7_LSAP_STATUS_REM:
    if (result == POS)
      *data_len_ptr = fm7gdl_get_lsap_status_data_len(primitive);
    else
      *data_len_ptr = fm7gdl_get_error_data_len(service);
    break;
  ...
  case FM7_IDENT_REM:
    if (result == POS)
      *data_len_ptr = fm7gdl_get_ident_data_len(primitive);
    else
      *data_len_ptr = fm7gdl_get_error_data_len(service);
    break;
  ...
  case FM7_EXIT:
    if (result == POS)
      *data_len_ptr = fm7gdl_get_fault_mngt_data_len();
    else
      *data_len_ptr = fm7gdl_get_error_data_len(service);
    break;
  ...
  case FM7_GET_LIVE_LIST:
    if (result == POS)
      *data_len_ptr = fm7gdl_get_live_list_data_len();
    else
      *data_len_ptr = fm7gdl_get_error_data_len(service);
    break;
  ...
  case FM7_ABORT:
    if (result == POS)
      *data_len_ptr = fm7gdl_get_ctxt_data_len(service, primitive);
    else
      *data_len_ptr = fm7gdl_get_error_data_len(service);
    break;
