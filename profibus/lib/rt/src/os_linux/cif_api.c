  ...
  if (hDevDrv == INVALID_HANDLE_VALUE) {
    ...
    /*,				// address of buffer for input data
       ...
       // size of output buffer
                                                                    (unsigned
       int *)&lBytesReturned,  // address of actual bytes of output
                                                                    NULL) )
    ...
    unsigned short usSize, void* pvData)
{
  ...
  if (hDevDrv == INVALID_HANDLE_VALUE) {
    ...
    // insert error for older driver versions, because function which are not
    // included
...
/* <ST>
  ...
  ---------------------------------------------------------------------------------------
  Input   : usDevNumber   - number of the DEV board     (0..3)
            usSize        - size of the users data area (>0..<=
  sizeof(MSG_STRUC))
...
/* <ST>
  =================================================================================
  Function: CloseFile
            Close a previously opened file by OpenFile(), free all memory and
  handles
  ---------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
short ReadDeviceInformation(unsigned short usDevNumber, const char* fileName)
{
  int dwStrLen;
  short sRet;
    ...
    unsigned short usDevNumber, FILEDATA* ptFile, unsigned long* pdwByte)
{
  ...
  *pdwByte = lActIdx;
  ...
  if (lFileLength > MAX_DOWNLOAD_MSG_LENGTH)
    lSendLen = MAX_DOWNLOAD_MSG_LENGTH;
  else
    lSendLen = lFileLength;
  ...
  do {
    ...
    case RCS_FIRST_MSK:
      ...
      // Create check sum
      usCheckSum = CreateChecksum(pabData, lFileLength, &tSendMsg);
      // Insert check sum
    ...
    case RCS_LAST_MSK:
      ...
      // Process message
      sRet = TransferMessage(usDevNumber, &tSendMsg, &tRecvMsg, TO_LAST_MSG);
    ...
    unsigned char* pszFileName, unsigned long* pdwBytes)
{
  ...
  if (hDevDrv == INVALID_HANDLE_VALUE) {
    ...
    case CONFIGURATION_DOWNLOAD:
      sRet = ConfigDownload(usDevNumber, (const char*)pszFileName, pdwBytes);
      break;
    default:
      sRet = DRV_USR_MODE_INVALID;
    } // end switch
