...
----------------------------------------------------------------------------*/
{
  ...
  if (CUSTOMER_FLAG_SET(LastErrorCode))
    return ((INT16)LastErrorCode);
  else
...
IN:  Index             -> index of the device to open
                          1..128           DP service-, DP slave data- and DP/V1
service devices
...
FUNCTION GLOBAL BOOL CALL_CONV profi_close(IN HANDLE hDevice)
/*----------------------------------------------------------------------------
FUNCTIONAL_DESCRIPTION
...
- E_IF_INVALID_DATA_SIZE         -> not enough CMI memory available for REQ or
RES
...
possible return values:
- E_OK                          -> OK
- E_IF_INVALID_DATA_SIZE        -> data_size does not match the expected input
data size
- E_IF_OS_ERROR                 -> NT system error
