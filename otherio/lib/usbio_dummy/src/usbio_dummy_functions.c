...
int USBIO_WriteLowFreq(int* Handle, int Freq, unsigned char PortA,
    unsigned char PortB, unsigned char PortC)
{
  return -1;
}
...
int USBIO_ConfigWatchdog(int* Handle, int Active, int TimeOut, int Reset,
    unsigned char PortMask[], unsigned char Port[], int AOfunction)
{
  return -1;
}
...
// Advanced functions
int USBIO_WriteAndRead(
    int* Handle, unsigned char WriteBuffer[], unsigned char ReadBuffer[])
{
  return -1;
}
