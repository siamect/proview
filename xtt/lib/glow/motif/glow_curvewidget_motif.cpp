...
static void Redisplay(Widget w, XEvent* event, Region region)
{
  ((GlowDrawXLib*)((CurveCtx*)((CurveWidget)w)->curve.curve_ctx)->gdraw)
      ->event_handler(*event);
}
...
{
  (*curveWidgetClass->core_class.superclass->core_class.realize)(w, mask, swa);

  if (((CurveWidget)w)->curve.is_navigator) {
    if (!((CurveWidget)w)->curve.curve_ctx) {
      CurveWidget main_curve
          = (CurveWidget)((CurveWidget)w)->curve.main_curve_widget;
    ...
    if (!((CurveWidget)w)->curve.curve_ctx) {
      ((CurveWidget)w)->curve.draw_ctx = new GlowDrawXLib(w,
          &((CurveWidget)w)->curve.curve_ctx, curve_init_proc,
          ((CurveWidget)w)->curve.client_data, glow_eCtxType_Curve);
    }
  }
}

extern "C" Widget CurveCreate(Widget parent, char* name, ArgList args,
    int argCount, int (*init_proc)(GlowCtx* ctx, void* client_data),
    void* client_data)
{
  CurveWidget w;

  w = (CurveWidget)XtCreateWidget(
      name, curveWidgetClass, parent, args, argCount);
...
extern "C" Widget ScrolledCurveCreate(Widget parent, char* name, ArgList args,
    int argCount, int (*init_proc)(GlowCtx* ctx, void* client_data),
    void* client_data, Widget* curve_w)
{
  ...
  */
  curve = (CurveWidget)CurveCreate(
      form, (char*)"curve", arg, i, init_proc, client_data);
...
Widget CurveCreateNav(
    Widget parent, char* name, ArgList args, int argCount, Widget main_curve)
{
  CurveWidget w;
