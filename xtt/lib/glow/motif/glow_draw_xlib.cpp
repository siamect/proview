...
static int glow_allocate_named_color(
    GlowDrawXLib* draw_ctx, const char* named_color);
static int glow_allocate_color(
    GlowDrawXLib* draw_ctx, int rgb_red, int rgb_green, int rgb_blue);
...
static GC get_gc(GlowDrawXLib* draw_ctx, int i, int j)
{
  if (!draw_ctx->gcs[i][j]) {
    ...
    draw_ctx->gcs[i][j] = XCreateGC(draw_ctx->display, draw_ctx->m_wind.window,
        GCForeground | GCBackground | GCLineWidth | GCCapStyle | GCFillRule,
        &xgcv);
  }
...
static int glow_create_gc(GlowDrawXLib* draw_ctx, Window window)
{
  ...
  xgcv.cap_style = CapButt;
  draw_ctx->gc_inverse = XCreateGC(draw_ctx->display, window,
      GCForeground | GCBackground | GCCapStyle, &xgcv);
  ...
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    ...
    draw_ctx->gcs[glow_eDrawType_Line][i] = XCreateGC(draw_ctx->display, window,
        GCForeground | GCBackground | GCLineWidth | GCCapStyle, &xgcv);
  }
  ...
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    ...
    draw_ctx->gcs[glow_eDrawType_LineErase][i]
        = XCreateGC(draw_ctx->display, window,
            GCForeground | GCBackground | GCLineWidth | GCCapStyle | GCFillRule,
            &xgcv);
  }
  ...
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    ...
    draw_ctx->gcs[glow_eDrawType_LineGray][i]
        = XCreateGC(draw_ctx->display, window,
            GCForeground | GCBackground | GCLineWidth | GCCapStyle | GCFillRule,
            &xgcv);
  }
  ...
  if (ODD(sts)) {
    ...
    for (int j = glow_eDrawType_Color4; j <= glow_eDrawType_Color300; j++) {
      ...
      for (i = 0; i < DRAW_TYPE_SIZE; i++) {
        ...
        draw_ctx->gcs[j][i] = XCreateGC(draw_ctx->display, window,
            GCForeground | GCBackground | GCLineWidth | GCCapStyle | GCFillRule,
            &xgcv);
      }
      color_p++;
  ...
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    ...
    draw_ctx->gcs[glow_eDrawType_LineDashed][i] = XCreateGC(draw_ctx->display,
        window, GCForeground | GCBackground | GCLineWidth | GCLineStyle
            | GCDashOffset | GCDashList | GCCapStyle,
        &xgcv);
  }
  ...
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    ...
    draw_ctx->gcs[glow_eDrawType_LineDashedRed][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground | GCLineWidth
            | GCLineStyle | GCDashOffset | GCDashList | GCCapStyle,
        &xgcv);
  }
  ...
  xgcv.background = draw_ctx->background;
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    draw_ctx->gcs[glow_eDrawType_TextHelvetica][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground, &xgcv);
  }

  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    draw_ctx->gcs[glow_eDrawType_TextHelveticaBold][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground, &xgcv);
  }
  ...
  xgcv.background = draw_ctx->background;
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    draw_ctx->gcs[glow_eDrawType_TextHelveticaErase][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground, &xgcv);
  }

  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    draw_ctx->gcs[glow_eDrawType_TextHelveticaEraseBold][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground, &xgcv);
  }
  ...
  for (i = 0; i < DRAW_FONT_SIZE; i++) {
    ...
    XSetFont(draw_ctx->display, draw_ctx->gcs[glow_eDrawType_TextHelvetica][i],
        font);
    XSetFont(draw_ctx->display,
    ...
    GlowDrawXLib* draw_ctx, const char* named_color)
{
  ...
  if (visual->c_class == TrueColor || visual->c_class == PseudoColor
      || visual->c_class == DirectColor || visual->c_class == StaticColor) {
    if (XAllocNamedColor(draw_ctx->display,
            XDefaultColormapOfScreen(draw_ctx->screen), named_color,
    ...
    GlowDrawXLib* draw_ctx, int rgb_red, int rgb_green, int rgb_blue)
{
  ...
  if (visual->c_class == TrueColor || visual->c_class == PseudoColor
    ...
    if (XAllocColor(draw_ctx->display,
            XDefaultColormapOfScreen(draw_ctx->screen), &exact_color))
...
GlowDrawXLib::GlowDrawXLib(Widget toplevel, void** glow_ctx,
    int (*init_proc)(Widget w, GlowCtx* ctx, void* client_data),
    void* client_data, glow_eCtxType type)
    : ef(0), background(0), original_background(0), timer_id(0),
      click_sensitivity(0)
{
  int i;
  Arg args[20];
  ...
  *glow_ctx = (void*)ctx;
  ...
  i = 0;
  XtSetArg(args[i], XmNbackground, &background);
  i++;
...
int GlowDrawXLib::event_handler(XEvent event)
{
  ...
  if (event.xany.window == m_wind.window) {
    ...
    case KeyPress: {
      ...
      XtTranslateKeycode(display, event.xkey.keycode, 0, &mod, &keysym);
      keysym &= 0xFFFF;
      if ((keysym >= 0x020 && keysym <= 0x20ac)
          || (keysym >= 0xFF80 && keysym <= 0xFFB9 && keysym != XK_KP_Enter
                 && keysym != 0xFF44)) {
        char buff;
    ...
    case ButtonPress:
      ...
      case Button1:
        ctx->event_handler(
            glow_eEvent_MB1Down, event.xbutton.x, event.xbutton.y, 0, 0);
      ...
      case Button3:
        ctx->event_handler(
            glow_eEvent_MB3Down, event.xbutton.x, event.xbutton.y, 0, 0);
        if (click_sensitivity & glow_mSensitivity_MB3Press
          ...
          ctx->event_handler(
              glow_eEvent_MB3Press, event.xbutton.x, event.xbutton.y, 0, 0);
          click_sensitivity = 0;
          return 1;
      ...
      if (!button_pressed) {
        ...
        case Button1:
          ...
          if ((event.xbutton.state & ShiftMask)
            ...
            ctx->event_handler(glow_eEvent_MB1PressShift, event.xbutton.x,
                event.xbutton.y, 0, 0);
            click_sensitivity = 0;
          } else if (!(event.xbutton.state & ShiftMask)
              && (event.xbutton.state & ControlMask)) {
            ctx->event_handler(glow_eEvent_MB1PressCtrl, event.xbutton.x,
                event.xbutton.y, 0, 0);
            click_sensitivity = 0;
          } else if ((event.xbutton.state & ShiftMask)
              && (event.xbutton.state & ControlMask)) {
            ctx->event_handler(glow_eEvent_MB1PressShiftCtrl, event.xbutton.x,
                event.xbutton.y, 0, 0);
            click_sensitivity = 0;
          } else {
            ctx->event_handler(
                glow_eEvent_MB1Press, event.xbutton.x, event.xbutton.y, 0, 0);
            click_sensitivity = 0;
          }
          break;
        case Button2:
          ...
          if ((event.xbutton.state & ShiftMask)
              && !(event.xbutton.state & ControlMask)) {
            ctx->event_handler(glow_eEvent_MB2PressShift, event.xbutton.x,
                event.xbutton.y, 0, 0);
            click_sensitivity = 0;
          } else if (!(event.xbutton.state & ShiftMask)
              && (event.xbutton.state & ControlMask)) {
            ...
            click_sensitivity = 0;
              ...
              && (event.xbutton.state & ControlMask)) {
            ctx->event_handler(glow_eEvent_MB2PressShiftCtrl, event.xbutton.x,
                event.xbutton.y, 0, 0);
            click_sensitivity = 0;
          } else {
            ctx->event_handler(
                glow_eEvent_MB2Press, event.xbutton.x, event.xbutton.y, 0, 0);
            click_sensitivity = 0;
          }
          break;
        case Button3:
    ...
    case ButtonRelease:
      ...
      case Button1:
        ctx->event_handler(
            glow_eEvent_MB1Up, event.xbutton.x, event.xbutton.y, 0, 0);
        break;
      }
      ...
      if (!button_pressed) {
        if (button_clicked) {
          ...
          // cout << "Button click detected state " << event.xbutton.state <<
          // endl;
          button_clicked = 0;
          switch (event.xbutton.button) {
          case Button1:
            if ((event.xbutton.state & ShiftMask)
                && !(event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB1ClickShift, event.xbutton.x,
                  event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else if (!(event.xbutton.state & ShiftMask)
                && (event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB1ClickCtrl, event.xbutton.x,
                  event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else if ((event.xbutton.state & ShiftMask)
                && (event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB1ClickShiftCtrl, event.xbutton.x,
                  event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else {
              ctx->event_handler(
                  glow_eEvent_MB1Click, event.xbutton.x, event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            }
            break;
          case Button2:
            if ((event.xbutton.state & ShiftMask)
                && !(event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB2ClickShift, event.xbutton.x,
                  event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else if (!(event.xbutton.state & ShiftMask)
                && (event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB2ClickCtrl, event.xbutton.x,
                  event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else if ((event.xbutton.state & ShiftMask)
                && (event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB2ClickShiftCtrl, event.xbutton.x,
                  event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else {
              ctx->event_handler(
                  glow_eEvent_MB2Click, event.xbutton.x, event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            }
            break;
          case Button3:
            ctx->event_handler(
                glow_eEvent_MB3Click, event.xbutton.x, event.xbutton.y, 0, 0);
            click_sensitivity = 0;
            break;
          }
        } else {
          ...
          ctx->event_handler(glow_eEvent_ButtonRelease, event.xbutton.x,
              event.xbutton.y, 0, 0);
        }
      } else {
        /* Button click */
        ...
        if (!button_clicked_and_pressed) {
          ...
          case Button1:
            if ((event.xbutton.state & ShiftMask)
                && !(event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB1DoubleClickShift,
                  event.xbutton.x, event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else if (!(event.xbutton.state & ShiftMask)
                && (event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB1DoubleClickCtrl,
                  event.xbutton.x, event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else if ((event.xbutton.state & ShiftMask)
                && (event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB1DoubleClickShiftCtrl,
                  event.xbutton.x, event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else {
              ctx->event_handler(glow_eEvent_MB1DoubleClick, event.xbutton.x,
                  event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            }
            break;
          case Button2:
            if ((event.xbutton.state & ShiftMask)
                && !(event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB2DoubleClickShift,
                  event.xbutton.x, event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else if (!(event.xbutton.state & ShiftMask)
                && (event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB2DoubleClickCtrl,
                  event.xbutton.x, event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else if ((event.xbutton.state & ShiftMask)
                && (event.xbutton.state & ControlMask)) {
              ctx->event_handler(glow_eEvent_MB2DoubleClickShiftCtrl,
                  event.xbutton.x, event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            } else {
              ctx->event_handler(glow_eEvent_MB2DoubleClick, event.xbutton.x,
                  event.xbutton.y, 0, 0);
              click_sensitivity = 0;
            }
            break;
    ...
    case Expose:
      ...
      ctx->event_handler(glow_eEvent_Exposure, event.xexpose.x, event.xexpose.y,
    ...
    case MotionNotify:
      ...
      if (button1_pressed || button2_pressed || button3_pressed)
        ctx->event_handler(
            glow_eEvent_ButtonMotion, event.xbutton.x, event.xbutton.y, 0, 0);
      else
        ctx->event_handler(
            glow_eEvent_CursorMotion, event.xbutton.x, event.xbutton.y, 0, 0);
      break;
    case EnterNotify:
      //        printf( "-- Enter event: (%d,%d)\n", event.xbutton.x,
      //		event.xbutton.y);
      ctx->event_handler(
          glow_eEvent_Enter, event.xbutton.x, event.xbutton.y, 0, 0);
      break;
    case LeaveNotify:
      ...
      ctx->event_handler(
          glow_eEvent_Leave, event.xbutton.x, event.xbutton.y, 0, 0);
      break;
    case MapNotify:
      //        printf( "-- Enter event: (%d,%d)\n", event.xbutton.x,
      //		event.xbutton.y);
    ...
    case ButtonPress:
      ...
      case Button1:
        ...
        ctx->event_handler_nav(
            glow_eEvent_MB1Press, event.xbutton.x, event.xbutton.y);
        break;
      case Button2:
        button2_pressed = 1;
        ctx->event_handler_nav(
            glow_eEvent_MB2Press, event.xbutton.x, event.xbutton.y);
        break;
      case Button3:
        button3_pressed = 1;
        break;
    ...
    case ButtonRelease:
      ...
      ctx->event_handler_nav(
          glow_eEvent_ButtonRelease, event.xbutton.x, event.xbutton.y);
      break;
    case Expose:
      //        printf( "-- Navigator expose event..\n" );
      ctx->event_handler_nav(
          glow_eEvent_Exposure, event.xbutton.x, event.xbutton.y);
      break;
    case MotionNotify:
      ...
      if (button1_pressed || button2_pressed || button3_pressed)
        ctx->event_handler_nav(
            glow_eEvent_ButtonMotion, event.xbutton.x, event.xbutton.y);
      else
        ctx->event_handler_nav(
            glow_eEvent_CursorMotion, event.xbutton.x, event.xbutton.y);
      break;
    }
  }
  XFlush(display);
...
void GlowDrawXLib::enable_event(glow_eEvent event, glow_eEventType event_type,
    ...
    glow_eDrawType gc_type, int idx, int highlight)
{
  ...
  if (!w->draw_buffer_only)
    XDrawRectangle(display, w->window, get_gc(this, gc_type + highlight, idx),
        x, y, width, height);
  if (w->double_buffer_on)
    XDrawRectangle(display, w->buffer, get_gc(this, gc_type + highlight, idx),
        x, y, width, height);
    ...
    GlowWind* wind, int x, int y, int width, int height, int idx)
{
  ...
  if (!w->draw_buffer_only)
    XDrawRectangle(display, w->window,
        get_gc(this, glow_eDrawType_LineErase, idx), x, y, width, height);
  if (w->double_buffer_on)
...
int GlowDrawXLib::arrow(GlowWind* wind, int x1, int y1, int x2, int y2, int x3,
    int y3, glow_eDrawType gc_type, int idx, int highlight)
{
  ...
  if (!w->draw_buffer_only)
    XFillPolygon(display, w->window, get_gc(this, gc_type + highlight, idx), p,
        4, Convex, CoordModeOrigin);
  if (w->double_buffer_on)
    XFillPolygon(display, w->buffer, get_gc(this, gc_type + highlight, idx), p,
        4, Convex, CoordModeOrigin);
    ...
    GlowWind* wind, int x1, int y1, int x2, int y2, int x3, int y3, int idx)
{
  ...
  if (!w->draw_buffer_only)
    XFillPolygon(display, w->window,
        get_gc(this, glow_eDrawType_LineErase, idx), p, 4, Convex,
        CoordModeOrigin);
  if (w->double_buffer_on)
    ...
    int angle1, int angle2, glow_eDrawType gc_type, int idx, int highlight)
{
  ...
  if (!w->draw_buffer_only)
    XDrawArc(display, w->window, get_gc(this, gc_type + highlight, idx), x, y,
        width, height, angle1 * 64, angle2 * 64);

  if (w->double_buffer_on)
    XDrawArc(display, w->buffer, get_gc(this, gc_type + highlight, idx), x, y,
        width, height, angle1 * 64, angle2 * 64);
    ...
    int angle1, int angle2, glow_eDrawType gc_type, int highlight)
{
  ...
  if (!w->draw_buffer_only)
    XFillArc(display, w->window, get_gc(this, gc_type + highlight, 0), x, y,
        width, height, angle1 * 64, angle2 * 64);
  if (w->double_buffer_on)
    XFillArc(display, w->buffer, get_gc(this, gc_type + highlight, 0), x, y,
        width, height, angle1 * 64, angle2 * 64);
    ...
    int angle1, int angle2, int idx)
{
  ...
  if (!w->draw_buffer_only)
    XDrawArc(display, w->window, get_gc(this, glow_eDrawType_LineErase, idx), x,
        y, width, height, angle1 * 64, angle2 * 64);
  if (w->double_buffer_on)
    XDrawArc(display, w->buffer, get_gc(this, glow_eDrawType_LineErase, idx), x,
        y, width, height, angle1 * 64, angle2 * 64);
    ...
    glow_eDrawType gc_type, int idx, int highlight)
{
  ...
  if (!w->draw_buffer_only)
    XDrawLine(display, w->window, get_gc(this, gc_type + highlight, idx), x1,
        y1, x2, y2);
  if (w->double_buffer_on)
    XDrawLine(display, w->buffer, get_gc(this, gc_type + highlight, idx), x1,
        y1, x2, y2);
    ...
    glow_eDrawType gc_type, int idx, int highlight, glow_eLineType line_type)
{
  ...
  case glow_eLineType_Dotted: {
    ...
    list[0] = 1 + idx;
    list[1] = 1 + 4 * idx;
    XSetDashes(display, get_gc(this, gc_type + highlight, idx), 0, list, 2);
    XChangeGC(display, get_gc(this, gc_type + highlight, idx),
  ...
  case glow_eLineType_DotDashed1: {
    ...
    list[2] = 1 + idx;
    list[3] = 1 + 2 * idx;
    XSetDashes(display, get_gc(this, gc_type + highlight, idx), 0, list, 4);
    XChangeGC(display, get_gc(this, gc_type + highlight, idx),
        GCLineStyle | GCDashOffset, &xgcv);
    break;
  }
  ...
  XChangeGC(
      display, get_gc(this, gc_type + highlight, idx), GCLineStyle, &xgcv);
  return 1;
}

int GlowDrawXLib::line_erase(
    GlowWind* wind, int x1, int y1, int x2, int y2, int idx)
{
  ...
  if (!w->draw_buffer_only)
    XDrawLine(display, w->window, get_gc(this, glow_eDrawType_LineErase, idx),
        x1, y1, x2, y2);
  if (w->double_buffer_on)
    XDrawLine(display, w->buffer, get_gc(this, glow_eDrawType_LineErase, idx),
        x1, y1, x2, y2);
    ...
    glow_eDrawType gc_type, int idx, int highlight)
{
  ...
  if (!w->draw_buffer_only)
    XDrawLines(display, w->window, get_gc(this, gc_type + highlight, idx),
        (XPoint*)points, point_cnt, CoordModeOrigin);
  if (w->double_buffer_on)
    XDrawLines(display, w->buffer, get_gc(this, gc_type + highlight, idx),
        (XPoint*)points, point_cnt, CoordModeOrigin);
...
int GlowDrawXLib::fill_polyline(GlowWind* wind, glow_sPointX* points,
    int point_cnt, glow_eDrawType gc_type, int highlight)
{
  ...
  if (!w->draw_buffer_only)
    XFillPolygon(display, w->window, get_gc(this, gc_type + highlight, 0),
        (XPoint*)points, point_cnt, Nonconvex, CoordModeOrigin);
  if (w->double_buffer_on)
    XFillPolygon(display, w->buffer, get_gc(this, gc_type + highlight, 0),
        (XPoint*)points, point_cnt, Nonconvex, CoordModeOrigin);
...
int GlowDrawXLib::polyline_erase(
    GlowWind* wind, glow_sPointX* points, int point_cnt, int idx)
{
  ...
  if (!w->draw_buffer_only)
    XDrawLines(display, w->window, get_gc(this, glow_eDrawType_LineErase, idx),
        (XPoint*)points, point_cnt, CoordModeOrigin);
  if (w->double_buffer_on)
    XDrawLines(display, w->buffer, get_gc(this, glow_eDrawType_LineErase, idx),
        (XPoint*)points, point_cnt, CoordModeOrigin);
    ...
    int line, glow_eFont font_idx, double size, int rot)
{
  ...
  if (font_idx != glow_eFont_Helvetica)
    XSetFont(
        display, get_gc(this, gc_type, idx), font[font_idx][font_type][idx]);

  if (color != glow_eDrawType_Line) {
    XGCValues xgcv;

    XGetGCValues(display, get_gc(this, color, 0), GCForeground, &xgcv);

    XChangeGC(display, get_gc(this, gc_type, idx), GCForeground, &xgcv);
  }

  if (!w->draw_buffer_only)
    XDrawString(
        display, w->window, get_gc(this, gc_type, idx), x, y, text, len);
  if (w->double_buffer_on)
    XDrawString(
        display, w->buffer, get_gc(this, gc_type, idx), x, y, text, len);
  if (w->clip_on)
  ...
  if (color != glow_eDrawType_Line) {
    ...
    XGetGCValues(
        display, get_gc(this, glow_eDrawType_Line, 0), GCForeground, &xgcv);
...
int GlowDrawXLib::text_cursor(GlowWind* wind, int x, int y, char* text, int len,
    glow_eDrawType gc_type, glow_eDrawType color, int idx, int highlight,
    int pos, glow_eFont font_idx, double size)
{
  if (ctx->nodraw)
    return 1;
  DrawWindXLib* w = (DrawWindXLib*)wind->window;
  ...
  get_text_extent(
      text, pos, gc_type, idx, font_idx, &width, &height, &descent, size, 0);

  if (w->clip_on)
    set_clip(w, get_gc(this, gc_type, idx));

  if (!w->draw_buffer_only) {
    XDrawLine(display, w->window, get_gc(this, color, 1), x + width,
        y + descent, x + width, y - height + descent);
  }
  if (w->double_buffer_on) {
    XDrawLine(display, w->buffer, get_gc(this, color, 1), x + width,
        y + descent, x + width, y - height + descent);
  }
  if (w->clip_on)
    reset_clip(w, get_gc(this, gc_type, idx));
...
int GlowDrawXLib::pixmaps_create(
    GlowWind* wind, glow_sPixmapData* pixmap_data, void** pixmaps)
{
  DrawWindXLib* w = (DrawWindXLib*)wind->window;
  draw_sPixmap* pms;
  glow_sPixmapDataElem *prev_pdata,
      *pdata = (glow_sPixmapDataElem *)pixmap_data;
  int i;

  pms = (draw_sPixmap*)calloc(1, sizeof(*pms));
  for (i = 0; i < DRAW_PIXMAP_SIZE; i++) {
    if (i == 0 || (i > 0 && pdata->bits != prev_pdata->bits))
      pms->pixmap[i] = XCreateBitmapFromData(
          display, w->window, pdata->bits, pdata->width, pdata->height);
    else
...
int GlowDrawXLib::pixmap(GlowWind* wind, int x, int y,
    ...
    int idx, int highlight, int line)
{
  ...
  if (!w->draw_buffer_only)
    XCopyPlane(display, pms->pixmap[idx], w->window, get_gc(this, gc_type, idx),
        0, 0, pdata->width, pdata->height, x, y, 1);
  if (w->double_buffer_on)
    XCopyPlane(display, pms->pixmap[idx], w->buffer, get_gc(this, gc_type, idx),
        0, 0, pdata->width, pdata->height, x, y, 1);
...
int GlowDrawXLib::pixmap_inverse(GlowWind* wind, int x, int y,
    glow_sPixmapData* pixmap_data, void* pixmaps, glow_eDrawType gc_type,
    int idx, int line)
{
  DrawWindXLib* w = (DrawWindXLib*)wind->window;
  ...
  if (!w->draw_buffer_only)
    XCopyPlane(display, pms->pixmap[idx], w->window, gc_inverse, 0, 0,
        pdata->width, pdata->height, x, y, 1);
  if (w->double_buffer_on)
    XCopyPlane(display, pms->pixmap[idx], w->buffer, gc_inverse, 0, 0,
        pdata->width, pdata->height, x, y, 1);
  if (w->clip_on)
...
int GlowDrawXLib::pixmap_erase(GlowWind* wind, int x, int y,
    glow_sPixmapData* pixmap_data, void* pixmaps, glow_eDrawType gc_type,
    int idx, int line)
{
  DrawWindXLib* w = (DrawWindXLib*)wind->window;
  ...
  if (!w->draw_buffer_only)
    XClearArea(display, w->window, x, y, pdata->width, pdata->height, False);
  if (w->double_buffer_on)
    XClearArea(display, w->buffer, x, y, pdata->width, pdata->height, False);
  return 1;
}
    ...
    glow_tImImage image, glow_tPixmap pixmap, glow_tPixmap clip_mask)
{
  ...
  if (!w->draw_buffer_only)
    XCopyArea(display, (Pixmap)pixmap, w->window,
        get_gc(this, glow_eDrawType_Line, 0), 0, 0, width, height, x, y);
  if (w->double_buffer_on)
    ...
    GlowWind* wind, int x, int y, int w, int h, glow_eDrawType gc_type)
{
  ...
  if (!ww->draw_buffer_only)
    XFillPolygon(display, ww->window, get_gc(this, gc_type, 0), p, 5, Convex,
        CoordModeOrigin);
  if (ww->double_buffer_on)
    XFillPolygon(display, ww->buffer, get_gc(this, gc_type, 0), p, 5, Convex,
        CoordModeOrigin);
  if (ww->clip_on)
    ...
    GlowWind* wind, int ll_x, int ll_y, int ur_x, int ur_y)
{
  ...
  XCopyArea(display, w->buffer, w->window, get_gc(this, glow_eDrawType_Line, 0),
...
void GlowDrawXLib::set_window_size(GlowWind* wind, int width, int height)
{
  ...
  XConfigureWindow(display, w->window, CWWidth | CWHeight, &xwc);
}
...
static void event_timer(GlowCtx* ctx, int time_ms)
{
  ...
  //  printf( "Add timer\n");
  draw_ctx->timer_id
      = XtAppAddTimeOut(XtWidgetToApplicationContext(draw_ctx->m_wind.toplevel),
          time_ms, (XtTimerCallbackProc)event_timer_cb, ctx);
}

void GlowDrawXLib::set_timer(
    GlowCtx* gctx, int time_ms, void (*callback_func)(GlowCtx* ctx), void** id)
{
  ...
  timer_cb->timer_id
      = XtAppAddTimeOut(XtWidgetToApplicationContext(m_wind.toplevel), time_ms,
...
void GlowDrawXLib::copy_area(GlowWind* wind, int x, int y)
{
  ...
  if (x >= 0 && y >= 0) {
    XCopyArea(display, w->window, w->window, gc, 0, 0, window_width - x,
        window_height - y, x, y);
    if (!w->double_buffer_on) {
      if (x)
        XClearArea(display, w->window, 0, 0, x, window_height, 0);
      if (y)
        XClearArea(display, w->window, x, 0, window_width, y, 0);
    } else {
      XCopyArea(display, w->buffer, w->buffer, gc, 0, 0, window_width - x,
          window_height - y, x, y);
      if (x)
        XFillRectangle(display, w->buffer,
            get_gc(this, glow_eDrawType_LineErase, 0), 0, 0, x, window_height);
      if (y)
        XFillRectangle(display, w->buffer,
            get_gc(this, glow_eDrawType_LineErase, 0), x, 0, window_width, y);
    }
  } else if (x <= 0 && y <= 0) {
    XCopyArea(display, w->window, w->window, gc, -x, -y, window_width + x,
        window_height + y, 0, 0);
    if (!w->double_buffer_on) {
      if (x)
        XClearArea(display, w->window, window_width + x, 0, window_width,
            window_height, 0);
      if (y)
        XClearArea(display, w->window, 0, window_height + y, window_width + x,
            window_height, 0);
    } else {
      XCopyArea(display, w->buffer, w->buffer, gc, -x, -y, window_width + x,
          window_height + y, 0, 0);
      if (x)
        XFillRectangle(display, w->buffer,
            get_gc(this, glow_eDrawType_LineErase, 0), window_width + x, 0,
            window_width, window_height);
      if (y)
        XFillRectangle(display, w->buffer,
            get_gc(this, glow_eDrawType_LineErase, 0), 0, window_height + y,
            window_width + x, window_height);
    }
  } else if (x <= 0 && y >= 0) {
    XCopyArea(display, w->window, w->window, gc, -x, 0, window_width + x,
        window_height - y, 0, y);
    if (!w->double_buffer_on) {
      if (x)
        XClearArea(display, w->window, window_width + x, 0, window_width,
            window_height, 0);
      if (y)
        XClearArea(display, w->window, 0, 0, window_width + x, y, 0);
    } else {
      XCopyArea(display, w->buffer, w->buffer, gc, -x, 0, window_width + x,
          window_height - y, 0, y);
      if (x)
        XFillRectangle(display, w->buffer,
            get_gc(this, glow_eDrawType_LineErase, 0), window_width + x, 0,
      ...
      if (y)
        XFillRectangle(display, w->buffer,
            get_gc(this, glow_eDrawType_LineErase, 0), 0, 0, window_width + x,
            y);
    }
  } else {
    XCopyArea(display, w->window, w->window, gc, 0, -y, window_width - x,
        window_height + y, x, 0);
    if (!w->double_buffer_on) {
      if (x)
        XClearArea(display, w->window, 0, 0, x, window_height, 0);
      if (y)
        XClearArea(display, w->window, x, window_height + y, window_width,
            window_height, 0);
    } else {
      XCopyArea(display, w->buffer, w->buffer, gc, 0, -y, window_width - x,
          window_height + y, x, 0);
      if (x)
        XFillRectangle(display, w->buffer,
            get_gc(this, glow_eDrawType_LineErase, 0), 0, 0, x, window_height);
      if (y)
        XFillRectangle(display, w->buffer,
            get_gc(this, glow_eDrawType_LineErase, 0), x, window_height + y,
            window_width, window_height);
    }
  }
}

void GlowDrawXLib::clear_area(
    GlowWind* wind, int ll_x, int ur_x, int ll_y, int ur_y)
{
  ...
  XClearArea(display, w->window, ll_x, ll_y, ur_x - ll_x, ur_y - ll_y, 0);
}

void GlowDrawXLib::set_inputfocus(GlowWind* wind)
{
  DrawWindXLib* w = (DrawWindXLib*)wind->window;
  XSetInputFocus(display, w->window, RevertToNone, CurrentTime);
}
    ...
    int pixmap_height)
{
  ...
  sts = XGetGCValues(display, get_gc(this, drawtype, 0), GCForeground, &xgcv);
  if (!sts) {
...
void GlowDrawXLib::set_clip(DrawWind* wind, GC gc)
{
  ...
  XSetClipRectangles(
      display, gc, 0, 0, &w->clip_rectangle[w->clip_cnt - 1], 1, Unsorted);
}
...
{
  XSetClipMask(display, get_gc(this, glow_eDrawType_Line, 0), (Pixmap)pixmap);
  XSetClipOrigin(display, get_gc(this, glow_eDrawType_Line, 0), x, y);
}

void GlowDrawXLib::reset_image_clip_mask()
{
  XSetClipMask(display, get_gc(this, glow_eDrawType_Line, 0), None);
  XSetClipOrigin(display, get_gc(this, glow_eDrawType_Line, 0), 0, 0);
}

int GlowDrawXLib::set_clip_rectangle(
    GlowWind* wind, int ll_x, int ll_y, int ur_x, int ur_y)
{
  DrawWindXLib* w = (DrawWindXLib*)wind->window;
    ...
    GlowWind* wind, int x1, int y1, glow_eDrawType gc_type)
{
  ...
  XDrawPoint(display, w->window, get_gc(this, gc_type, 0), x1, y1);
  if (w->double_buffer_on)
    XDrawPoint(display, w->buffer, get_gc(this, gc_type, 0), x1, y1);
...
int GlowDrawXLib::draw_points(
    GlowWind* wind, glow_sPointX* points, int point_num, glow_eDrawType gc_type)
{
  if (ctx->nodraw)
    return 1;
  DrawWindXLib* w = (DrawWindXLib*)wind->window;
  ...
  if (!w->draw_buffer_only)
    XDrawPoints(display, w->window, get_gc(this, gc_type, 0), (XPoint*)points,
        point_num, CoordModeOrigin);
  if (w->double_buffer_on)
    XDrawPoints(display, w->buffer, get_gc(this, gc_type, 0), (XPoint*)points,
        point_num, CoordModeOrigin);
...
void GlowDrawXLib::load_font(glow_eFont font_idx, int font_type, int idx)
{
  if (!font[font_idx][font_type][idx]) {
    ...
    font_struct[font_idx][font_type][idx] = XQueryFont(display, fnt);
  }
}
...
int GlowDrawXLib::create_buffer(GlowWind* wind)
{
  ...
  w->buffer
      = XCreatePixmap(display, w->window, window_width, window_height, depth);
...
void GlowDrawXLib::buffer_background(DrawWind* wind, GlowCtx* cctx)
{
  ...
  if (w->background_pixmap) {
    ...
    if (!((GrowCtx*)ctx)->background_tiled)
      XCopyArea(display, w->background_pixmap, w->buffer,
          get_gc(this, glow_eDrawType_Line, 0), 0, 0, w->buffer_width,
          w->buffer_height, 0, 0);
    else {
      int i, j;
      for (i = 0; i <= w->buffer_width / w->background_pixmap_width; i++)
        for (j = 0; j <= w->buffer_height / w->background_pixmap_height; j++)
          XCopyArea(display, w->background_pixmap, w->buffer,
              get_gc(this, glow_eDrawType_Line, 0), 0, 0,
    ...
    XFillRectangle(display, w->buffer,
        get_gc(this, glow_eDrawType_LineErase, 0), subwindow_x, subwindow_y,
        window_width, window_height);
    if (w->clip_on)
...
#define ps_cTopMargin 100
  ...
  if (w->double_buffer_on)
    image = Imlib_create_image_from_drawable((ImlibData*)imlib, w->buffer, 0, 0,
        0, w->buffer_width, w->buffer_height);
  else
    image = Imlib_create_image_from_drawable(
        (ImlibData*)imlib, w->window, 0, 0, 0, window_width, window_height);
  if (!image)
    return 0;
  ...
  for (i = 0; i < image->rgb_height * image->rgb_width * 3; i += 3) {
    if (!colorimage) {
      if (*rgb == transp[0] && *(rgb + 1) == transp[1]
          && *(rgb + 2) == transp[2]) {
        grey = 255;
      } else {
...
void GlowDrawXLib::image_rotate(
...
int GlowDrawXLib::image_scale(int width, int height, glow_tImImage orig_im,
...
int GlowDrawXLib::image_load(char* imagefile, glow_tImImage* orig_im,
...
int GlowDrawXLib::image_render(int width, int height, glow_tImImage orig_im,
    glow_tImImage* im, glow_tPixmap* im_pixmap, glow_tPixmap* im_mask)
{
  ...
  *im_pixmap
      = (glow_tPixmap)Imlib_move_image((ImlibData*)imlib, (ImlibImage*)*im);
  if (im_mask)
    *im_mask
        = (glow_tPixmap)Imlib_move_mask((ImlibData*)imlib, (ImlibImage*)*im);
...
void GlowDrawXLib::image_pixel_iter(glow_tImImage orig_image,
