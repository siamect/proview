...
static void Redisplay(Widget w, XEvent* event, Region region)
{
  ((GlowDrawXLib*)((GrowCtx*)((GrowWidget)w)->grow.grow_ctx)->gdraw)
      ->event_handler(*event);
}
...
static Boolean AcceptFocus(Widget w, Time* t)
{
  if (((GrowWidget)w)->grow.is_navigator)
    return 0;
  ((GrowCtx*)((GrowWidget)w)->grow.grow_ctx)
      ->gdraw->set_inputfocus(&((GrowCtx*)((GrowWidget)w)->grow.grow_ctx)->mw);
  return 1;
}

static void Realize(Widget w, unsigned long* mask, XSetWindowAttributes* swa)
{
  (*growWidgetClass->core_class.superclass->core_class.realize)(w, mask, swa);
  ...
  if (((GrowWidget)w)->grow.is_navigator) {
    ...
    if (!((GrowWidget)w)->grow.grow_ctx) {
      ((GrowWidget)w)->grow.draw_ctx
          = new GlowDrawXLib(w, &((GrowWidget)w)->grow.grow_ctx, grow_init_proc,
              ((GrowWidget)w)->grow.client_data, glow_eCtxType_Grow);
    }
  }
}

extern "C" Widget GrowCreate(Widget parent, char* name, ArgList args,
...
extern "C" Widget ScrolledGrowCreate(Widget parent, char* name, ArgList args,
    int argCount, int (*init_proc)(GlowCtx* ctx, void* client_data),
    void* client_data, Widget* grow_w)
{
  ...
  */
  grow = (GrowWidget)GrowCreate(
      form, (char*)"grow", arg, i, init_proc, client_data);
...
Widget GrowCreateNav(
    Widget parent, char* name, ArgList args, int argCount, Widget main_grow)
{
  GrowWidget w;
