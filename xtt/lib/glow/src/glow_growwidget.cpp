/* 
 * Proview   $Id: glow_growwidget.cpp,v 1.2 2005-09-01 14:57:54 claes Exp $
...
static void Redisplay( Widget w, XEvent *event, Region region)
{
  ...
  draw_event_handler( (GrowCtx *)((GrowWidget)w)->grow.grow_ctx, *event);
}
...
static void Destroy( Widget w)
{
  if ( ((GrowWidget) w)->grow.is_navigator)
    return;
  glow_draw_delete( (GlowCtx *)((GrowWidget)w)->grow.grow_ctx);
}
...
static Boolean AcceptFocus( Widget w, Time *t)
{
  if ( ((GrowWidget) w)->grow.is_navigator)
    return 0;
  glow_set_inputfocus( (GlowCtx *)((GrowWidget)w)->grow.grow_ctx);
  return 1;
}
...
static void Realize( Widget w, unsigned long *mask, XSetWindowAttributes *swa)
{
  ...
  if ( ((GrowWidget) w)->grow.is_navigator)
  {
    if ( !((GrowWidget) w)->grow.grow_ctx)
    {
      ...
      glow_draw_init_nav( w, main_grow->grow.grow_ctx);
    }
  }
  else
  {
    if ( !((GrowWidget) w)->grow.grow_ctx)
    {
      glow_draw_init( w, &((GrowWidget) w)->grow.grow_ctx, 
		grow_init_proc, 
