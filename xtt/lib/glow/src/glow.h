...
typedef enum {
  ...
  glow_eType_ToneOrColor, //!< Type is a tone or, if the value is greater then
  //! glow_eDrawTone__, a drawtype
...
typedef enum {
  ...
  //! select rectangle
  glow_eSelectPolicy_Partial, //!< If only a part of the object is inside the
  //! select rectangle, it will be selected
  glow_eSelectPolicy_Both //!< the policy will be Partial when the select
  //! rectangle is dragged from right to left, and
  //! Surround when dragged from left to right
...
typedef enum {
  ...
  glow_eMB3Action_PopupMenu, //!< Display a popup menu, when this is defined for
  //! the object
  glow_eMB3Action_Both //!< Close the window, if the click doesn't hit an
  //! object, otherwise it will display the popmenu for the
  //! object.
} glow_eMB3Action;
...
/*! The Up and Down direction might cause some confusion, because Up is
...
typedef enum {
  ...
  glow_eAlignDirection_Right, //!< Align the right side of the objects
  //! vertically
  ...
  glow_eAlignDirection_Down, //!< Align the lower side of the object
  //! horizontally
  glow_eAlignDirection_CenterVert, //!< Align the center position of the object
  //! vertically
  glow_eAlignDirection_CenterHoriz //!< Align the center position of the objects
  //! horizontally
...
typedef enum { glow_eConGroup_Common, glow_eConGroup_Trace } glow_eConGroup;
...
typedef enum {
  ...
  glow_eMoveRestriction_VerticalSlider, //!< Restrictions used for a vertical
  //! slider
  glow_eMoveRestriction_HorizontalSlider //!< Restrictions user for a horizontal
  //! slider
...
typedef enum {
  ...
  glow_eConType_Routed, //!< A routed connection. The breakpoints are calculated
  //! dependent on the position other objects.
...
typedef enum {
  glow_eRelief_Up, //!< The shadow is lighter on the left-upper side, and darker
  //! on the right-lower side
  glow_eRelief_Down //!< The shadow is darker on the left-upper side, and
  //! lighter on the left-lower side
} glow_eRelief;
...
/*! The drawtype is index in an array that contains the gc for colors in the
 color palette.
 The 300 first are the colors in the color palette, the seven last are used for
 erase, and texts. */
typedef enum {
  glow_eDrawType_Line,
...
//! Draw tone is a set of colors with the same color tone, displayed on the same
//! row in the color palette.
/*! The first nine are pure color tones, the following are shifted to lighter,
  darker,
...
//! Type of event
/*! Glow has a number of event that is enabled with the enble_event() function.
  With this
  function you enable an event type and connect it to a specific mouse klick
  action.
  The event has data structure glow_sEventAny as default, but some of the event
  types
  has extended data structures.
*/
typedef enum {
  glow_eEventType_CallBack, //!< An ordinary callback. Event structure is
  //! glow_sEventAny
  glow_eEventType_RegionSelect, //!< Region selected callback. Event structure
  //! is glow_sEventAny
  glow_eEventType_RegionAddSelect, //!< Region add selected callback. Event
  //! structure is  glow_sEventAny
  glow_eEventType_CreateCon, //!< Create connection callback. Event structure is
  //! glow_sEventConCreate
  glow_eEventType_CreateNode, //!< Create node callback. Event structure is
  //! glow_sEventAny
  glow_eEventType_MoveNode, //!< Node moved callback. Event structure is
  //! glow_sEventAny
  glow_eEventType_Object, //!< Callback with a specified object. Event structure
  //! is glow_sEventObject
  glow_eEventType_AnnotationInput, //!< Annotation input callback. Event
  //! structure is glow_sEventAnnotInput
  glow_eEventType_Radiobutton, //!< Radiobutton clicked callback. Event
  //! structure is glow_sEventRadiobutton
  glow_eEventType_CreateGrowObject, //!< Create grow object callback. Event
  //! structure is glow_sEventCreateGrowObject
  glow_eEventType_Dynamics, //!< Execute dynamics callback. Event structure is
  //! glow_sEventDynamics

  glow_eEventType_ColorTone,
  glow_eEventType_Translate, //!< Translation callback. Event structure is
  //! glow_sEventTranslate
...
//! Conversion version
typedef enum { glow_eConvert_V34 } glow_eConvert;
...
//! Data structure for color tone callback, when a color tone is selected in the
//! color palette.
typedef struct {
  glow_eEvent event; //!< Event
  glow_eEventType type; //!< Event type
...
/*! If translate on is set before opening a .pwg file, all annotations and text
  objects
  will cause en translation callback, and the text will be replaced by the
  translated text.
*/
typedef struct {
  glow_eEvent event; //!< Event
...
//! Data structure for custom color callback, when a color color is modified in
//! the color palette.
typedef struct {
  glow_eEvent event; //!< Event
  glow_eEventType type; //!< Event type
...
typedef struct {
  ...
  int x; //!< x-coordinate for shadow polygon for the corresponding polyline
  //! point
  int y; //!< y-coordinate for shadow polygon for the corresponding polyline
  //! point
} glow_sShadowInfo;
