...
/*! A polyline object for drawing a polyline with specified bordercolor,
...
public:
  ...
  */
  GrowPolyLine(GrowCtx* glow_ctx, const char* name, glow_sPoint* pointarray,
      int point_cnt, glow_eDrawType border_d_type = glow_eDrawType_Line,
      int line_w = 1, int fix_line_w = 0, int fill = 0, int display_border = 1,
      int display_shadow = 0, glow_eDrawType fill_d_type = glow_eDrawType_Line,
      int closed = 0, int nodraw = 0, int noround = 0,
      glow_eCurveType ctype = glow_eCurveType_Line);
  ...
  */
  ...
  {
    return glow_eObjectType_GrowPolyLine;
  }
  ...
  */
  ...
  //! Set border.
  /*!
    \param borderval	If 1 the object will be draw with border, if 0 the
    object will be drawn without border.
  */
  ...
  */
  ...
  /*! The shadow is drawn on the inside of the polyline. The sum of the inside
    angles is always 180 degrees
    less the the angles on the outside. The angles is added to decide which side
    is the inside.
  */
  ...
  /*!
    \param s		Retured buffer that contains the coordinates and colors
    of
    the shadow. This buffer should be freed.
    \param num		Number of points in s.
    \param ish		The shadow width.
    \param highlight	The object is drawn width highlight colors.
    \param colornode	Node that determines the colors of the polyline. Can be
    zero.
    \param exportjava	Called from export java function.

    Calculates a shadow point for each point in the polyline. The shadow point
    is the crossing point for two lines
    parallell to the lines of the polyline, but with a distance ish from them.
    The color of the shadow segment is
    calculated from the direction of the line.
  */

  void calculate_shadow(glow_sShadowInfo** s, int* num, int ish, int highlight,
      void* colornode, int exportjava, int chot);
  ...
  */
  ...
  //! Add a new y-value for the first point, and shift the y-values one step
  //! forward.
  /*!
    \param value	New y-value.

    Function to add a new value in a trend curve. All the y-values are shifted
    forward and the new value
  ...
  */
  ...
  //! Add a new y-value for the first point for a filled curve, and shift the
  //! y-values one step forward.
  /*!
    \param value	New y-value.

    Function to add a new value in a filled trend curve. All the y-values are
    shifted forward and the new value
    is inserted in the first point. The x-value are kept intact.
  */
