...
//! Draw tooltip text.
/*! A tooltip text is a text frame that becomes visible some time after the
  cursor has visited an object, it
  the cursor still rest on that object. When the object is hit by the cursor,
  draw_text() should be called.
  The text will be displayed within 1 second if not the remove_text() or
  remove() function are called.
...
public:
  ...
  */
  GlowTipText(GrowCtx* gctx, int tsize)
      : ctx(gctx), text_object(0), active(false), timer_id(0), text_size(tsize)
  {
  }
  ...
  */
  ...
  /*!
    ...
    This function is called when the cursor hits an object. It starts a timer,
    and if
    no remove function is called, the text will be displayed within 1 second,
    and remain until
  ...
  */
  ...
  /*! If the timer is running, it is stopped. If the text is diplayed, it is
   * erased.
   */
  void remove();
