...
GlowCon::GlowCon( GrowCtx *glow_ctx, char *name, GlowConClass *con_class,
	...
	trace_p(NULL), border(cborder), shadow(cshadow)
{
  ...
  switch ( cc->con_type)
  {
    ...
    case glow_eConType_StraightOneArrow:
      ...
      line_a.insert( l1);
      l_num = 1;
      arrow = new GlowArrow( ctx, src_x, src_y, dest_x, dest_y, 
  ...
  if ( temporary_ref || cc->con_type == glow_eConType_Reference)
  {
    ...
    t1 = new GlowText( ctx, reftext, text_x, text_y, 
		glow_eDrawType_TextHelveticaBold, glow_eDrawType_Line, ctx->refcon_textsize);
    ref_a.insert( r1);
    ref_a.insert( t1);
...
void GlowCon::select_region_insert( double ll_x, double ll_y, double ur_x, 
		double ur_y)
{
...
{ 
  ...
  fp << int(glow_eSave_Con) << endl;
  fp << int(glow_eSave_Con_x_right) << FSPACE << x_right << endl;
  ...
  fp << int(glow_eSave_Con_source_conpoint) << FSPACE << source_conpoint << endl;
  fp << int(glow_eSave_Con_dest_direction) << FSPACE << int(dest_direction) << endl;
  fp << int(glow_eSave_Con_source_direction) << FSPACE << int(source_direction) << endl;
  fp << int(glow_eSave_Con_line_a) << endl;
  line_a.save( fp, mode);
  fp << int(glow_eSave_Con_arc_a) << endl;
  arc_a.save( fp, mode);
  fp << int(glow_eSave_Con_arrow_a) << endl;
  arrow_a.save( fp, mode);
  fp << int(glow_eSave_Con_ref_a) << endl;
  ref_a.save( fp, mode);
  fp << int(glow_eSave_Con_p_num) << FSPACE << p_num << endl;
  fp << int(glow_eSave_Con_l_num) << FSPACE << l_num << endl;
  fp << int(glow_eSave_Con_a_num) << FSPACE << a_num << endl;
  fp << int(glow_eSave_Con_arrow_num) << FSPACE << arrow_num << endl;
  fp << int(glow_eSave_Con_ref_num) << FSPACE << ref_num << endl;
  fp << int(glow_eSave_Con_point_x) << endl;
  for ( i = 0; i < p_num; i++)
    fp << point_x[i] << endl;
  fp << int(glow_eSave_Con_point_y) << endl;
  for ( i = 0; i < p_num; i++)
    fp << point_y[i] << endl;
  ...
  fp << int(glow_eSave_Con_temporary_ref) << FSPACE << temporary_ref << endl;
  fp << int(glow_eSave_Con_border) << FSPACE << border << endl; 
  fp << int(glow_eSave_Con_shadow) << FSPACE << shadow << endl;
  fp << int(glow_eSave_End) << endl;  
}

void GlowCon::open( ifstream& fp)
{
  int		type;
  int 		end_found = 0;
  char		dummy[40];
  ...
  for (;;)
  {
    ...
    switch( type) {
      ...
      case glow_eSave_Con_y_high: fp >> y_high; break;
      case glow_eSave_Con_y_low: fp >> y_low; break;
      case glow_eSave_Con_dest_node:
        fp.get();
        fp.getline( name, sizeof(name));
        dest_node = (GlowNode *) ctx->get_node_from_name( name);
        if ( !dest_node)
          cout << "GlowCon:dest_node not found" << endl;
        break;
      case glow_eSave_Con_source_node:
        ...
        if ( !source_node)
          cout << "GlowCon:source_node not found" << endl;
        break;
      case glow_eSave_Con_dest_conpoint: fp >> dest_conpoint; break;
      case glow_eSave_Con_source_conpoint: fp >> source_conpoint; break;
      case glow_eSave_Con_dest_direction: fp >> tmp; dest_direction = (glow_eDirection)tmp; break;
      case glow_eSave_Con_source_direction: fp >> tmp; source_direction = (glow_eDirection)tmp; break;
      case glow_eSave_Con_line_a: line_a.open( ctx, fp); break;
      case glow_eSave_Con_arc_a: arc_a.open( ctx, fp); break;
      case glow_eSave_Con_arrow_a: arrow_a.open( ctx, fp); break;
      case glow_eSave_Con_ref_a: ref_a.open( ctx, fp); break;
      case glow_eSave_Con_p_num: fp >> p_num; break;
      case glow_eSave_Con_l_num: fp >> l_num; break;
      case glow_eSave_Con_a_num: fp >> a_num; break;
      case glow_eSave_Con_arrow_num: fp >> arrow_num; break;
      case glow_eSave_Con_ref_num: fp >> ref_num; break;
      ...
      case glow_eSave_Con_point_y:
        for ( i = 0; i < p_num; i++)
          fp >> point_y[i];
        break;
      case glow_eSave_Con_source_ref_cnt: fp >> source_ref_cnt; break;
      case glow_eSave_Con_dest_ref_cnt: fp >> dest_ref_cnt; break;
      ...
      case glow_eSave_Con_trace_attribute:
        ...
        break;
      case glow_eSave_Con_trace_attr_type: fp >> tmp; trace_attr_type = (glow_eTraceType)tmp; break;
      case glow_eSave_Con_temporary_ref: fp >> temporary_ref; break;
      case glow_eSave_Con_border: fp >> border; break;
      case glow_eSave_Con_shadow: fp >> shadow; break;
      case glow_eSave_End: end_found = 1; break;
      default:
        cout << "GlowCon:open syntax error" << endl;
...
int GlowCon::con_route_grafcet( glow_eConType con_type, 
...
int GlowCon::con_route_area( double wind_ll_x, double wind_ll_y, 
	double wind_ur_x, double wind_ur_y)
{
  ...
  if ( fabs( dest_y - src_y) < CON_EPSILON && 
	  ...
	  dest_x <= src_x) ||
        (dest_dir == glow_eDirection_Left && src_dir == glow_eDirection_Right &&
	  dest_x >= src_x)))
  {
    found = 0;
    ...
    if ( !found)
    {
	  ...
	  dest_y <= src_y) ||
        (dest_dir == glow_eDirection_Down && src_dir == glow_eDirection_Up &&
	  dest_y >= src_y)))
  {
    found = 0;
	...
	double  src_x, double src_y, glow_eDirection src_dir)
{
  ...
  sort_dest = dest_node;
  if ( dest_dir == glow_eDirection_Right && 
       src_dir == glow_eDirection_Left &&
       src_x > dest_x)
  {
...
int GlowCon::find_horiz_line_up( double check_y, double check_l_x,
	...
	GlowCon *conlist, GlowCon *next_con,
...
void GlowCon::draw_routed( int points, double *x, double *y)
{
  ...
  for ( i = 0; i < points - 1; i++)
  {
    ...
    if ( i < l_num)
      ...
      l->move_noerase( &cc->zero, x[i], y[i],
		x[i+1], y[i+1], highlight, hot);
  }
...
void GlowCon::draw_routed_roundcorner( int points, double *x, double *y)
{
  ...
  for ( i = 0; i < points - 1; i++)
  {
    ...
    if ( i < l_num)
      ...
      l->move_noerase( &cc->zero, line_x1[i], line_y1[i],
		line_x2[i], line_y2[i], highlight, hot);
  }
	...
	glow_eExportPass pass, int *shape_cnt, int node_cnt, int in_nc, ofstream &fp)
{
  ...
  {
    for ( i = 0; i < l_num; i++)
     ((GlowLine *)line_a[i])->export_javabean( t, node, pass, shape_cnt,
		node_cnt, in_nc, fp);
    for ( i = 0; i < a_num; i++)
      ((GlowArc *)arc_a[i])->export_javabean( t, node, pass, shape_cnt,
		node_cnt, in_nc, fp);
    if ( (shadow || border) && cc->con_type == glow_eConType_Routed && cc->corner == glow_eCorner_Rounded) {    
      for ( i = 0; i < l_num; i++)
