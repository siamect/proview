...
public:
  DrawWindQt() : window(0), buffer(0), clip_on(0), clip_cnt(0),
                 background_pixmap(0), background_pixmap_width(0),
                 background_pixmap_height(0)
  {
...
public:
  ...
  virtual int arrow(GlowWind *w, int x1, int y1, int x2, int y2, int x3, int y3,
                    glow_eDrawType painter_type, int idx, int highlight);
  virtual int arrow_erase(GlowWind *w, int x1, int y1, int x2, int y2, int x3,
                          int y3, int idx);
  virtual int arc(GlowWind *w, int x, int y, int width, int height, int angle1,
                  int angle2, glow_eDrawType painter_type, int idx,
                  int highlight);
  virtual int fill_arc(GlowWind *w, int x, int y, int width, int height,
                       int angle1, int angle2, glow_eDrawType painter_type,
                       int highlight);
  ...
  virtual int text(GlowWind *w, int x, int y, char *text, int len,
                   glow_eDrawType painter_type, glow_eDrawType color, int idx,
                   int highlight, int line, glow_eFont font_idx, double size,
                   int rot);
  virtual int text_cursor(GlowWind *w, int x, int y, char *text, int len,
                          glow_eDrawType painter_type, glow_eDrawType color,
                          int idx, int highlight, int pos, glow_eFont font,
                          double size);
  virtual int text_erase(GlowWind *w, int x, int y, char *text, int len,
                         glow_eDrawType painter_type, int idx, int line,
  ...
  virtual int get_text_extent(const char *text, int len,
                              ...
                              glow_eFont font_idx, int *width, int *height,
                              int *descent, double size, int rot);
  ...
  virtual void set_background(GlowWind *w, glow_eDrawType drawtype,
                              glow_tPixmap pixmap, glow_tImImage image,
                              int pixmap_width, int pixmap_height);
  virtual void reset_background(GlowWind *w);
  virtual void set_image_clip_mask(QPainter *painter, glow_tPixmap pixmap,
                                   int x, int y);
  ...
  virtual int set_clip_rectangle(GlowWind *w, int ll_x, int ll_y, int ur_x,
                                 int ur_y);
  virtual void reset_clip_rectangle(GlowWind *w);
  virtual int clip_level(GlowWind *w);
  virtual int draw_point(GlowWind *w, int x1, int y1,
                         glow_eDrawType painter_type);
  ...
  QPoint *points_to_qt_points_curve(GlowWind *w, glow_sPointX *points,
  ...
  int image_render(int width, int height, glow_tImImage orig_im,
                   glow_tImImage *im, glow_tPixmap *im_pixmap,
  ...
  void image_pixel_iter(glow_tImImage orig_image, glow_tImImage *image,
                        void (*pixel_cb)(void *, unsigned char *),
                        void *userdata);
  ...
  int get_text_extent_qt(const char *text, int len, glow_eDrawType painter_type,
                         int idx, glow_eFont font_idx, int *width, int *height,
                         int *descent, double size, int rot);
