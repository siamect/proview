...
static gboolean scroll_callback_cb(void* d)
{
  ...
  if (scroll_data->scroll_v_managed) {
    ...
    if (data->window_height
            != ((CurveWidgetGtk*)scroll_data->curve)->scroll_v_pagesize
        || data->total_height
            != ((CurveWidgetGtk*)scroll_data->curve)->scroll_v_upper
    ...
    ((CurveWidgetGtk*)scroll_data->curve)->scroll_v_value
        = (gdouble)data->offset_y;
    ((CurveWidgetGtk*)scroll_data->curve)->scroll_h_pagesize
        = data->window_width;
...
static gboolean curvewidgetgtk_expose(GtkWidget* glow, GdkEventExpose* event)
{
  ...
  ((GlowDrawGtk*)((CurveCtx*)((CurveWidgetGtk*)glow)->curve_ctx)->gdraw)
...
static void curvewidgetgtk_destroy(GtkObject* object)
{
  ...
  if (!curvew->destroyed) {
    ...
    if (curvew->is_navigator) {
      if (curvew->curve_ctx
          && !((CurveWidgetGtk*)curvew->main_curve_widget)->destroyed)
...
static void curvewidgetgtk_realize(GtkWidget* widget)
{
  ...
  if (curve->is_navigator) {
    ...
    if (!curve->curve_ctx) {
      curve->draw_ctx = new GlowDrawGtk(widget, &curve->curve_ctx,
          curve_init_proc, curve->client_data, glow_eCtxType_Curve);
    }
