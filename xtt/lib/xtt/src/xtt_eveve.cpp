...
EvEve::EvEve(void* ev_parent_ctx, char* eve_name, pwr_tObjid ev_user,
    ...
    : parent_ctx(ev_parent_ctx), user(ev_user), eventname_seg(ev_eventname_seg),
      ...
      ala(NULL), eve_displayed(0), list_copied(0)
{
}
    ...
    unsigned long item_type, unsigned long utility, char* arg, int x, int y)
{
  if (((EvEve*)ctx)->popup_menu_cb)
    (((EvEve*)ctx)->popup_menu_cb)(
        ((EvEve*)ctx)->parent_ctx, attrref, item_type, utility, arg, x, y);
}
...
int EvEve::get_last_not_acked_prio(
