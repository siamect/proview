...
XttAudio::XttAudio(
    CoWow* a_wow, const char* a_OSS_device, const char* a_ALSA_device)
    ...
    while (1) {
      ...
      case pwr_cClass_SoundSequence: {
        ...
        for (j = 0; j < 3; j++) {
          if (cdh_ObjidIsNotNull(seq.SoundObject[j].Objid)) {
            sts = gdh_GetObjectInfoAttrref(
                &seq.SoundObject[j], &sound[j], sizeof(sound[j]));
            if (EVEN(sts))
              break;
          } else
            break;
        ...
        for (int i = 0;
          ...
          for (int j = 0; j < int(sizeof(sound[0].ToneTable)
            ...
            if (sound[seq.SequenceTable[i].SoundIdx].VolumeTable[j] > 0) {
              if (str_StartsWith(
                ...
                MakeSine(buffer, size, 0, seq.SequenceTable[i].StartTime,
                    ...
                    seq.SequenceTable[i].VolumeRight / 100
                        * sound[seq.SequenceTable[i].SoundIdx].VolumeTable[j],
                    seq.SequenceTable[i].VolumeLeft / 100
                        * sound[seq.SequenceTable[i].SoundIdx].VolumeTable[j],
...
void XttAudio::audio_write(void* data)
{
  ...
  if (audio->OSS_audio_ok) {
    ...
    if (audio->write_buffer_asize
        && audio->write_buffer_asize < audio->write_buffer_size) {
...
void XttAudio::audio_write_buff(void* a)
{
  ...
  rc = snd_pcm_writei(audio->ALSA_handle,
      &audio->write_buffer[audio->write_buffer_idx], size / 2);

  if (debug)
    printf(
        "Write idx: %6d totsize: %6d size: %6d written: %6d actsize: %4.2f\n",
        audio->write_buffer_idx, audio->write_buffer_size, size, rc * 2,
        ((float)time) / 1000);
  ...
  if ((audio->write_buffer_asize
          && audio->write_buffer_idx < audio->write_buffer_asize)
      || (!audio->write_buffer_asize
             && audio->write_buffer_idx < audio->write_buffer_size)) {
    // if ( audio->write_buffer_idx < audio->write_buffer_size) {
    // Submit next write
...
int XttAudio::Init_ALSA(char* device, unsigned int samplerate)
{
  ...
  if ((err = snd_pcm_open(&ALSA_handle, device, SND_PCM_STREAM_PLAYBACK, 0))
      < 0)
    return -1;
  else {
    ...
    if ((err = snd_pcm_hw_params_set_access(
             ALSA_handle, hw_params, SND_PCM_ACCESS_RW_INTERLEAVED))
        < 0) {
    ...
    if ((err = snd_pcm_hw_params_set_format(
             ALSA_handle, hw_params, SND_PCM_FORMAT_S16_LE))
        < 0) {
    ...
    if ((err = snd_pcm_hw_params_set_rate_near(
             ALSA_handle, hw_params, samplerate, 0))
        < 0)
#else
    if ((err = snd_pcm_hw_params_set_rate_near(
             ALSA_handle, hw_params, &samplerate, 0))
    ...
    if ((err = snd_pcm_hw_params_set_period_size_near(
             ALSA_handle, hw_params, period_size, 0))
        < 0)
#else
    if ((err = snd_pcm_hw_params_set_period_size_near(
             ALSA_handle, hw_params, &period_size, 0))
    ...
    err = snd_pcm_hw_params_get_buffer_size_min(
        hw_params, (snd_pcm_uframes_t*)&msize);
    if (err >= 0) {
      ...
      if ((err = snd_pcm_hw_params_set_buffer_size_near(
               ALSA_handle, hw_params, (snd_pcm_uframes_t*)&msize))
          < 0) {
    ...
    double decay, double sustain, double release)
{
  ...
  return (1.0 - (time - endtime) / release)
      * (sustain + (1.0 - sustain) * exp(-(endtime - attack) / decay));
}

void XttAudio::MakeSine(short* buffer, int buffersize, double time,
    double starttime, double endtime, int tone, double volume_ch1,
    double volume_ch2, double attack, double decay, double sustain,
    double release, double tremolo)
{
  ...
  t = starttime;
  for (i = 2 * int(starttime * srate); i < buffersize;
       i += 2, t += 1.0 / srate) {
    if (t < starttime)
      continue;
...
void XttAudio::MakeSquare(short* buffer, int buffersize, double time,
    double starttime, double endtime, int tone, double volume_ch1,
    double volume_ch2, double attack, double decay, double sustain,
    double release, double tremolo)
{
