...
//
XNavMotif::XNavMotif(void* xn_parent_ctx, Widget xn_parent_wid,
    const char* xn_name, Widget* w, xnav_sStartMenu* root_menu,
...
{
  form_widget = ScrolledBrowCreate(parent_wid, name, NULL, 0, init_brow_base_cb,
...
XAtt* XNavMotif::xatt_new(
    pwr_tAttrRef* arp, int advanced_user, pwr_tStatus* sts)
{
  return new XAttMotif(form_widget, this, arp, advanced_user, sts);
}

XCrr* XNavMotif::xcrr_new(
    pwr_tAttrRef* arp, int advanced_user, pwr_tStatus* sts)
{
...
{
  return new EvMotif(this, parent_wid, eve_name, ala_name, blk_name, ev_user,
...
XttTrend* XNavMotif::xtttrend_new(char* name, pwr_tAttrRef* objar,
    pwr_tAttrRef* plotgroup, int width, int height, unsigned int options,
    void* basewidget, pwr_tStatus* sts)
{
  Widget w;

  return new XttTrendMotif(this, parent_wid, name, &w, objar, plotgroup, width,
      height, options, sts);
}

XttFast* XNavMotif::xttfast_new(char* name, pwr_tAttrRef* objar, int width,
    int height, unsigned int options, void* basewidget, pwr_tStatus* sts)
{
  Widget w;

  return new XttFastMotif(
      this, parent_wid, name, &w, objar, width, height, options, sts);
}

XAttOne* XNavMotif::xattone_new(
...
XttGe* XNavMotif::xnav_ge_new(const char* name, const char* filename,
...
{
  return new XttGeMotif(parent_wid, this, name, filename, scrollbar, menu,
      navigator, width, height, x, y, scan_time, object_name,
      use_default_access, access, options, basewidget, scroll_x, scroll_y,
...
{
  return new GeCurveMotif(
      this, parent_wid, name, filename, data, pos_right, 0, 0, options);
}

CoLogin* XNavMotif::login_new(const char* name, const char* groupname,
    void (*bc_success)(void*), void (*bc_cancel)(void*), void* basewidget,
    pwr_tStatus* status)
{
  return new CoLoginMotif(
      this, parent_wid, name, groupname, bc_success, bc_cancel, status);
}
...
void XNavMotif::get_popup_menu(pwr_sAttrRef attrref, xmenu_eItemType item_type,
    xmenu_mUtility caller, unsigned int priv, char* arg, int x, int y)
{
  ...
  if (popup != NULL)
    XtAddCallback(popup, XmNunmapCallback, (XtCallbackProc)popup_unmap_cb, mcp);
...
Widget XNavMotif::build_menu(Widget Parent, int MenuType, const char* MenuTitle,
    void* MenuUserData, void (*Callback)(Widget, XNav*, XmAnyCallbackStruct*),
    void* CallbackData, xmenu_sMenuItem* Items, int* idx)
{
  ...
  for (; Items[*idx].Level != 0 && Items[*idx].Level >= Level; (*idx)++) {
    if (Items[*idx].Item == xmenu_eMenuItem_Cascade
      ...
      if (MenuType == MENU_OPTION) {
        ...
        i = *idx;
        (*idx)++;
        build_menu(Menu, MENU_PULLDOWN, Lng::translate(Items[i].Name),
      ...
      W = XtVaCreateManagedWidget(Lng::translate(Items[*idx].Name), Class, Menu,
          XmNuserData, *idx, XmNsensitive,
          (Boolean)(Items[*idx].Flags.f.Sensitive == 1), NULL);
...
void XNavMotif::popup_unmap_cb(
    Widget w, xmenu_sMenuCall* ip, XmAnyCallbackStruct* data)
{
  // XtFree( (char *)ip);
  XtDestroyWidget(w);
}
