...
void BlockMotif::create_toggleNo(
    Widget w, Block* blk, XmAnyCallbackStruct* data)
{
...
void BlockMotif::activate_cancel(
    Widget w, Block* blk, XmAnyCallbackStruct* data)
{
  delete blk;
}
...
BlockMotif::BlockMotif(void* b_parent_ctx, Widget b_parent_wid,
    ...
    : Block(b_parent_ctx, b_oar, name, priv, sts), parent_wid(b_parent_wid)
{
  ...
  static MrmRegisterArg reglist[] = { { (char*)"blk_ctx", 0 },
  ...
  *sts = gdh_AttrrefToName(&oar, aname, sizeof(aname), cdh_mNName);
  ...
  msts = MrmFetchWidgetOverride(s_DRMh, (char*)"blk_window", parent_wid, name,
  ...
  flow_AddCloseVMProtocolCb(parent_wid, (XtCallbackProc)activate_cancel, this);
}
