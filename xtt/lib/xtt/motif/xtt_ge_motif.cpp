...
void XttGeMotif::disable_set_focus(XttGeMotif* ge, int time)
{
  ge->set_focus_disabled++;
  ge->focus_timerid
      = XtAppAddTimeOut(XtWidgetToApplicationContext(ge->toplevel), time,
...
void XttGeMotif::set_size(int width, int height)
{
  ...
  i = 0;
  XtSetArg(args[i], XmNwidth, default_width);
  i++;
  XtSetArg(args[i], XmNheight, default_height);
  i++;
  ...
  if (!((XNav*)parent_ctx)->gbl.no_graph_ratio) {
    XtSetArg(args[i], XmNminAspectX, x1);
    i++;
    XtSetArg(args[i], XmNminAspectY, y1);
    i++;
    XtSetArg(args[i], XmNmaxAspectX, x2);
    i++;
    XtSetArg(args[i], XmNmaxAspectY, y2);
    i++;
  }
  XtSetValues(toplevel, args, i);
}

void XttGeMotif::ge_change_value_cb(
    void* ge_ctx, void* value_object, char* text)
{
  Arg args[1];
  ...
  // XtSetKeyboardFocus( ((XttGeMotif *)ge)->toplevel, ((XttGeMotif
  // *)ge)->value_input);
  ...
  XmTextSetSelection(
      ((XttGeMotif*)ge)->value_input, 0, strlen(text), CurrentTime);
...
void XttGeMotif::activate_value_input(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
  char* text;
...
void XttGeMotif::activate_confirm_ok(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
  XtUnmanageChild(((XttGeMotif*)ge)->confirm_widget);
  ge->confirm_open = 0;
...
void XttGeMotif::activate_confirm_cancel(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
  ge->confirm_open = 0;
  XtUnmanageChild(((XttGeMotif*)ge)->confirm_widget);
...
void XttGeMotif::activate_zoom_in(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
  ge->graph->zoom(1.2);
}

void XttGeMotif::activate_zoom_out(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
  ge->graph->zoom(5.0 / 6);
}

void XttGeMotif::activate_zoom_reset(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
  ge->graph->unzoom();
}
...
void XttGeMotif::create_graph_form(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
  ((XttGeMotif*)ge)->graph_form = w;
}

void XttGeMotif::create_message_dia(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
...
void XttGeMotif::create_value_input(
    Widget w, XttGe* ge, XmAnyCallbackStruct* data)
{
...
XttGeMotif::XttGeMotif(Widget xg_parent_wid, void* xg_parent_ctx,
    ...
    double* borders, int (*xg_command_cb)(void*, char*, void*),
    ...
    : XttGe(xg_parent_ctx, xg_name, xg_filename, xg_scrollbar, xg_menu,
          ...
          use_default_access, access, options, xg_command_cb,
          xg_get_current_objects_cb, xg_is_authorized_cb),
      parent_wid(xg_parent_wid), set_focus_disabled(0), focus_timerid(0)
{
  ...
  static char translations[] = "<ConfigureNotify>: resize()\n\
<FocusIn>: ge_xtt_inputfocus()";
  ...
  sts = MrmFetchWidgetOverride(s_DRMh, (char*)"ge_window", toplevel, wname,
      args, 1, &ge_widget, &dclass);
  if (sts != MrmSUCCESS)
    printf("can't fetch %s\n", wname);

  sts = MrmFetchWidget(
      s_DRMh, (char*)"input_dialog", toplevel, &value_dialog, &dclass);
  if (sts != MrmSUCCESS)
  ...
  sts = MrmFetchWidget(
      s_DRMh, (char*)"message_dialog", toplevel, &message_dia_widget, &dclass);
  if (sts != MrmSUCCESS)
    printf("can't fetch message dialog\n");
  ...
  if (width == 0 || height == 0) {
    ...
    i++;
    XtSetArg(args[i], XmNminAspectX, width);
    i++;
    XtSetArg(args[i], XmNminAspectY, height);
    i++;
    XtSetArg(args[i], XmNmaxAspectX, width);
    i++;
    XtSetArg(args[i], XmNmaxAspectY, height);
    i++;
  }
  if (!(x == 0 && y == 0)) {
    XtSetArg(args[i], XmNx, x);
    i++;
    XtSetArg(args[i], XmNy, y);
    i++;
  }
  XtSetValues(toplevel, args, i);
  ...
  graph = new GraphMotif(this, graph_form, "Plant", &grow_widget, &sts,
  ...
  if (navigator) {
    ...
    nav_shell = XmCreateDialogShell(grow_widget, (char*)"Navigator", args, i);
