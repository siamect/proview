...
GraphMotif::GraphMotif(void* xn_parent_ctx, Widget xn_parent_wid,
    ...
    unsigned int xn_default_access, unsigned int xn_options)
      ...
      parent_wid(xn_parent_wid)
{
  ...
  if (scrollbar)
    ...
    grow_widget = form_widget
        = GrowCreate(parent_wid, name, NULL, 0, graph_init_grow_base_cb, this);
...
{
  trace_timerid = XtAppAddTimeOut(XtWidgetToApplicationContext(grow_widget),
      time, (XtTimerCallbackProc)Graph::trace_scan, this);
}
...
int GraphMotif::create_navigator(Widget parent)
{
  ...
  i++;
  nav_widget = GrowCreateNav(parent, (char*)"navigator", args, i, grow_widget);
  XtManageChild(nav_widget);

  return 1;
}

Attr* GraphMotif::attr_new(
    void* parent_ctx, void* object, attr_sItem* itemlist, int item_cnt)
{
  Attr* attr
      = new AttrMotif(parent_wid, parent_ctx, object, itemlist, item_cnt);
  return attr;
}
