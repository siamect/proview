...
static void subgraphs_create_sg_form(
    Widget w, SubGraphsMotif* subgraphs, XmAnyCallbackStruct* data)
{
  subgraphs->subgraphs_form = w;
}

static void subgraphs_activate_exit(
    Widget w, SubGraphsMotif* subgraphs, XmAnyCallbackStruct* data)
{
...
static void subgraphs_activate_attr(
    Widget w, SubGraphsMotif* subgraphs, XmAnyCallbackStruct* data)
{
  ItemSubGraph* item;
  int sts;
...
static void subgraphs_activate_set_extern(
    Widget w, SubGraphsMotif* subgraphs, XmAnyCallbackStruct* data)
{
  ItemSubGraph* item;
  int sts;
...
static void subgraphs_activate_set_intern(
    Widget w, SubGraphsMotif* subgraphs, XmAnyCallbackStruct* data)
{
  ItemSubGraph* item;
  int sts;
...
static void subgraphs_activate_help(
    Widget w, SubGraphsMotif* subgraphs, XmAnyCallbackStruct* data)
{
}
...
//
SubGraphsMotif::SubGraphsMotif(void* xn_parent_ctx, Widget xn_parent_wid,
    const char* xn_name, void* xn_growctx, Widget* w, pwr_tStatus* status)
      ...
      parent_wid(xn_parent_wid), trace_timerid(0)
{
  ...
  static MrmRegisterArg reglist[] = { { (char*)"subgraphs_ctx", 0 },
    { (char*)"subgraphs_activate_exit", (caddr_t)subgraphs_activate_exit },
    { (char*)"subgraphs_activate_attr", (caddr_t)subgraphs_activate_attr },
    { (char*)"subgraphs_activate_set_extern",
        (caddr_t)subgraphs_activate_set_extern },
    { (char*)"subgraphs_activate_set_intern",
        (caddr_t)subgraphs_activate_set_intern },
  ...
  i++;
  XtSetArg(args[i], XmNheight, 1200);
  i++;
  ...
  form_widget = ScrolledBrowCreate(subgraphs_form, name, NULL, 0,
  ...
  *w = toplevel;
  ...
  flow_AddCloseVMProtocolCb(
      parent_wid, (XtCallbackProc)subgraphs_activate_exit, this);
...
void SubGraphsMotif::trace_scan(SubGraphsMotif* subgraphs)
{
  ...
  if (subgraphs->trace_started) {
    ...
    ((SubGraphsMotif*)subgraphs)->trace_timerid
        = XtAppAddTimeOut(XtWidgetToApplicationContext(subgraphs->brow_widget),
            time, (XtTimerCallbackProc)SubGraphsMotif::trace_scan, subgraphs);
  }
}
