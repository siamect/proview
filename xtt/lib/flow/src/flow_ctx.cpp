...
FlowCtx::FlowCtx(const char* ctx_name, double zoom_fact, int offs_x, int offs_y)
    : ctx_type(flow_eCtxType_Flow), zoom_factor(zoom_fact),
      ...
      conpoint_select_idx(0), auto_scrolling_active(0), is_dirty(0), a(50, 50),
      a_sel(20, 20), a_paste(20, 20), a_move(20, 20), a_nc(20, 20),
      a_cc(20, 20), event_region_select(flow_eEvent_Null),
...
void FlowCtx::draw(int ll_x, int ll_y, int ur_x, int ur_y)
{
  ...
  if (select_rect_active) {
    fdraw->rect(select_rect_ll_x, select_rect_ll_y,
...
void FlowCtx::paste_execute()
{
  ...
  fdraw->set_cursor(mw, draw_eCursor_DiamondCross);
  nav_zoom();
...
void FlowCtx::nav_zoom()
{
  ...
  if (a.size() > 0) {
    ...
    double x_nav_left = MIN(x_left, offset_x / zoom_factor);
    double x_nav_right = MAX(x_right, (offset_x + window_width) / zoom_factor);
    double y_nav_low = MIN(y_low, offset_y / zoom_factor);
    double y_nav_high = MAX(y_high, (offset_y + window_height) / zoom_factor);
    if (feq(x_nav_right, x_nav_left) || feq(y_nav_high, y_nav_low))
      return;

    double nzf = nav_zoom_factor;
    int nox = nav_offset_x, noy = nav_offset_y;
    ...
    if (!feq(nav_zoom_factor, nzf) || nav_offset_x != nox || nav_offset_y != noy) {
      set_dirty();
    }
  }
}
...
int FlowCtx::event_handler(flow_eEvent event, int x, int y, int w, int h)
{
  ...
  case flow_eEvent_MB1Click:
    ...
    if (node_movement_paste_active) {
      ...
      nav_zoom();
      fdraw->set_cursor(mw, draw_eCursor_Normal);
  ...
  case flow_eEvent_ButtonMotion:
    ...
    if (node_movement_active) {
      ...
      if (con_create_last_x != x || con_create_last_y != y) {
        con_create_last_x = x;
        con_create_last_y = y;
        set_dirty();
      }
  ...
  case flow_eEvent_ButtonRelease:
    ...
    if (node_movement_active) {
      ...
      nav_zoom();
      fdraw->set_cursor(mw, draw_eCursor_CrossHair);
...
int FlowCtx::event_handler_nav(flow_eEvent event, int x, int y)
{
  ...
  case flow_eEvent_CursorMotion:
    if (nav_rect_ll_x < x && x < nav_rect_ur_x && nav_rect_ll_y < y
        && y < nav_rect_ur_y) {
      if (!nav_rect_hot) {
        fdraw->set_cursor(navw, draw_eCursor_CrossHair);
        nav_rect_hot = 1;
      }
    } else {
      if (nav_rect_hot) {
        fdraw->set_cursor(navw, draw_eCursor_Normal);
        nav_rect_hot = 0;
      }
    }
    break;
...
void FlowCtx::auto_scrolling_stop()
{
  ...
  fdraw->cancel_timer(auto_scrolling_id);
}
...
void FlowCtx::expand_select_rect(int x, int y) {
