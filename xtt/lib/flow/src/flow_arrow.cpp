...
void FlowArrow::print(void* pos, void* node, int highlight)
{
  ...
  ctx->current_print->arrow(p_dest.print_z_x + ((FlowPoint*)pos)->print_z_x,
      ...
      p2.print_z_y + ((FlowPoint*)pos)->print_z_y, draw_type, idx);
}

void FlowArrow::save(std::ofstream& fp, flow_eSaveMode mode)
{
  fp << int(flow_eSave_Arrow) << '\n';
  fp << int(flow_eSave_Arrow_arrow_width) << FSPACE << arrow_width << '\n';
  fp << int(flow_eSave_Arrow_arrow_length) << FSPACE << arrow_length << '\n';
  fp << int(flow_eSave_Arrow_draw_type) << FSPACE << int(draw_type) << '\n';
  fp << int(flow_eSave_Arrow_line_width) << FSPACE << line_width << '\n';
  fp << int(flow_eSave_Arrow_p_dest) << '\n';
  p_dest.save(fp, mode);
  fp << int(flow_eSave_Arrow_p1) << '\n';
  p1.save(fp, mode);
  fp << int(flow_eSave_Arrow_p2) << '\n';
  p2.save(fp, mode);
  fp << int(flow_eSave_End) << '\n';
}

void FlowArrow::open(std::ifstream& fp)
{
  int type;
  int end_found = 0;
  char dummy[40];
  ...
  for (;;) {
    ...
    case flow_eSave_Arrow:
      break;
    case flow_eSave_Arrow_arrow_width:
      fp >> arrow_width;
      break;
    case flow_eSave_Arrow_arrow_length:
      fp >> arrow_length;
      break;
    case flow_eSave_Arrow_draw_type:
      fp >> tmp;
      draw_type = (flow_eDrawType)tmp;
      break;
    case flow_eSave_Arrow_line_width:
      fp >> line_width;
      break;
    case flow_eSave_Arrow_p_dest:
      p_dest.open(fp);
      break;
    case flow_eSave_Arrow_p1:
      p1.open(fp);
      break;
    case flow_eSave_Arrow_p2:
      p2.open(fp);
      break;
    case flow_eSave_End:
      end_found = 1;
      break;
    default:
      std::cout << "FlowArrow:open syntax error\n";
