...
void FlowPoint::posit(double x1, double y1)
{
  x = x1;
  y = y1;
  z_x = ROUND(x * ctx->zoom_factor);
  z_y = ROUND(y * ctx->zoom_factor);
  nav_z_x = ROUND(x * ctx->nav_zoom_factor);
  nav_z_y = ROUND(y * ctx->nav_zoom_factor);
}
...
void FlowPoint::open(std::ifstream& fp)
{
  int type;
  int end_found = 0;
  char dummy[40];
  ...
  for (;;) {
    ...
    case flow_eSave_End:
      ...
      break;
    default:
      std::cout << "FlowPoint:open syntax error\n";
