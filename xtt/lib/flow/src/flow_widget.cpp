/* 
 * Proview   $Id: flow_widget.cpp,v 1.2 2005-09-01 14:56:12 claes Exp $
...
static void Redisplay( Widget w, XEvent *event, Region region)
{
  ...
  draw_event_handler( (FlowCtx *)((FlowWidget)w)->flow.flow_ctx, *event);
}
...
static void Destroy( Widget w)
{
  if ( ((FlowWidget) w)->flow.is_navigator)
    return;
  flow_draw_delete( (FlowCtx *)((FlowWidget)w)->flow.flow_ctx);
}
...
static void Realize( Widget w, unsigned long *mask, XSetWindowAttributes *swa)
{
  ...
  if ( ((FlowWidget) w)->flow.is_navigator)
  {
    if ( !((FlowWidget) w)->flow.flow_ctx)
    {
      ...
      flow_draw_init_nav( w, main_flow->flow.flow_ctx);
    }
  }
  else
  {
    if ( !((FlowWidget) w)->flow.flow_ctx)
    {
      flow_draw_init( w, &((FlowWidget) w)->flow.flow_ctx, 
		flow_init_proc, 
