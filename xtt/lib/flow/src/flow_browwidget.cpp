/* 
 * Proview   $Id: flow_browwidget.cpp,v 1.2 2005-09-01 14:56:12 claes Exp $
...
static void Redisplay( Widget w, XEvent *event, Region region)
{
  ...
  draw_event_handler( (BrowCtx *)((BrowWidget)w)->brow.brow_ctx, *event);
}
...
static void Destroy( Widget w)
{
  if ( ((BrowWidget) w)->brow.is_navigator)
    return;
  flow_draw_delete( (FlowCtx *)((BrowWidget)w)->brow.brow_ctx);
}
...
static Boolean AcceptFocus( Widget w, Time *t)
{
  if ( ((BrowWidget) w)->brow.is_navigator)
    return 0;
  flow_set_inputfocus( (FlowCtx *)((BrowWidget)w)->brow.brow_ctx);
  return 1;
}
...
static void Realize( Widget w, unsigned long *mask, XSetWindowAttributes *swa)
{
  ...
  if ( ((BrowWidget) w)->brow.is_navigator)
  {
    if ( !((BrowWidget) w)->brow.brow_ctx)
    {
      ...
      flow_draw_init_nav( w, main_brow->brow.brow_ctx);
    }
  }
  else
  {
    if ( !((BrowWidget) w)->brow.brow_ctx)
    {
      flow_draw_init( w, &((BrowWidget) w)->brow.brow_ctx, 
		brow_init_proc, 
