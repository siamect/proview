...
public:
  ...
  FlowArc(FlowCtx* flow_ctx, double x1 = 0, double y1 = 0, double x2 = 0,
      ...
      : ctx(flow_ctx), angle1(ang1), angle2(ang2), ll(flow_ctx, x1, y1),
        ur(flow_ctx, x2, y2), draw_type(d_type), line_width(line_w){}
  friend std::ostream& operator<<(std::ostream& o, const FlowArc a);
  void zoom();
  void nav_zoom();
  void print_zoom();
  ...
  void move(void* pos, double x1, double y1, double x2, double y2, int ang1,
      int ang2, int highlight, int dimmed, int hot);
