...
static void Redisplay(Widget w, XEvent* event, Region region)
{
  ((FlowDrawXLib*)((FlowCtx*)((FlowWidget)w)->flow.flow_ctx)->fdraw)
      ->event_handler((FlowCtx*)((FlowWidget)w)->flow.flow_ctx, *event);
}
...
{
  (*flowWidgetClass->core_class.superclass->core_class.realize)(w, mask, swa);
  ...
  if (((FlowWidget)w)->flow.is_navigator) {
    if (!((FlowWidget)w)->flow.flow_ctx) {
      ...
      ((FlowDrawXLib*)((FlowCtx*)((FlowWidget)w)->flow.flow_ctx)->fdraw)
          ->init_nav(w, main_flow->flow.flow_ctx);
    }
  } else {
    if (!((FlowWidget)w)->flow.flow_ctx) {
      FlowDrawXLib* fdraw
          = new FlowDrawXLib(w, &((FlowWidget)w)->flow.flow_ctx, flow_init_proc,
              ((FlowWidget)w)->flow.client_data, flow_eCtxType_Flow);
      ((FlowCtx*)((FlowWidget)w)->flow.flow_ctx)->fdraw = fdraw;
    }
  }
}

extern "C" Widget FlowCreate(Widget parent, char* name, ArgList args,
...
extern "C" Widget ScrolledFlowCreate(Widget parent, char* name, ArgList args,
    int argCount, int (*init_proc)(FlowCtx* ctx, void* client_data),
    void* client_data, Widget* flow_w)
{
  ...
  */
  flow = (FlowWidget)FlowCreate(
      form, (char*)"flow", arg, i, init_proc, client_data);
...
Widget FlowCreateNav(
    Widget parent, char* name, ArgList args, int argCount, Widget main_flow)
{
  FlowWidget w;
