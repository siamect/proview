...
static void Redisplay(Widget w, XEvent* event, Region region)
{
  ((FlowDrawXLib*)((BrowCtx*)((BrowWidget)w)->brow.brow_ctx)->fdraw)
      ->event_handler((BrowCtx*)((BrowWidget)w)->brow.brow_ctx, *event);
}
...
static Boolean AcceptFocus(Widget w, Time* t)
{
  if (((BrowWidget)w)->brow.is_navigator)
    return 0;
  ((FlowCtx*)((BrowWidget)w)->brow.brow_ctx)
      ->fdraw->set_inputfocus((FlowCtx*)((BrowWidget)w)->brow.brow_ctx);
  return 1;
}

static void Realize(Widget w, unsigned long* mask, XSetWindowAttributes* swa)
{
  (*browWidgetClass->core_class.superclass->core_class.realize)(w, mask, swa);
  ...
  if (((BrowWidget)w)->brow.is_navigator) {
    if (!((BrowWidget)w)->brow.brow_ctx) {
      ...
      ((FlowDrawXLib*)((BrowCtx*)((BrowWidget)w)->brow.brow_ctx)->fdraw)
          ->init_nav(w, main_brow->brow.brow_ctx);
    }
  } else {
    if (!((BrowWidget)w)->brow.brow_ctx) {
      FlowDrawXLib* fdraw
          = new FlowDrawXLib(w, &((BrowWidget)w)->brow.brow_ctx, brow_init_proc,
              ((BrowWidget)w)->brow.client_data, flow_eCtxType_Brow);
      ((BrowCtx*)((BrowWidget)w)->brow.brow_ctx)->fdraw = fdraw;
    }
  }
}

extern "C" Widget BrowCreate(Widget parent, char* name, ArgList args,
    int argCount, int (*init_proc)(FlowCtx* ctx, void* client_data),
    void* client_data)
{
  BrowWidget w;
...
extern "C" Widget ScrolledBrowCreate(Widget parent, char* name, ArgList args,
    int argCount, int (*init_proc)(FlowCtx* ctx, void* client_data),
    void* client_data, Widget* brow_w)
{
  ...
  */
  brow = (BrowWidget)BrowCreate(
      form, (char*)"brow", arg, i, init_proc, client_data);
