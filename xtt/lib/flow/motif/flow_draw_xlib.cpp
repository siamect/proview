...
static int flow_create_gc(FlowDrawXLib* draw_ctx, Window window)
{
  ...
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    ...
    draw_ctx->gcs[flow_eDrawType_Line][i] = XCreateGC(draw_ctx->display, window,
  ...
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    ...
    draw_ctx->gcs[flow_eDrawType_LineDashed][i]
        = XCreateGC(draw_ctx->display, window, GCForeground | GCBackground
                | GCLineWidth | GCLineStyle | GCDashOffset | GCDashList,
            &xgcv);
  }
  ...
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    ...
    draw_ctx->gcs[flow_eDrawType_LineDashedRed][i]
        = XCreateGC(draw_ctx->display, window, GCForeground | GCBackground
                | GCLineWidth | GCLineStyle | GCDashOffset | GCDashList,
            &xgcv);
  }
  ...
  xgcv.background = draw_ctx->background;
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    draw_ctx->gcs[flow_eDrawType_TextHelvetica][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground, &xgcv);
  }

  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    draw_ctx->gcs[flow_eDrawType_TextHelveticaBold][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground, &xgcv);
  }
  ...
  xgcv.background = draw_ctx->background;
  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    draw_ctx->gcs[flow_eDrawType_TextHelveticaErase][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground, &xgcv);
  }

  for (i = 0; i < DRAW_TYPE_SIZE; i++) {
    draw_ctx->gcs[flow_eDrawType_TextHelveticaEraseBold][i] = XCreateGC(
        draw_ctx->display, window, GCForeground | GCBackground, &xgcv);
  }

  for (i = 0; i < DRAW_FONT_SIZE; i++) {
    font
        = XLoadFont(draw_ctx->display, font_names[draw_eFont_HelveticaBold][i]);
  ...
  for (i = 0; i < DRAW_FONT_SIZE; i++) {
    ...
    XSetFont(draw_ctx->display, draw_ctx->gcs[flow_eDrawType_TextHelvetica][i],
        font);
    XSetFont(draw_ctx->display,
...
int FlowDrawXLib::create_secondary_ctx(FlowCtx* flow_ctx,
...
int FlowDrawXLib::change_ctx(FlowCtx* from_ctx, FlowCtx* to_ctx)
{
...
FlowDrawXLib::FlowDrawXLib(Widget x_toplevel, void** flow_ctx,
    int (*init_proc)(Widget w, FlowCtx* ctx, void* client_data),
    void* client_data, flow_eCtxType type)
    : toplevel(x_toplevel), nav_shell(0), nav_toplevel(0), app_ctx(0),
      display(0), window(0), nav_window(0), screen(0), background(0),
      timer_id(0)
{
  int i;
  Arg args[20];
...
int FlowDrawXLib::event_handler(FlowCtx* ctx, XEvent event)
{
  ...
  if (event.xany.window == window) {
    ...
    case ButtonPress:
      ...
      case Button3:
        ctx->event_handler(
            flow_eEvent_MB3Down, event.xbutton.x, event.xbutton.y, 0, 0);
        if (click_sensitivity & flow_mSensitivity_MB3Press
          ...
          ctx->event_handler(
              flow_eEvent_MB3Press, event.xbutton.x, event.xbutton.y, 0, 0);
          click_sensitivity = 0;
          return 1;
        }
        break;
      ...
      if (!button_pressed) {
        ...
        case Button1:
          ...
          if ((event.xbutton.state & ShiftMask)
            ...
            sts = ctx->event_handler(flow_eEvent_MB1PressShift, event.xbutton.x,
                event.xbutton.y, 0, 0);
          else if (!(event.xbutton.state & ShiftMask)
              && (event.xbutton.state & ControlMask))
            sts = ctx->event_handler(flow_eEvent_MB1PressCtrl, event.xbutton.x,
                event.xbutton.y, 0, 0);
          else if ((event.xbutton.state & ShiftMask)
              && (event.xbutton.state & ControlMask))
            ...
            ctx->event_handler(
                flow_eEvent_MB1Press, event.xbutton.x, event.xbutton.y, 0, 0);
          break;
        case Button2:
          button2_pressed = 1;
          if ((event.xbutton.state & ShiftMask)
              && !(event.xbutton.state & ControlMask))
            sts = ctx->event_handler(flow_eEvent_MB2PressShift, event.xbutton.x,
                event.xbutton.y, 0, 0);
          else if (!(event.xbutton.state & ShiftMask)
              && (event.xbutton.state & ControlMask))
            sts = ctx->event_handler(flow_eEvent_MB2PressShift, event.xbutton.x,
            ...
            sts = ctx->event_handler(
                flow_eEvent_MB2Press, event.xbutton.x, event.xbutton.y, 0, 0);
          break;
        case Button3:
          button3_pressed = 1;
          sts = ctx->event_handler(
        ...
        if (button_clicked) {
          ...
          case Button1:
            if ((event.xbutton.state & ShiftMask)
              ...
              sts = ctx->event_handler(
                  flow_eEvent_MB1Click, event.xbutton.x, event.xbutton.y, 0, 0);
            break;
          case Button2:
            if ((event.xbutton.state & ShiftMask)
                && !(event.xbutton.state & ControlMask))
              ...
              sts = ctx->event_handler(
                  flow_eEvent_MB2Click, event.xbutton.x, event.xbutton.y, 0, 0);
            break;
          case Button3:
            sts = ctx->event_handler(
                flow_eEvent_MB3Click, event.xbutton.x, event.xbutton.y, 0, 0);
            break;
          }
        } else {
          /* Button release */
          sts = ctx->event_handler(flow_eEvent_ButtonRelease, event.xbutton.x,
              event.xbutton.y, 0, 0);
        }
      } else {
        /* Button click */
    ...
    case MotionNotify:
      ...
      if (button1_pressed || button2_pressed || button3_pressed)
        sts = ctx->event_handler(
            flow_eEvent_ButtonMotion, event.xbutton.x, event.xbutton.y, 0, 0);
      else
        sts = ctx->event_handler(
            flow_eEvent_CursorMotion, event.xbutton.x, event.xbutton.y, 0, 0);
      break;
    case EnterNotify:
      sts = ctx->event_handler(
          flow_eEvent_Enter, event.xbutton.x, event.xbutton.y, 0, 0);
      break;
    case LeaveNotify:
      sts = ctx->event_handler(
          flow_eEvent_Leave, event.xbutton.x, event.xbutton.y, 0, 0);
      break;
    case MapNotify:
    ...
    case ButtonPress:
      ...
      case Button1:
        ...
        sts = ctx->event_handler_nav(
            flow_eEvent_MB1Press, event.xbutton.x, event.xbutton.y);
        break;
      case Button2:
        button2_pressed = 1;
        sts = ctx->event_handler_nav(
            flow_eEvent_MB2Press, event.xbutton.x, event.xbutton.y);
        break;
      case Button3:
        button3_pressed = 1;
        break;
    ...
    case ButtonRelease:
      ...
      sts = ctx->event_handler_nav(
          flow_eEvent_ButtonRelease, event.xbutton.x, event.xbutton.y);
      break;
    case Expose:
      sts = ctx->event_handler_nav(
          flow_eEvent_Exposure, event.xbutton.x, event.xbutton.y);
      break;
    case MotionNotify:
      if (button1_pressed || button2_pressed || button3_pressed)
        sts = ctx->event_handler_nav(
            flow_eEvent_ButtonMotion, event.xbutton.x, event.xbutton.y);
      else
        sts = ctx->event_handler_nav(
            flow_eEvent_CursorMotion, event.xbutton.x, event.xbutton.y);
      break;
    }
  }
    ...
    flow_eDrawType gc_type, int idx, int highlight, int dimmed)
{
  ...
  XDrawRectangle(display, window, gcs[gc_type][idx], x, y, width, height);
  return 1;
}

int FlowDrawXLib::rect_erase(
    FlowCtx* ctx, int x, int y, int width, int height, int idx)
{
  ...
  XDrawRectangle(
      display, window, gcs[flow_eDrawType_LineErase][idx], x, y, width, height);
  return 1;
}
    ...
    flow_eDrawType gc_type, int idx, int highlight)
{
  ...
  XDrawRectangle(
      display, nav_window, gcs[gc_type + highlight][idx], x, y, width, height);
  return 1;
}

int FlowDrawXLib::nav_rect_erase(
    FlowCtx* ctx, int x, int y, int width, int height, int idx)
{
  if (ctx->no_nav || ctx->nodraw)
    return 1;

  XDrawRectangle(display, nav_window, gcs[flow_eDrawType_LineErase][idx], x, y,
      width, height);
  return 1;
}

int FlowDrawXLib::arrow(FlowCtx* ctx, int x1, int y1, int x2, int y2, int x3,
    int y3, flow_eDrawType gc_type, int idx, int highlight)
{
  ...
  XFillPolygon(display, window, gcs[gc_type + highlight][idx], p, 4, Complex,
      CoordModeOrigin);
  return 1;
}

int FlowDrawXLib::arrow_erase(
    FlowCtx* ctx, int x1, int y1, int x2, int y2, int x3, int y3, int idx)
{
  ...
  XFillPolygon(display, window, gcs[flow_eDrawType_LineErase][idx], p, 4,
      Complex, CoordModeOrigin);
  return 1;
}

int FlowDrawXLib::nav_arrow(FlowCtx* ctx, int x1, int y1, int x2, int y2,
    int x3, int y3, flow_eDrawType gc_type, int idx, int highlight)
{
  XPoint p[4] = { { x1, y1 }, { x2, y2 }, { x3, y3 }, { x1, y1 } };
  if (ctx->no_nav || ctx->nodraw)
    return 1;
  ...
  XFillPolygon(display, nav_window, gcs[gc_type + highlight][idx], p, 4,
      Complex, CoordModeOrigin);
  return 1;
}

int FlowDrawXLib::nav_arrow_erase(
    FlowCtx* ctx, int x1, int y1, int x2, int y2, int x3, int y3, int idx)
{
  XPoint p[4] = { { x1, y1 }, { x2, y2 }, { x3, y3 }, { x1, y1 } };
  if (ctx->no_nav || ctx->nodraw)
  ...
  XFillPolygon(display, nav_window, gcs[flow_eDrawType_LineErase][idx], p, 4,
      Complex, CoordModeOrigin);
  return 1;
}
    ...
    int dimmed)
{
  ...
  XDrawArc(display, window, gcs[gc_type][idx], x, y, width, height, angle1 * 64,
      angle2 * 64);
  return 1;
}
    ...
    int angle1, int angle2, int idx)
{
  ...
  XDrawArc(display, window, gcs[flow_eDrawType_LineErase][idx], x, y, width,
    ...
    int angle1, int angle2, flow_eDrawType gc_type, int idx, int highlight)
{
  ...
  XDrawArc(display, nav_window, gcs[gc_type + highlight][idx], x, y, width,
      height, angle1 * 64, angle2 * 64);
  return 1;
}

int FlowDrawXLib::nav_arc_erase(FlowCtx* ctx, int x, int y, int width,
    int height, int angle1, int angle2, int idx)
{
  if (ctx->no_nav || ctx->nodraw)
    return 1;

  XDrawArc(display, nav_window, gcs[flow_eDrawType_LineErase][idx], x, y, width,
      height, angle1 * 64, angle2 * 64);
  return 1;
}
    ...
    flow_eDrawType gc_type, int idx, int highlight, int dimmed)
{
  ...
  XDrawLine(display, window, gcs[gc_type][idx], x1, y1, x2, y2);
  return 1;
}

int FlowDrawXLib::line_erase(
    FlowCtx* ctx, int x1, int y1, int x2, int y2, int idx)
{
  ...
  XDrawLine(
      display, window, gcs[flow_eDrawType_LineErase][idx], x1, y1, x2, y2);
  return 1;
}
    ...
    flow_eDrawType gc_type, int idx, int highlight)
{
  ...
  XDrawLine(display, nav_window, gcs[gc_type + highlight][idx], x1, y1, x2, y2);
  return 1;
}

int FlowDrawXLib::nav_line_erase(
    FlowCtx* ctx, int x1, int y1, int x2, int y2, int idx)
{
  if (ctx->no_nav || ctx->nodraw)
    return 1;
  ...
  XDrawLine(
      display, nav_window, gcs[flow_eDrawType_LineErase][idx], x1, y1, x2, y2);
  return 1;
}
    ...
    double size)
{
  ...
  XDrawString(display, window, gcs[gc_type][idx], x, y, text, len);
  return 1;
}
    ...
    flow_eDrawType gc_type, int idx, int highlight, int line, double size)
{
  ...
  XDrawString(display, nav_window, gcs[gc_type][idx], x, y, text, len);
  return 1;
}

int FlowDrawXLib::nav_text_erase(FlowCtx* ctx, int x, int y, char* text,
...
int FlowDrawXLib::pixmaps_create(
  ...
  for (i = 0; i < DRAW_PIXMAP_SIZE; i++) {
    if (i == 0 || (i > 0 && pdata->bits != prev_pdata->bits))
      pms->pixmap[i] = XCreateBitmapFromData(
          display, window, pdata->bits, pdata->width, pdata->height);
    else
...
int FlowDrawXLib::pixmap(FlowCtx* ctx, int x, int y,
    ...
    int idx, int highlight, int line)
{
  ...
  XCopyPlane(display, pms->pixmap[idx], window, gcs[gc_type][idx], 0, 0,
      pdata->width, pdata->height, x, y, 1);
  return 1;
}

int FlowDrawXLib::pixmap_inverse(FlowCtx* ctx, int x, int y,
    flow_sPixmapData* pixmap_data, void* pixmaps, flow_eDrawType gc_type,
    int idx, int line)
{
  draw_sPixmap* pms;
  ...
  XCopyPlane(display, pms->pixmap[idx], window, gc_inverse, 0, 0, pdata->width,
      pdata->height, x, y, 1);
  return 1;
}

int FlowDrawXLib::pixmap_erase(FlowCtx* ctx, int x, int y,
    flow_sPixmapData* pixmap_data, void* pixmaps, flow_eDrawType gc_type,
    int idx, int line)
{
  flow_sPixmapDataElem* pdata = (flow_sPixmapDataElem*)pixmap_data + idx;
  ...
  XClearArea(display, window, x, y, pdata->width, pdata->height, False);
  return 1;
}

int FlowDrawXLib::nav_pixmap(FlowCtx* ctx, int x, int y,
    flow_sPixmapData* pixmap_data, void* pixmaps, flow_eDrawType gc_type,
    int idx, int highlight, int line)
{
  draw_sPixmap* pms;
  ...
  XCopyPlane(display, pms->pixmap[idx], nav_window, gcs[gc_type][idx], 0, 0,
      pdata->width, pdata->height, x, y, 1);
  return 1;
}

int FlowDrawXLib::nav_pixmap_erase(FlowCtx* ctx, int x, int y,
    flow_sPixmapData* pixmap_data, void* pixmaps, flow_eDrawType gc_type,
    int idx, int line)
{
  flow_sPixmapDataElem* pdata = (flow_sPixmapDataElem*)pixmap_data + idx;
  ...
  XClearArea(display, nav_window, x, y, pdata->width, pdata->height, False);
  return 1;
}

int FlowDrawXLib::fill_rect(
    FlowCtx* ctx, int x, int y, int w, int h, flow_eDrawType gc_type)
{
  ...
  if (gc_type == flow_eDrawType_Green)
    XFillPolygon(display, window, gc_green, p, 5, Convex, CoordModeOrigin);
  else if (gc_type == flow_eDrawType_Yellow)
    XFillPolygon(display, window, gc_yellow, p, 5, Convex, CoordModeOrigin);
  else if (gc_type == flow_eDrawType_DarkGray)
    XFillPolygon(display, window, gc_darkgray, p, 5, Convex, CoordModeOrigin);
  else
    XFillPolygon(
        display, window, gcs[gc_type][0], p, 5, Convex, CoordModeOrigin);
  return 1;
}
    ...
    flow_tImImage im, flow_tPixmap pixmap, flow_tPixmap clip_mask)
{
  ...
  XCopyArea(display, (Pixmap)pixmap, window, gcs[flow_eDrawType_Line][0], 0, 0,
      width, height, x, y);
...
void FlowDrawXLib::set_nav_window_size(FlowCtx* ctx, int width, int height)
{
  ...
  XConfigureWindow(display, nav_window, CWWidth | CWHeight, &xwc);
}
...
static void event_timer(FlowCtx* ctx, int time_ms)
{
  FlowDrawXLib* draw_ctx = (FlowDrawXLib*)ctx->fdraw;
  draw_ctx->timer_id
      = XtAppAddTimeOut(XtWidgetToApplicationContext(draw_ctx->toplevel),
          time_ms, (XtTimerCallbackProc)event_timer_cb, ctx);
}

void FlowDrawXLib::set_timer(
    FlowCtx* ctx, int time_ms, void (*callback_func)(FlowCtx* ctx), void** id)
{
  ...
  timer_cb->timer_id = XtAppAddTimeOut(XtWidgetToApplicationContext(toplevel),
      time_ms, (XtTimerCallbackProc)draw_timer_cb, timer_cb);
...
void FlowDrawXLib::set_nav_cursor(FlowCtx* ctx, draw_eCursor cursor)
{
  ...
  if (cursor == draw_eCursor_Normal)
    ...
    XDefineCursor(display, nav_window, cursors[cursor]);
  XFlush(display);
}
    ...
    flow_eDrawType gc_type, int idx, int* width, int* height, double size)
{
  ...
  XTextExtents(font_struct[font_idx][idx], text, len, &direction, &ascent,
      &descent, &char_struct);
...
void FlowDrawXLib::copy_area(FlowCtx* ctx, int x, int y)
{
  ...
  if (x >= 0 && y >= 0) {
    XCopyArea(display, window, window, gc, 0, 0, ctx->window_width - x,
        ctx->window_height - y, x, y);
    if (x)
      XClearArea(display, window, 0, 0, x, ctx->window_height, 0);
    if (y)
      XClearArea(display, window, x, 0, ctx->window_width, y, 0);
  } else if (x <= 0 && y <= 0) {
    XCopyArea(display, window, window, gc, -x, -y, ctx->window_width + x,
        ctx->window_height + y, 0, 0);
    if (x)
      XClearArea(display, window, ctx->window_width + x, 0, ctx->window_width,
          ctx->window_height, 0);
    if (y)
      XClearArea(display, window, 0, ctx->window_height + y,
          ctx->window_width + x, ctx->window_height, 0);
  } else if (x <= 0 && y >= 0) {
    XCopyArea(display, window, window, gc, -x, 0, ctx->window_width + x,
        ctx->window_height - y, 0, y);
    if (x)
      XClearArea(display, window, ctx->window_width + x, 0, ctx->window_width,
          ctx->window_height, 0);
    if (y)
      XClearArea(display, window, 0, 0, ctx->window_width + x, y, 0);
  } else {
    XCopyArea(display, window, window, gc, 0, -y, ctx->window_width - x,
        ctx->window_height + y, x, 0);
    if (x)
      XClearArea(display, window, 0, 0, x, ctx->window_height, 0);
    if (y)
      XClearArea(display, window, x, ctx->window_height + y, ctx->window_width,
          ctx->window_height, 0);
  }
}

void FlowDrawXLib::clear_area(
    FlowCtx* ctx, int ll_x, int ur_x, int ll_y, int ur_y)
{
  if (ctx->nodraw)
    return;

  XClearArea(display, window, ll_x, ll_y, ur_x - ll_x, ur_y - ll_y, 0);
}
    ...
    int idx, int width, int height, void* node, int number, void** data)
{
  ...
  entry = XmFontListEntryCreate(
      (char*)"tag", XmFONT_IS_FONT, font_struct[0][idx]);
    ...
    FlowCtx* ctx, void* data, int x, int y, flow_ePosition pos_type)
{
  ...
  XtMoveWidget(
      annot_data->w, annot_data->x, annot_data->y - annot_data->height);
}
...
void FlowDrawXLib::set_image_clip_mask(
    FlowCtx* ctx, flow_tPixmap pixmap, int x, int y)
{
  XSetClipMask(display, gcs[flow_eDrawType_Line][0], (Pixmap)pixmap);
  XSetClipOrigin(display, gcs[flow_eDrawType_Line][0], x, y);
}

void FlowDrawXLib::reset_image_clip_mask(FlowCtx* ctx)
{
  XSetClipMask(display, gcs[flow_eDrawType_Line][0], None);
...
void FlowDrawXLib::image_scale(float scale, flow_tImImage* im,
    flow_tPixmap* im_pixmap, flow_tPixmap* im_mask)
{
  ...
  Imlib_render(
      (ImlibData*)imlib, (ImlibImage*)*im, current_width, current_height);

  if (*im_pixmap)
    Imlib_free_pixmap((ImlibData*)imlib, (Pixmap)*im_pixmap);
  *im_pixmap
      = (flow_tPixmap)Imlib_move_image((ImlibData*)imlib, (ImlibImage*)*im);
  *im_mask = (flow_tPixmap)Imlib_move_mask((ImlibData*)imlib, (ImlibImage*)*im);
#endif
}

int FlowDrawXLib::image_load(const char* imagefile, float scale,
    float nav_scale, flow_tImImage* orig_im, flow_tImImage* im,
    flow_tPixmap* im_pixmap, flow_tPixmap* im_mask, flow_tPixmap* im_nav_pixmap,
    flow_tPixmap* im_nav_mask)
{
  ...
  *im_mask = (flow_tPixmap)Imlib_move_mask((ImlibData*)imlib, (ImlibImage*)*im);
  ...
  Imlib_render((ImlibData*)imlib, (ImlibImage*)*im, current_nav_width,
      current_nav_height);
  *im_nav_pixmap
      = (flow_tPixmap)Imlib_move_image((ImlibData*)imlib, (ImlibImage*)*im);
  *im_nav_mask
      = (flow_tPixmap)Imlib_move_mask((ImlibData*)imlib, (ImlibImage*)*im);
#endif
  return 1;
}
