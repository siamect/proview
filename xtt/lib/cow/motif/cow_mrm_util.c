...
void mrm_TextInit(Widget w, XtActionProc value_changed_cb, mrm_eUtility utility)
{
  ...
  case mrm_eUtility_Admin: {
    ...
    static XtActionsRec actions_wtt[1] = { { "key_down_wtt", 0 } };
    actions_wtt[0].proc = value_changed_cb;

    if (compiled_translations_wtt == NULL)
      XtAppAddActions(
          XtWidgetToApplicationContext(w), actions_wtt, XtNumber(actions_wtt));
  ...
  case mrm_eUtility_Ge: {
    ...
    static XtActionsRec actions_ge[1] = { { "key_down_ge", 0 } };
    actions_ge[0].proc = value_changed_cb;

    if (compiled_translations_ge == NULL)
      XtAppAddActions(
          XtWidgetToApplicationContext(w), actions_ge, XtNumber(actions_ge));
  ...
  case mrm_eUtility_XAtt: {
    ...
    static XtActionsRec actions_watt[1] = { { "key_down_watt", 0 } };
    actions_watt[0].proc = value_changed_cb;

    if (compiled_translations_watt == NULL)
      XtAppAddActions(XtWidgetToApplicationContext(w), actions_watt,
          XtNumber(actions_watt));
  ...
  case mrm_eUtility_XAttOne: {
    ...
    static XtActionsRec actions_wda[1] = { { "key_down_wda", 0 } };
    actions_wda[0].proc = value_changed_cb;

    if (compiled_translations_wda == NULL)
      XtAppAddActions(
          XtWidgetToApplicationContext(w), actions_wda, XtNumber(actions_wda));
  ...
  case mrm_eUtility_Attr: {
    ...
    static XtActionsRec actions_attr[1] = { { "key_down_attr", 0 } };
    actions_attr[0].proc = value_changed_cb;

    if (compiled_translations_attr == NULL)
      XtAppAddActions(XtWidgetToApplicationContext(w), actions_attr,
          XtNumber(actions_attr));
  ...
  case mrm_eUtility_GsdAttr: {
    ...
    static XtActionsRec actions_gsdattr[1] = { { "key_down_gsdattr", 0 } };
    actions_gsdattr[0].proc = value_changed_cb;

    if (compiled_translations_gsdattr == NULL)
      XtAppAddActions(XtWidgetToApplicationContext(w), actions_gsdattr,
          XtNumber(actions_gsdattr));

    if (compiled_translations_gsdattr == NULL)
      compiled_translations_gsdattr
          = XtParseTranslationTable(translations_gsdattr);
    ...
    int recall_size, int* current_recall_line)
{
  ...
  XtTranslateKeycode(
      flow_Display(w), event->xkey.keycode, event->xkey.state, &mod, &keysym);
  keysym &= 0xFFFF;
  switch (keysym) {
  case XK_Return:
  case XK_Linefeed:
  ...
  case XK_Up:
    ...
    XmTextSetCursorPosition(
        w, strlen(recall + (*current_recall_line - 1) * line_size));
    break;
  case XK_Down:
...
#define WM_STATE_ELEMENTS 1
  ...
  if (WmStateAtom) {
    sts = XGetWindowProperty(XtDisplay(Shell), XtWindow(Shell), WmStateAtom, 0L,
        WM_STATE_ELEMENTS, FALSE, WmStateAtom, &atom_ret, &format_ret,
        &nitems_ret, &bytesleft, (unsigned char**)&IconStateData);

    if (!(sts == Success && atom_ret == WmStateAtom
