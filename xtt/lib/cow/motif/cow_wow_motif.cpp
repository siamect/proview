...
void CoWowMotif::question_ok_cb(
    Widget dialog, XtPointer data, XmAnyCallbackStruct* cbs)
{
...
void CoWowMotif::question_cancel_cb(
    Widget dialog, XtPointer data, XmAnyCallbackStruct* cbs)
{
  wow_t_question_cb* cbdata = (wow_t_question_cb*)data;
...
void CoWowMotif::question_help_cb(
    Widget dialog, XtPointer data, XmAnyCallbackStruct* cbs)
{
  wow_t_question_cb* cbdata = (wow_t_question_cb*)data;
    ...
    void (*questionbox_cancel)(void*, void*), void* data)
{
  ...
  question_widget
      = XmCreateQuestionDialog(m_parent, (char*)"questionDialog", arg, 10);
  XtAddCallback(
      question_widget, XmNokCallback, (XtCallbackProc)question_ok_cb, cbdata);
...
void CoWowMotif::DisplayError(const char* title, const char* text)
{
  ...
  XtAddCallback(err_widget, XmNokCallback, (XtCallbackProc)error_ok_cb, NULL);
...
void CoWowMotif::list_ok_cb(Widget w, XtPointer data, XmAnyCallbackStruct* cbs)
{
...
void CoWowMotif::list_cancel_cb(
    Widget w, XtPointer data, XmAnyCallbackStruct* cbs)
{
  wow_tListCtx ctx = (wow_tListCtx)data;
...
void CoWowMotif::list_action_cb(
    Widget w, XtPointer data, XmListCallbackStruct* cbs)
{
  wow_tListCtx ctx = (wow_tListCtx)data;
  char* item_str;
    ...
    void* parent_ctx, int show_apply_button)
{
  ...
  i = 0;
  XtSetArg(args[i], XmNiconName, title);
  i++;

  ctx->toplevel
      = XtCreatePopupShell(title, topLevelShellWidgetClass, m_parent, args, i);
  ...
  i = 0;
  XtSetArg(args[i], XmNbuttonFontList, fontlist);
  i++;
  ...
  i++;
  XtSetArg(args[i], XmNresizePolicy, XmRESIZE_NONE);
  i++;
  ...
  i++;
  XtSetArg(args[i], XmNbottomOffset, 20);
  i++;
  XtSetArg(args[i], XmNleftAttachment, XmATTACH_FORM);
  i++;
  XtSetArg(args[i], XmNleftOffset, 20);
  i++;
  XtSetArg(args[i], XmNwidth, 50);
  i++;
  XtSetArg(args[i], XmNlabelString, cstr);
  i++;

  ok_button = XmCreatePushButton(form, (char*)"okButton", args, i);
  XtAddCallback(
      ok_button, XmNactivateCallback, (XtCallbackProc)list_ok_cb, ctx);
  ...
  i++;
  XtSetArg(args[i], XmNrightOffset, 20);
  i++;
  XtSetArg(args[i], XmNwidth, 50);
  i++;
  XtSetArg(args[i], XmNlabelString, cstr);
  ...
  XtAddCallback(
      cancel_button, XmNactivateCallback, (XtCallbackProc)list_cancel_cb, ctx);
  XtManageChild(cancel_button);

  XmStringFree(cstr);

  i = 0;
  XtSetArg(args[i], XmNdefaultButton, ok_button);
  i++;
  XtSetArg(args[i], XmNcancelButton, cancel_button);
  i++;
  ...
  i++;
  XtSetArg(args[i], XmNbottomWidget, ok_button);
  i++;
  XtSetArg(args[i], XmNbottomOffset, 15);
  i++;
  XtSetArg(args[i], XmNrightAttachment, XmATTACH_FORM);
  i++;
  XtSetArg(args[i], XmNrightOffset, 15);
  i++;
  XtSetArg(args[i], XmNtopAttachment, XmATTACH_FORM);
  i++;
  XtSetArg(args[i], XmNtopOffset, 15);
  i++;
  XtSetArg(args[i], XmNleftAttachment, XmATTACH_FORM);
  i++;
  XtSetArg(args[i], XmNleftOffset, 15);
  i++;
  XtSetArg(args[i], XmNselectionPolicy, XmSINGLE_SELECT);
  i++;
  XtSetArg(args[i], XmNfontList, fontlist);
  i++;
  ctx->list = XmCreateScrolledList(form, (char*)"scrolledList", args, i);
  XtAddCallback(
      ctx->list, XmNdefaultActionCallback, (XtCallbackProc)list_action_cb, ctx);
...
void CoWowMotif::file_search_cb(Widget widget, XtPointer data)
{
  ...
  if (file_cnt) {
    XtVaSetValues(widget, XmNfileListItems, names, XmNfileListItemCount,
        file_cnt, XmNdirSpec, names[0], XmNlistUpdated, True, NULL);
    for (i = 0; i < file_cnt; i++)
      XmStringFree(names[i]);
  } else
    XtVaSetValues(widget, XmNfileListItems, NULL, XmNfileListItemCount, 0,
    ...
    wow_eFileSelType file_type)
{
  ...
  i = 0;
  XtSetArg(args[i], XmNdialogTitle, ctitle);
  i++;
  XtSetArg(args[i], XmNfileTypeMask, XmFILE_REGULAR);
  i++;
  XtSetArg(args[i], XmNfileSearchProc, file_search_cb);
  i++;
  ...
  case wow_eFileSelType_Dbs:
    ...
    XtSetArg(args[i], XmNdirectory, cdirectory);
    i++;
    XtSetArg(args[i], XmNpattern, cpattern);
    i++;
    break;
  ...
  ctx->dialog
      = XmCreateFileSelectionDialog(m_parent, (char*)"fileseldia", args, i);
...
int CoWowMotif::DisplayWarranty()
{
  ...
  okstr
      = XmStringCreateLtoR(lng_translate("I Accept"), XmSTRING_DEFAULT_CHARSET);
  cancelstr
      = XmStringCreateLtoR(lng_translate("Quit"), XmSTRING_DEFAULT_CHARSET);
  helpstr = XmStringCreateLtoR(
      lng_translate("Show License"), XmSTRING_DEFAULT_CHARSET);
  XtSetArg(arg[0], XmNheight, 75);
  ...
  question_widget
      = XmCreateMessageDialog(m_parent, (char*)"questionDialog", arg, 12);
  XtAddCallback(
      question_widget, XmNokCallback, (XtCallbackProc)question_ok_cb, cbdata);
  XtAddCallback(question_widget, XmNcancelCallback,
      (XtCallbackProc)question_cancel_cb, cbdata);
...
void CoWowMotif::DisplayLicense()
{
  ...
  question_widget
      = XmCreateMessageDialog(m_parent, (char*)"questionDialog", arg, 9);
  XmStringFree(CStr2);
  XmStringFree(TitleStr);
  XmStringFree(cancelstr);
  XmFontListFree(fontlist);
  ...
  XtVaSetValues(XtParent(w), XmNleftAttachment, XmATTACH_FORM,
      XmNrightAttachment, XmATTACH_FORM, XmNtopAttachment, XmATTACH_FORM,
      XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, wcancel, NULL);
  XtManageChild(w);
...
void CoWowMotif::GetAtoms(
    Widget w, Atom* graph_atom, Atom* objid_atom, Atom* attrref_atom)
{
  if (graph_atom)
...
static void wow_get_selection_cb(Widget w, XtPointer clientdata,
    Atom* selection, Atom* type, XtPointer value, unsigned long* len,
    int* format)
{
...
int CoWowMotif::GetSelection(Widget w, char* str, int size, Atom atom)
{
  ...
  data.atom = atom;
  XtGetSelectionValue(
      w, XA_PRIMARY, atom, wow_get_selection_cb, &data, CurrentTime);
...
void CoWowMotif::PopupPosition(
    Widget parent, int x_event, int y_event, int* x, int* y)
{
  short x0, y0, x1, y1;
  Arg args[2];
  Widget grandparent;
...
void CoWowFocusTimerMotif::disable(Widget w, int time)
{
  ...
  if (request_cnt > 1)
    request_cnt = 0;
  focus_timerid = XtAppAddTimeOut(
      XtWidgetToApplicationContext(w), time, enable_set_focus, this);
}
